...
export default function usePmsSession(){
  ...
  const load=useCallback(async()=>{setStatus("loading");setError("");try{const{data:userData,error:userError}=await supabase.auth.getUser();if(userError){if(isMissingSession(userError)){setUser(null);setProperties([]);setPropertyId(null);setStatus("unauthenticated");return}throw userError}const currentUser=userData?.user||null;setUser(currentUser);if(!currentUser){setProperties([]);setPropertyId(null);setStatus("unauthenticated");return}
