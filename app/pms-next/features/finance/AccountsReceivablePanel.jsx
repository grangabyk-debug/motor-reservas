...
export default function AccountsReceivablePanel({propertyId}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[partnerRes,docRes,resRes,settingsRes]=await Promise.all([
        ...
        supabase.from("hotel_finance_documents").select("id,partner_id,reservation_id,document_type,number,status,currency,total,balance,billing_to,due_at,issued_at,created_at").eq("property_id",propertyId).not("partner_id","is",null).order("created_at",{ascending:false}).limit(300),
  ...
  async function createAccount(){
    ...
    try{
      ...
      const payload={property_id:propertyId,reservation_id:Number(reservation.id),partner_id:partner.id,document_type:"invoice",number:null,status:"issued",currency:reservation.moneda||baseCurrency,subtotal,tax,total,balance:total,billing_to:{name:partner.name,tax_id:partner.tax_id||null,email:partner.email||null,phone:partner.phone||null,contact_name:partner.contact_name||null},items:[{description:`Estadía ${reservation.fecha_entrada} → ${reservation.fecha_salida} · ${reservation.nombre_huesped||"Huésped"}`,quantity:1,unit_price:subtotal,tax_rate:rate,subtotal}],issued_at:new Date().toISOString(),due_at:accountDraft.due_at||null,notes:accountDraft.notes?.trim()||null,created_by:userData?.user?.id||null,billing_mode:"manual"}
      const{error:insertError}=await supabase.from("hotel_finance_documents").insert(payload);if(insertError)throw insertError
      setAccountOpen(false);setNotice(`Cuenta creada para ${partner.name}.`);await load()
