...
const headText={display:"grid",gap:3,minWidth:0},headButton={height:31,border:"1px solid color-mix(in srgb,var(--accent) 24%,var(--line))",borderRadius:9,padding:"0 10px",background:"color-mix(in srgb,var(--accent) 5%,var(--panelSolid))",color:"var(--accent)",font:"inherit",fontSize:9.5,fontWeight:900,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",whiteSpace:"nowrap",flexShrink:0}
...
export default function ReservationPaymentChargeSelector({lines=[],selectedIds,onToggle,onToggleAll,selectedTotal=0,currency="ARS"}){
  ...
  return <section aria-label="Seleccionar cargos a cobrar">
    <header style={head}><div style={headText}><b style={{fontSize:11}}>¿Qué querés cobrar?</b></div>{unpaid.length?<button type="button" style={headButton} onClick={onToggleAll}>{allSelected?"Quitar selección":"Seleccionar todo"}</button>:null}</header>
