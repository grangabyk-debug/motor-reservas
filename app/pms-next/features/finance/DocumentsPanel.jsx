...
const STATUS_LABELS={draft:"Borrador",issued:"Emitido",partial:"Pago parcial",paid:"Pagado",void:"Anulado"}
const TYPE_LABELS={invoice:"Factura",receipt:"Recibo",credit_note:"Nota de crédito",debit_note:"Nota de débito",proforma:"Proforma",folio:"Folio"}
...
export default function DocumentsPanel({propertyId}){
  ...
  async function createDocument(){
    ...
    try{
      ...
      if(form.lines.some(line=>!String(line.description||"").trim()))throw new Error("Completá la descripción de todos los conceptos.")
      const{data:userData,error:userError}=await supabase.auth.getUser();if(userError)throw userError
      const items=form.lines.map(line=>{const quantity=Number(line.quantity||0),unitPrice=Number(line.unit_price||0),taxRate=Number(line.tax_rate||0),subtotal=quantity*unitPrice,tax=subtotal*taxRate/100;return{description:String(line.description||"").trim(),quantity,unit_price:unitPrice,tax_rate:taxRate,subtotal,tax,total:subtotal+tax}})
  ...
  async function changeStatus(doc,status){
    ...
    try{
      ...
      if(status==="issued"&&!doc.issued_at)patch.issued_at=new Date().toISOString()
      const{error:updateError}=await supabase.from("hotel_finance_documents").update(patch).eq("id",doc.id).eq("property_id",propertyId).eq("status",current)
      if(updateError)throw updateError
      await load()
    }catch(err){setError(err?.message||"No se pudo actualizar el documento.")}
    finally{setSaving(false)}
  }

  const actionStyle={height:30,padding:"0 9px",border:"1px solid var(--line)",borderRadius:8,background:"var(--panelSolid)",color:"var(--text)",font:"inherit",fontSize:9.5,fontWeight:850,cursor:"pointer"}
  return <div className={s.financeBody}>
    <div className={s.docTop}><div className={s.docStats}><article><span>Cobrado</span><b>{money(totals.collected)}</b></article><article><span>Pendiente</span><b>{money(totals.pending)}</b></article><article><span>Vencido</span><b>{money(totals.overdue)}</b></article><article><span>Borradores</span><b>{totals.drafts}</b></article></div><button className={s.primary} onClick={()=>setOpen(true)}>+ Nueva factura</button></div>
    {error&&<div className={s.alert}>{error}</div>}
    <article className={s.glass}><header><div><small>DOCUMENTOS INTERNOS</small><h2>Facturas y documentos financieros</h2><p>Emitir no significa cobrado. Los estados de pago no se modifican manualmente.</p></div></header>{loading?<div className={s.empty}>Cargando documentos…</div>:!documents.length?<div className={s.empty}>Todavía no hay documentos financieros registrados.</div>:<div className={s.documentList}>{documents.map(doc=><article key={doc.id}><div><b>{doc.number||"Sin numerar"}</b><small>{doc.billing_to?.name||"Cliente"} · {new Intl.DateTimeFormat("es-AR").format(new Date(doc.created_at))}</small></div><span>{TYPE_LABELS[doc.document_type]||doc.document_type}</span><strong>{money(doc.total,doc.currency)}</strong><span className={s.status}>{STATUS_LABELS[doc.status]||doc.status}</span><div style={{display:"flex",gap:6,justifyContent:"flex-end"}}>{doc.status==="draft"?<><button type="button" style={actionStyle} disabled={saving} onClick={()=>changeStatus(doc,"issued")}>Emitir</button><button type="button" style={actionStyle} disabled={saving} onClick={()=>changeStatus(doc,"void")}>Descartar</button></>:doc.status==="issued"?<button type="button" style={actionStyle} disabled={saving} onClick={()=>changeStatus(doc,"void")}>Anular</button>:<small style={{color:"var(--muted)",fontSize:9.5}}>{doc.status==="partial"?"Pago registrado parcialmente":doc.status==="paid"?"Cobro completado":"Sin acciones"}</small>}</div></article>)}</div>}</article>
    {open&&<div className={s.modalBackdrop} onClick={()=>!saving&&setOpen(false)}><div className={s.modal} onClick={e=>e.stopPropagation()}><button className={s.modalClose} onClick={()=>!saving&&setOpen(false)}>×</button><small>NUEVA FACTURA INTERNA</small><h2>Preparar documento</h2><div className={s.formGrid}><label className={s.full}><span>Vincular reserva</span><select value={form.reservation_id} onChange={e=>selectReservation(e.target.value)}><option value="">Sin reserva vinculada</option>{reservations.map(r=><option key={r.id} value={r.id}>{r.numero_reserva||`#${r.id}`} · {r.nombre_huesped}</option>)}</select></label><label><span>Cliente</span><input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></label><label><span>Email</span><input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))}/></label><label><span>Teléfono</span><input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))}/></label><label><span>Vencimiento</span><input type="date" value={form.due_at} onChange={e=>setForm(f=>({...f,due_at:e.target.value}))}/></label><label><span>Moneda</span><select value={form.currency} onChange={e=>setForm(f=>({...f,currency:e.target.value}))}><option>ARS</option><option>USD</option></select></label><label><span>Estado inicial</span><select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))}><option value="draft">Borrador</option><option value="issued">Emitido</option></select></label></div><div className={s.lines}><header><h3>Conceptos</h3><button type="button" onClick={()=>setForm(f=>({...f,lines:[...f.lines,blankLine()]}))}>+ Agregar línea</button></header>{form.lines.map((line,index)=><div className={s.line} key={index}><input placeholder="Descripción" value={line.description} onChange={e=>updateLine(index,"description",e.target.value)}/><input type="number" min="0" step="1" value={line.quantity} onChange={e=>updateLine(index,"quantity",e.target.value)}/><input type="number" min="0" step="0.01" value={line.unit_price} onChange={e=>updateLine(index,"unit_price",e.target.value)}/><label><span>IVA %</span><input type="number" min="0" max="100" step="0.01" value={line.tax_rate} onChange={e=>updateLine(index,"tax_rate",e.target.value)}/></label><button type="button" onClick={()=>setForm(f=>({...f,lines:f.lines.filter((_,i)=>i!==index)}))} disabled={form.lines.length===1}>×</button></div>)}</div><label className={s.notes}><span>Nota interna</span><textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/></label><div className={s.totals}><span>Subtotal <b>{money(calc.subtotal,form.currency)}</b></span><span>Impuestos <b>{money(calc.tax,form.currency)}</b></span><strong>Total {money(calc.total,form.currency)}</strong></div><button className={s.primary} onClick={createDocument} disabled={saving}>{saving?"Guardando…":"Crear documento"}</button></div></div>}
  </div>
}