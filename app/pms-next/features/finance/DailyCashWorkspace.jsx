...
export default function DailyCashWorkspace({propertyId,property,onNavigate,focusReservationId,onFocusHandled}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const profileIds=[...new Set([...pay.map(row=>row.created_by),...mov.map(row=>row.created_by),...docs.map(row=>row.created_by),active?.opened_by].filter(Boolean))]
