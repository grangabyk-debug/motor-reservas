...
export default function DailyCashWorkspaceMultiCurrency({propertyId,property,onNavigate,focusReservationId,onFocusHandled}){
  ...
  const totals=useMemo(()=>{
    ...
    for(const row of scopedManual){const code=currency(row.currency),amount=Number(row.amount||0),out=["expense","refund"].includes(normalize(row.movement_type)),group=methodGroup(row.method);if(out)expense[code]+=amount;else income[code]+=amount;byMethod[group][code]+=out?-amount:amount}
  ...
  const sessionMeta=useMemo(()=>parseCashSessionMeta(session?.notes),[session?.notes]),turnScope=Boolean(session&&isToday)
