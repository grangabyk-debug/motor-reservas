...
const fmtTime=value=>{
  if(!value)return""
  try{return new Intl.DateTimeFormat("es-AR",{hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date(value))}
  catch{return""}
}
        ...
        <div className={s.moreWrap} ref={menuRef}>
          ...
          {moreOpen?<div className={s.menu} role="menu">
            ...
            <button type="button" role="menuitem" onClick={()=>runMore("Cierres")}>
              ...
              <b>Cierres anteriores</b>
            </button>
          </div>:null}
        </div>
      </div>

      <div className={`${s.sessionBlock} ${session?s.sessionOpen:s.sessionClosed}`} aria-label={session?`Caja abierta a las ${openedAt||"hora no disponible"}`:"Caja cerrada"}>
        <div className={s.status}>
          <span className={s.statusDot} aria-hidden="true"/>
          <span className={s.statusCopy}>
            <b>{session?"Caja abierta":"Caja cerrada"}</b>
            <small>{session?(openedAt||"—"):"Sin turno"}</small>
          </span>
        </div>
