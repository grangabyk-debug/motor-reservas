...
const money=(value,currency="ARS")=>{try{return new Intl.NumberFormat("es-AR",{style:"currency",currency:String(currency||"ARS").toUpperCase(),maximumFractionDigits:2}).format(Number(value||0))}catch{return`${currency||"ARS"} ${Number(value||0).toFixed(2)}`}}
const FREQ={weekly:"Semanal",monthly:"Mensual",yearly:"Anual"}
const blank=(currency="ARS")=>({name:"",supplier_name:"",supplier_tax_id:"",concept:"",category:"Operación",amount:"",tax_amount:"0",currency,payment_method:"Transferencia",frequency:"monthly",interval_count:1,next_due_date:today(),end_date:"",auto_post:true,active:true,notes:""})
...
function groupedMoney(map){const entries=[...map.entries()].filter(([,value])=>Math.abs(Number(value)||0)>.009);return entries.length?entries.map(([currency,value])=>money(value,currency)).join(" · "):money(0,"ARS")}

export default function RecurringExpensesPanel({propertyId}){
  const[rows,setRows]=useState([]),[baseCurrency,setBaseCurrency]=useState("ARS"),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[error,setError]=useState(""),[schemaReady,setSchemaReady]=useState(true),[form,setForm]=useState(()=>blank("ARS"))
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[rowRes,settingsRes]=await Promise.all([
        supabase.from("hotel_recurring_expenses").select("id,name,supplier_name,supplier_tax_id,concept,category,amount,tax_amount,currency,payment_method,frequency,interval_count,next_due_date,end_date,auto_post,active,notes,created_at,updated_at").eq("property_id",propertyId).order("active",{ascending:false}).order("next_due_date").order("name"),
        supabase.from("property_settings").select("settings").eq("property_id",propertyId).maybeSingle(),
      ])
      if(rowRes.error)throw rowRes.error;if(settingsRes.error)throw settingsRes.error
      const configured=String(settingsRes.data?.settings?.preferences?.currency||"ARS").toUpperCase();setRows(rowRes.data||[]);setBaseCurrency(configured);setSchemaReady(true)
    }catch(err){if(isMissingTable(err)){setRows([]);setSchemaReady(false)}else setError(err?.message||"No se pudieron cargar los gastos recurrentes.")}finally{setLoading(false)}
  },[propertyId])
  useEffect(()=>{load()},[load])

  const summary=useMemo(()=>{const active=rows.filter(r=>r.active),monthly=new Map();for(const row of active.filter(r=>r.frequency==="monthly")){const code=String(row.currency||baseCurrency).toUpperCase();monthly.set(code,(monthly.get(code)||0)+Number(row.amount||0))}return{active:active.length,monthly,next:active.filter(r=>r.next_due_date).sort((a,b)=>a.next_due_date.localeCompare(b.next_due_date))[0]||null}},[rows,baseCurrency])
  function startCreate(){setEditing(null);setForm(blank(baseCurrency));setOpen(true);setError("")}
  ...
  return <div className={s.wrap}>
    ...
    <div className={s.top}><div className={s.metrics}><article><span>Obligaciones activas</span><b>{summary.active}</b></article><article><span>Mensuales configurados</span><b style={{fontSize:summary.monthly.size>1?18:24}}>{groupedMoney(summary.monthly)}</b><small>Sin mezclar monedas</small></article><article><span>Próximo vencimiento</span><b>{summary.next?.next_due_date||"—"}</b><small>{summary.next?.name||"Sin vencimientos"}</small></article></div><div className={s.actions}><button type="button" onClick={processDue} disabled={saving||!rows.length}>Procesar vencimientos</button><button type="button" className={s.primary} onClick={startCreate}>＋ Nueva obligación</button></div></div>
