...
export default function ReportsWorkspace({propertyId,property}){
  ...
  useEffect(()=>{load()},[load])
  const liveGeneration=usePmsAutoRefresh(propertyId,load,["reservas","pagos","habitaciones","hotel_no_show_history"])
  ...
  const report=useMemo(()=>{
    ...
    const channels={}
    const rows=valid.map(r=>{const nights=nightsOverlap(r,from,to),revenue=nights*(Number(r.tarifa_noche)||0);soldNights+=nights;roomRevenue+=revenue;if(r.fecha_entrada>=from&&r.fecha_entrada<=to)arrivals++;if(r.fecha_salida>=from&&r.fecha_salida<=to)departures++;guests+=Number(r.cantidad_huespedes)||0;const channel=r.canal_reserva||"Directo";channels[channel]=(channels[channel]||0)+1;return{...r,nights,revenue}})
    const available=Math.max(0,rooms.length*days),occupancy=available?soldNights/available*100:0,adr=soldNights?roomRevenue/soldNights:0,revpar=available?roomRevenue/available:0
  ...
  function exportCsv(){const lines=[["Reserva","Huésped","Entrada","Salida","Noches en período","Canal","Estado","Ingreso alojamiento"],...report.rows.map(r=>[r.numero_reserva||r.id,r.nombre_huesped,r.fecha_entrada,r.fecha_salida,r.nights,r.canal_reserva||"Directo",r.estado,r.revenue])],csv=lines.map(row=>row.map(value=>`"${String(value??"").replaceAll('"','""')}"`).join(",")).join("\n"),blob=new Blob(["\ufeff"+csv],{type:"text/csv;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`habitacion-llena-${from}-${to}.csv`;a.click();URL.revokeObjectURL(url)}
  ...
  return <section className={s.page}>
    ...
    <OperationalReportsPanel key={`${propertyId}-${from}-${to}-${liveGeneration}`} propertyId={propertyId} from={from} to={to} currency={currency}/>
