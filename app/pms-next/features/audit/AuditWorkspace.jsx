...
export default function AuditWorkspace({propertyId,property}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[planning,reservation,cash,automation,roomBlocks]=await Promise.all([
      ...
      for(const result of[planning,reservation,cash,automation,roomBlocks])if(result.error)throw result.error
  ...
  return <section className={s.page}>
    ...
    <div className={s.toolbar}><label className={s.search}>⌕<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar reserva, bloqueo, acción o usuario"/></label><div className={s.filters}>{[["all","Todo"],["planning","Planning"],["block","Bloqueos"],["reservation","Reservas"],["cash","Caja"],["automation","Automatizaciones"]].map(([id,label])=><button key={id} className={filter===id?s.active:""} onClick={()=>setFilter(id)}>{label}</button>)}</div></div>
...
function kindLabel(value){return({planning:"Planning",block:"Bloqueo",reservation:"Reserva",cash:"Caja",automation:"Automatización"})[value]||"Sistema"}
