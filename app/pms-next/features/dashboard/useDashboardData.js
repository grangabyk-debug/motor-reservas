...
export default function useDashboardData(propertyId){
  const[rooms,setRooms]=useState([]),[reservations,setReservations]=useState([]),[maintenance,setMaintenance]=useState([]),[housekeeping,setHousekeeping]=useState([]),[paymentsToday,setPaymentsToday]=useState([]),[reservationPayments,setReservationPayments]=useState([])
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[roomRes,resRes,maintRes,hkRes,payTodayRes]=await Promise.all([
        ...
        supabase.from("reservas").select("id,numero_reserva,nombre_huesped,habitacion_id,habitaciones_ids,fecha_entrada,fecha_salida,estado,no_show,precio_total,moneda,cantidad_huespedes,canal_reserva,hora_llegada_estimada,hora_salida_estimada,notas").eq("property_id",propertyId).lte("fecha_entrada",dayAfter).gte("fecha_salida",yesterday).neq("estado","cancelada"),
      ...
      const reservationRows=resRes.data||[],ids=reservationRows.map(row=>row.id)
      let allPayments=[]
      ...
      setRooms(roomRes.data||[]);setReservations(reservationRows);setMaintenance(maintRes.data||[]);setHousekeeping(hkRes.data||[]);setPaymentsToday(payTodayRes.data||[]);setReservationPayments(allPayments)
  ...
  const operationsByOffset=useMemo(()=>{
    const result={}
    for(const offset of[-1,0,1]){const day=dateKey(offset),valid=reservations.filter(r=>!r.no_show),arrivals=valid.filter(r=>r.fecha_entrada===day).map(enrich),departures=valid.filter(r=>r.fecha_salida===day).map(enrich),inhouse=valid.filter(r=>r.fecha_entrada<=day&&r.fecha_salida>day&&r.estado!=="finalizada").map(enrich);result[offset]={day,arrivals,inhouse,departures}}
    return result
  },[reservations,enrich])

  const metrics=useMemo(()=>{
    const today=dateKey(0),valid=reservations.filter(r=>!r.no_show),arrivals=valid.filter(r=>r.fecha_entrada===today).length,departures=valid.filter(r=>r.fecha_salida===today).length
    const occupiedIds=new Set();for(const reservation of valid.filter(r=>r.fecha_entrada<=today&&r.fecha_salida>today&&r.estado!=="finalizada"))roomIds(reservation).forEach(id=>occupiedIds.add(id))
    const inhouse=occupiedIds.size,occupancy=rooms.length?Math.min(100,inhouse/rooms.length*100):0
    let checkDone=0,checkTotal=0;for(const task of housekeeping){const list=Array.isArray(task.checklist)?task.checklist:[];if(list.length){checkTotal+=list.length;checkDone+=list.filter(item=>item?.done===true).length}else{checkTotal+=1;if(task.status==="done")checkDone+=1}}
    const collected=paymentsToday.filter(validPayment).reduce((sum,p)=>sum+Math.max(0,Number(p.monto||0)-Number(p.refunded_amount||0)),0)
