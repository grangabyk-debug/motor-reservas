...
const toneRank=tone=>tone==="red"?0:tone==="yellow"?1:tone==="blue"?2:3
...
export default function DashboardOperationsPulse({propertyId,data,onNavigate,allowedViews=[]}){
  ...
  const[freshIds,setFreshIds]=useState(()=>new Set()),[updatedNow,setUpdatedNow]=useState(false),[contextInsight,setContextInsight]=useState(null)
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[hubRes,stateRes]=await Promise.all([
      ...
      setHub(hubRes.data||null);setChannelStates(stateRes.data||[])
    }catch(err){setError(err?.message||"No pudimos comprobar las integraciones.")}
  ...
  const pmsState=data?.error?{tone:"red",label:"Revisar"}:data?.loading?{tone:"yellow",label:"Actualizando"}:{tone:"green",label:updatedNow?"Actualizado ahora":"Sincronizado"}
  ...
  const openHealthCenter=()=>{if(typeof window!=="undefined")window.dispatchEvent(new CustomEvent("hl:open-health-center"))}
  ...
  useEffect(()=>{
    ...
    setUpdatedNow(true);timers.push(window.setTimeout(()=>setUpdatedNow(false),1900))
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      const message=critical?`Veo ${critical} prioridad${critical===1?"":"es"} crítica${critical===1?"":"s"}. Yo empezaría por ${first.title.toLowerCase()}.`:`Hay ${warning} pendiente${warning===1?"":"s"}. Conviene revisar primero ${first.title.toLowerCase()}.`
      setContextInsight({signature:signalSignature,message,item:first})
    },800)
    return()=>window.clearTimeout(timer)
  },[signalSignature,critical,warning,signals])
  ...
  const firstAction=signals.find(item=>can(item.target))
  return <>
    <article className={u.operationCard} data-tone={tone} data-live-state={loading?"loading":"ready"}>
      <header className={u.operationHead}>
        <div><Status tone={tone} label="OPERACIÓN AHORA"/><strong>{headline}</strong></div>
        {firstAction?<button type="button" onClick={()=>navigate(firstAction)}>Resolver primero</button>:<button type="button" onClick={openHealthCenter}>Ver estado</button>}
      </header>
      <div className={u.operationSignals}>
        {loading?<div className={u.operationLoading}><i/><span>Revisando señales…</span></div>:signals.length?signals.map(item=><button type="button" key={item.id} data-tone={item.tone} data-fresh={freshIds.has(item.id)?"true":undefined} style={{position:"relative",boxShadow:"none"}} onClick={()=>navigate(item)} disabled={!can(item.target)}>{item.tone==="red"?<i aria-hidden="true" style={{position:"absolute",left:-12,top:6,bottom:6,width:3,borderRadius:"0 3px 3px 0",background:"#ef5b64",boxShadow:"0 0 12px rgba(239,91,100,.18)",pointerEvents:"none"}}/>:null}<span><PmsIcon name={item.icon} size={15}/></span><div><b>{item.title}</b><small>{item.detail}</small></div><em><PmsIcon name="chevronRight" size={13}/></em></button>):<div className={u.operationClear}><span><PmsIcon name="check" size={14}/></span><div><b>Sin excepciones críticas</b><small>No vemos nada urgente en la operación actual.</small></div></div>}
      </div>
      <footer className={u.operationFoot}>
        <span><i data-tone={hubState.tone}/><b>Channel Manager</b><small>{hubState.label}</small></span>
        <span><i data-tone={pmsState.tone}/><b>Datos PMS</b><small>{pmsState.label}</small></span>
        <button type="button" onClick={openHealthCenter} style={{gridColumn:"1 / -1",minHeight:34,padding:"0 11px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,border:0,borderTop:"1px solid color-mix(in srgb,var(--line) 72%,transparent)",background:"color-mix(in srgb,var(--panelSolid) 35%,transparent)",color:"var(--muted)",font:"800 10.5px inherit",cursor:"pointer"}}><span>Estado del sistema</span><PmsIcon name="chevronRight" size={13}/></button>
      </footer>
    </article>
    {contextInsight?<aside data-olivia-context-insight="true" role="status"><span data-olivia-context-avatar aria-hidden="true"/><div><b>OlivIA detectó algo</b><small>{contextInsight.message}</small>{can(contextInsight.item.target)?<button type="button" onClick={()=>{navigate(contextInsight.item);setContextInsight(null)}}>Ver ahora</button>:null}</div><button type="button" aria-label="Cerrar sugerencia de OlivIA" onClick={()=>setContextInsight(null)}><PmsIcon name="close" size={13}/></button></aside>:null}
