...
export default function DashboardOperationsPulse({propertyId,data,onNavigate,allowedViews=[]}){
  ...
  return <>
    <article className={u.operationCard} data-tone={tone} data-live-state={loading?"loading":"ready"}>
      ...
      <div className={u.operationSignals}>{loading?<div className={u.operationLoading}><i/><span>Revisando señales…</span></div>:signals.length?<>{signals.map(item=><button type="button" key={item.id} data-tone={item.tone} data-fresh={freshIds.has(item.id)?"true":undefined} style={{position:"relative",boxShadow:"none"}} onClick={()=>navigate(item)} disabled={!can(item.target)}>{item.tone==="red"?<i aria-hidden="true" style={{position:"absolute",left:-12,top:6,bottom:6,width:3,borderRadius:"0 3px 3px 0",background:"#ef5b64",boxShadow:"0 0 12px rgba(239,91,100,.18)",pointerEvents:"none"}}/>:null}<span><PmsIcon name={item.icon} size={15}/></span><div><b>{item.title}</b><small>{item.detail}</small></div><em>›</em></button>)}{resolvedSignal?<div data-operation-resolved="true"><span>✓</span><div><b>Resuelto</b><small>{resolvedSignal.title}</small></div></div>:null}</>:<div className={u.operationClear}><span>✓</span><div><b>Sin excepciones críticas</b><small>No vemos nada urgente en la operación actual.</small></div></div>}</div>
      <footer className={u.operationFoot}><span><i data-tone={hubState.tone}/><b>Channel Manager</b><small>{hubState.label}</small></span><span><i data-tone={data?.loading?"yellow":"green"}/><b>Datos PMS</b><small>{data?.loading?"Actualizando":updatedNow?"Actualizado ahora":"En vivo"}</small></span><button type="button" onClick={openHealthCenter} style={{gridColumn:"1 / -1",minHeight:34,padding:"0 11px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,border:0,borderTop:"1px solid color-mix(in srgb,var(--line) 72%,transparent)",background:"color-mix(in srgb,var(--panelSolid) 35%,transparent)",color:"var(--muted)",font:"800 10.5px inherit",cursor:"pointer"}}><span>Health Center · Ver diagnóstico</span><em style={{fontStyle:"normal",fontSize:15}}>›</em></button></footer>
    </article>
