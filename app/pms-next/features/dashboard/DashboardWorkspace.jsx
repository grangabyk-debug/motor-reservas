...
function GuestRow({item,kind,onOpen}){
  ...
  return <button type="button" className={s.guestRow} onClick={onOpen}>
    <span className={s.guestAvatar}>{initials(item.nombre_huesped)}</span><span className={s.guestMain}><b>{item.nombre_huesped}</b><small>{roomLabel} · {item.canal_reserva||"Directa"}{time?` · ${time}`:""}</small><span className={s.guestFlags}>{item.roomMaintenance?<em data-kind="danger">Mantenimiento</em>:item.roomDirty&&kind==="arrival"?<em data-kind="warn">Habitación sucia</em>:null}{item.balance>0?<em data-kind="money">Saldo {money(item.balance,item.moneda)}</em>:<em data-kind="ok">Pago cubierto</em>}</span></span><span className={s.guestPax}>{item.cantidad_huespedes||1} pax<br/><small>›</small></span>
  </button>
}
...
export default function DashboardWorkspace({propertyId,property,onNavigate,allowedViews=[]}){
  ...
  return <section className={s.page}>
    ...
    {can("reservations")&&<section className={s.frontDesk}><header className={s.frontDeskHead}><div><small>RECEPCIÓN</small><h2>Movimiento del hotel</h2><p>Entradas, huéspedes alojados y salidas con alertas operativas.</p></div><div className={s.frontDeskTools}><div className={s.dayTabs}>{[[-1,"Ayer"],[0,"Hoy"],[1,"Mañana"]].map(([value,label])=><button type="button" key={value} className={opsDay===value?s.dayActive:""} onClick={()=>setOpsDay(value)}>{label}</button>)}</div><label className={s.deskSearch}>⌕<input value={opsQuery} onChange={event=>setOpsQuery(event.target.value)} placeholder="Huésped, habitación o reserva"/></label></div></header><div className={s.frontDeskGrid}>{columns.map(column=>{const rows=filterRows(ops[column.key]||[]);return <article className={s.deskColumn} key={column.key}><header><b>{column.title}</b><span>{rows.length}</span></header><div className={s.guestList}>{rows.length?rows.map(item=><GuestRow key={`${column.key}-${item.id}`} item={item} kind={column.kind} onOpen={()=>openReservation(item)}/>):<div className={s.emptyOps}>Sin movimientos para esta vista.</div>}</div></article>})}</div></section>}
