...
const weekdayDate=value=>new Intl.DateTimeFormat("es-AR",{weekday:"long",day:"numeric",month:"long"}).format(fromKey(value)).replace(".","")
...
export function CreateReservationDrawer({draft,setDraft,drawerStep,setDrawerStep,draftState,externalError="",availableRooms,roomById,cancellationPolicies=[],nights,total,saving,onClose,onDiscard,onNext,onSave}){
  ...
  const[checkoutBlocks,setCheckoutBlocks]=useState([]),[checkoutBlocksLoading,setCheckoutBlocksLoading]=useState(false)
  ...
  const selectedCheckoutBlocks=checkoutBlocks.filter(block=>selectedRoomIds.includes(String(block.habitacion_id)))
  const selectedCheckoutMaintenance=selectedCheckoutBlocks.filter(isMaintenanceBlock)
  const currentMaintenanceAckKey=maintenanceAckKey(selectedCheckoutMaintenance)
  const maintenanceConfirmed=!currentMaintenanceAckKey||draft.maintenanceCheckoutAckKey===currentMaintenanceAckKey
  const maintenanceRooms=[...new Set(selectedCheckoutMaintenance.map(block=>roomById.get(Number(block.habitacion_id))?.nombre||String(block.habitacion_id)))]
  ...
  useEffect(()=>{
    ...
    async function loadBlocks(){
      ...
      if(!ids.length||!draft.start||!draft.end){if(!cancelled){setBlockedRoomIds(new Set());setCheckoutBlocks([]);setCheckoutBlocksLoading(false)}return}
      setCheckoutBlocksLoading(true)
      const[overlapRes,boundaryRes]=await Promise.all([
        supabase.from("bloqueos").select("id,habitacion_id,fecha_desde,fecha_hasta,motivo,detalle").in("habitacion_id",ids).lt("fecha_desde",draft.end).gt("fecha_hasta",draft.start),
        supabase.from("bloqueos").select("id,habitacion_id,fecha_desde,fecha_hasta,motivo,detalle").in("habitacion_id",ids).eq("fecha_desde",draft.end),
      ])
      if(cancelled)return
      if(overlapRes.error||boundaryRes.error){setBlockedRoomIds(new Set());setCheckoutBlocks([]);setCheckoutBlocksLoading(false);return}
      const hardBoundary=(boundaryRes.data||[]).filter(block=>!isMaintenanceBlock(block))
      setBlockedRoomIds(new Set([...(overlapRes.data||[]),...hardBoundary].map(row=>String(row.habitacion_id))))
      setCheckoutBlocks(boundaryRes.data||[])
      setCheckoutBlocksLoading(false)
    }
    loadBlocks()
  ...
  return <div className={s.drawerShade} onMouseDown={event=>event.target===event.currentTarget&&onClose()}><aside className={s.createDrawer} role="dialog" aria-modal="true" aria-label="Crear reserva"><header className={s.drawerHeader}><div><small>CREAR RESERVA</small><h2>{title}</h2><p>{draftState}</p></div><button className={s.close} onClick={onClose}>×</button></header><nav className={s.stepNav}>{STEPS.map((step,index)=><button key={step} className={index===drawerStep?s.stepActive:index<drawerStep?s.stepDone:""} onClick={()=>index<=drawerStep&&setDrawerStep(index)}><span>{index+1}</span>{step}</button>)}</nav><div className={s.drawerBody}>{visibleError?<div style={errorStyle} role="alert">{visibleError}</div>:null}
    {drawerStep===0?<div className={s.stepPanel}><div className={s.dateSummary}><span><small>Entrada</small><b>{longDate(draft.start)}</b></span><span className={s.nightsBadge}>{nights} noche{nights===1?"":"s"}</span><span><small>Salida</small><b>{longDate(draft.end)}</b></span></div><div className={s.formGrid}><label>Entrada<input type="date" value={draft.start} onChange={event=>setDraft(current=>({...current,start:event.target.value,end:current.end<=event.target.value?addDays(event.target.value,1):current.end,roomSelectionManual:false,maintenanceCheckoutAckKey:""}))}/></label><label>Salida<input type="date" min={addDays(draft.start,1)} value={draft.end} onChange={event=>setDraft(current=>({...current,end:event.target.value,roomSelectionManual:false,maintenanceCheckoutAckKey:""}))}/></label><label>Huéspedes<input type="number" min="1" value={draft.guests} onChange={event=>setDraft(current=>({...current,guests:Number(event.target.value)||1,roomSelectionManual:false}))}/></label><label>Moneda<select value={draft.currency} onChange={event=>setDraft(current=>({...current,currency:event.target.value}))}><option value="ARS">ARS</option><option value="USD">USD</option></select></label></div></div>:null}
