...
import{commercialPriceFromNet,finalPriceFromNet,priceTaxShortLabel}from"../../core/priceTax"
...
function compareRooms(a,b,guests){
  ...
  const priceA=Math.max(0,Number(a.pricing_final??a.precio)||0),priceB=Math.max(0,Number(b.pricing_final??b.precio)||0)
...
export function CreateReservationDrawer({draft,setDraft,drawerStep,setDrawerStep,draftState,externalError="",availableRooms,roomById,cancellationPolicies=[],nights,total,saving,onClose,onDiscard,onNext,onSave}){
  ...
  const netTotal=Math.max(0,grossTotal-discountAmount),taxEnabled=taxConfig.enabled!==false,vatRate=taxEnabled?Math.max(0,Number(taxConfig.vat_rate)||0):0,vatAmount=taxEnabled?Math.round(netTotal*vatRate)/100:0,totalWithTax=Math.round((netTotal+vatAmount)*100)/100,visibleError=localError||externalError,finalRateCaption=priceTaxShortLabel(taxConfig)
