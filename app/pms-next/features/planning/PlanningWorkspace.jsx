...
function ReservationBlock({item,days,selected,onSelect,onDragStart,onResizeStart}){
  ...
  return <div draggable role="button" tabIndex="0" aria-label={`${item.nombre_huesped}, ${diffDays(item.fecha_entrada,item.fecha_salida)} noches`} className={`${s.stay} ${s[kind]} ${selected?s.selected:""}`} style={{gridColumn:`${start+1} / span ${span}`}} onClick={()=>onSelect(item)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onSelect(item)}}} onDragStart={event=>onDragStart(event,item)}>
    <span className={s.avatar}>{initials(item.nombre_huesped)}</span><span className={s.stayText}>{item.nombre_huesped}</span><span className={s.stayMeta}>{diffDays(item.fecha_entrada,item.fecha_salida)}n</span>
    <span className={s.resizeHandle} draggable title="Arrastrá para cambiar la salida" aria-label="Cambiar fecha de salida" onMouseDown={event=>event.stopPropagation()} onClick={event=>event.stopPropagation()} onDragStart={event=>{event.stopPropagation();onResizeStart(event,item)}}>↔</span>
  </div>
}
...
export default function PlanningWorkspace({propertyId,property,onNavigate}){
  ...
  const grid={gridTemplateColumns:`repeat(${days.length},minmax(56px,1fr))`}
  ...
  function makeDraft(roomId=data.rooms[0]?.id,start=today){
    const room=data.rooms.find(item=>String(item.id)===String(roomId))
    return{guest:"",email:"",phone:"",roomId:String(roomId),start,end:addDays(start,1),status:"confirmada",guests:1,rate:Number(room?.precio)||0,currency:"ARS",channel:"Directa",notes:""}
  }
  function jumpToday(){setYear(now.getFullYear());setMonth(now.getMonth())}
  function shiftYear(amount){setYear(value=>value+amount)}
  function openFreshForm(roomId=data.rooms[0]?.id,start=today){
    if(!roomId)return data.setError("Primero configurá al menos una habitación activa.")
    setDraft(makeDraft(roomId,start));setDraftState("Los cambios se guardan automáticamente en este dispositivo");data.setError("");setFormOpen(true)
  }
  function openNewReservation(){
    ...
    if(draftKey){
      try{
        ...
        if(raw){const saved=JSON.parse(raw);if(saved?.roomId&&saved?.start&&saved?.end){setDraft(saved);setDraftState("Borrador recuperado");data.setError("");setFormOpen(true);return}}
      }catch{}
    }
    openFreshForm()
  }
  function clearDraft(){
    if(draftKey){try{window.localStorage.removeItem(draftKey)}catch{}}
    setHasSavedDraft(false);setDraft(null);setDraftState("")
  }
  function cancelReservation(){clearDraft();setFormOpen(false);data.setError("")}
  async function saveReservation(){
    ...
    setSaving(true)
    try{await data.createReservation(draft);clearDraft();setFormOpen(false)}catch(err){data.setError(err?.message||"No se pudo crear la reserva.")}finally{setSaving(false)}
  }
  function beginDrag(event,item){event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/plain",String(item.id));setDragging({item,mode:"move"});setSelected(null)}
  function beginResize(event,item){event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/plain",String(item.id));setDragging({item,mode:"resize"});setSelected(null)}
  async function dropReservation(event,roomId,day){
    ...
    }else{const stay=Math.max(1,diffDays(source.fecha_entrada,source.fecha_salida));end=addDays(day,stay)}
  ...
  return <section className={s.page}>
    <div className={s.toolbar}>
      ...
      <div className={s.toolbarRight}><label className={s.search}>⌕<input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Huésped, reserva, canal o habitación"/></label><button type="button" className={`${s.iconButton} ${attentionOnly?s.iconButtonActive:""}`} onClick={()=>setAttentionOnly(value=>!value)} title="Pendientes y tentativas">⌁</button><button type="button" className={s.iconButton} onClick={()=>onNavigate?.("rates")} title="Tarifas y disponibilidad">$</button><button type="button" className={s.newButton} onClick={openNewReservation}>＋ Nueva reserva{hasSavedDraft?<span className={s.draftDot} title="Hay un borrador guardado"/>:null}</button></div>
    </div>
