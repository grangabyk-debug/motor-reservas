...
export default function PlanningWorkspace({propertyId,property,onNavigate}){
  ...
  useEffect(()=>{if(!selecting)return;const cancel=()=>{setSelecting(false);setRangeSelection(null)};window.addEventListener("mouseup",cancel);return()=>window.removeEventListener("mouseup",cancel)},[selecting])
  ...
  function openFreshForm(roomId=data.rooms[0]?.id,start=today,end=addDays(start,1)){if(!roomId)return data.setError("Primero configurá una habitación activa.");setSelected(null);setDraft(makeDraft(roomId,start,end));setDrawerStep(0);setDraftState("Los cambios se guardan automáticamente en este dispositivo");data.setError("");setFormOpen(true)}
  ...
  function finishRange(event,roomId){if(!selecting||!rangeSelection||String(rangeSelection.roomId)!==String(roomId))return;event.stopPropagation();const range=rangeSelection;setSelecting(false);setRangeSelection(null);openFreshForm(roomId,range.start,range.end)}

  const availableRooms=useMemo(()=>draft?data.rooms.map(room=>({...room,available:!data.reservations.some(item=>roomHas(item,room.id)&&overlaps(item,draft.start,draft.end))})):data.rooms,[data.rooms,data.reservations,draft])
