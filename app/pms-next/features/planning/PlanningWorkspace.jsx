...
export default function PlanningWorkspace({propertyId,property,onNavigate,newReservationRequest=0}){
  ...
  function makeDraft(roomId=defaultRoomId,start=today,end=addDays(start,1),roomIds=[roomId]){
  ...
  function openFreshForm(roomId=defaultRoomId,start=today,end=addDays(start,1),roomIds=[roomId]){
  ...
  async function saveReservation(){
    ...
    try{
      ...
      clearDraft();setFormOpen(false);setDrawerStep(0);openReservationRecord(created.id)
  ...
  return <section className={s.page} style={{"--day-width":`${dayWidth}px`,"--room-width":settings.expanded?"190px":"160px"}}>
    ...
    {selected&&!formOpen?<ReservationDetailDrawer selected={selected} room={roomById.get(Number(selected.habitacion_id))} rooms={selectedRooms} onClose={()=>setSelected(null)} onOpen={()=>openReservationRecord(selected.id)}/>:null}
