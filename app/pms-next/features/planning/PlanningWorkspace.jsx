...
import{finalizeReservationConversions}from"./reservationConversionLinks"
...
export default function PlanningWorkspace({propertyId,property,onNavigate,newReservationRequest=0}){
  ...
  function makeDraft(roomId=defaultRoomId,start=today,end=addDays(start,1),roomIds=[roomId]){const ids=uniqueIds(roomIds.length?roomIds:[roomId]);return{firstName:"",lastName:"",email:"",phone:"",country:"",guestProfileId:"",guestProfileSnapshot:null,roomId:ids[0]||"",roomIds:ids,start,end,status:"confirmada",guests:1,rate:roomRate(ids),currency:"ARS",channel:"Walk-in",voucher:"",discountType:"none",discountValue:0,discountReason:"",discountReasonDetail:"",notes:"",cancellationPolicyId:defaultPolicyId}}
  function openFreshForm(roomId=defaultRoomId,start=today,end=addDays(start,1),roomIds=[roomId]){const roomSelectionManual=arguments.length>=2;if(!roomId)return data.setError("Primero configurá una habitación activa.");setPreview(null);setSelected(null);setRateMove(null);setRangeSelection(null);setSelecting(false);setDraft({...makeDraft(roomId,start,end,roomIds),roomSelectionManual});setDrawerStep(0);setDraftState(roomSelectionManual?"Selección del Planning conservada · los cambios se guardan automáticamente":"Los cambios se guardan automáticamente en este dispositivo");setFormError("");data.setError("");setFormOpen(true)}
  function consumeQuoteSeed(){if(!quoteSeedKey||!data.rooms.length||typeof window==="undefined")return;let seed=null;try{const raw=window.localStorage.getItem(quoteSeedKey);if(!raw)return;seed=JSON.parse(raw);window.localStorage.removeItem(quoteSeedKey)}catch{return}const ids=uniqueIds(seed?.roomIds||[]).filter(id=>roomById.has(Number(id)));if(!ids.length)return data.setError("Las habitaciones del presupuesto ya no están disponibles para preparar la reserva.");const start=seed.start||today,end=seed.end&&seed.end>start?seed.end:addDays(start,1),base=makeDraft(ids[0],start,end,ids);setAnchor(start);setPreview(null);setSelected(null);setRateMove(null);setRangeSelection(null);setSelecting(false);setDraft({...base,...seed,roomId:ids[0],roomIds:ids,start,end,roomSelectionManual:true,cancellationPolicyId:seed?.cancellationPolicyId||base.cancellationPolicyId});setDrawerStep(0);setDraftState(`Datos precargados desde ${seed.quoteNumber||"presupuesto"}`);setFormError("");data.setError("");setFormOpen(true)}
  function openNewReservation(){if(!defaultRoomId)return data.setError("Primero configurá una habitación activa.");if(draftKey)try{localStorage.removeItem(draftKey)}catch{}setHasSavedDraft(false);openFreshForm(defaultRoomId)}
  function openReservationRecord(id){onNavigate?.("reservations",{reservationId:id,restoreScroll:false});if(typeof window!=="undefined")requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:"auto"}))}
  function openHousekeepingRoom(roomId){if(!roomId)return;if(typeof window!=="undefined"){const url=new URL(window.location.href);url.searchParams.set("housekeeping_room",String(roomId));window.history.replaceState(window.history.state||{},"",url)}onNavigate?.("housekeeping",{restoreScroll:false})}
  useEffect(()=>{if(!newReservationRequest||!data.rooms.length||lastNewReservationRequest.current===newReservationRequest)return;lastNewReservationRequest.current=newReservationRequest;openNewReservation()},[newReservationRequest,data.rooms.length,defaultRoomId])
  ...
  async function saveReservation(){if(saving)return;const guest=`${draft?.firstName||""} ${draft?.lastName||""}`.trim();if(!String(draft?.firstName||"").trim()||!String(draft?.lastName||"").trim())return showFormError("Completá nombre y apellido para crear la reserva.");if(!draft?.cancellationPolicyId)return showFormError("Seleccioná una política de cancelación para esta reserva.");const discountType=draft?.discountType||"none",discountValue=Math.max(0,Number(draft?.discountValue)||0),hasDiscount=discountType!=="none"&&discountValue>0;if(hasDiscount&&!String(draft?.discountReason||"").trim())return showFormError("Indicá el motivo del descuento. El sistema no aplica descuentos sin una causa explícita.");if(hasDiscount&&draft?.discountReason==="other"&&!String(draft?.discountReasonDetail||"").trim())return showFormError("Especificá el motivo del descuento.");setSaving(true);setFormError("");try{const selectedDraftIds=uniqueIds(draft?.roomIds?.length?draft.roomIds:[draft?.roomId]),normalizedRate=selectedDraftIds.reduce((sum,id)=>sum+(Number(draft?.roomAssignments?.[id]?.rate??pricedRoomById.get(Number(id))?.pricing_net)||0),0),created=await data.createReservation({...draft,rate:normalizedRate,guest});await finalizeReservationConversions({propertyId,draft,reservation:created});clearDraft();setFormOpen(false);setDrawerStep(0);openReservationRecord(created.id)}catch(err){showFormError(err?.message||"No se pudo crear la reserva.")}finally{setSaving(false)}}
    ...
    if(roomChanged){
      ...
      try{
        const{data:quote,error:quoteError}=await supabase.rpc("hl_quote_room_upgrade_atomic",{
          p_reserva_id:Number(source.id),
          p_from_room_id:Number(source.habitacion_id),
          p_to_room_id:Number(roomId),
          p_start:start,
          p_end:end
        })
      ...
      }catch(err){data.setError(err?.message||"No se pudo calcular la tarifa del cambio de habitación.");return}
    }else{setDragging(null);setDropCell("")}
    await commitMove({reservationId:source.id,roomId,start,end},false)
  }
  function roomsBetween(anchorRoomId,currentRoomId){const a=visibleRooms.findIndex(room=>String(room.id)===String(anchorRoomId)),b=visibleRooms.findIndex(room=>String(room.id)===String(currentRoomId));if(a<0||b<0)return[String(anchorRoomId)];return visibleRooms.slice(Math.min(a,b),Math.max(a,b)+1).map(room=>String(room.id))}
  ...
  useEffect(()=>{if(!draft?.start||!draft?.end||draft.end<=draft.start||!data.rooms.length){setDraftPricing(null);return}let cancelled=false;resolvePricingQuote({propertyId,start:draft.start,end:draft.end,roomIds:data.rooms.map(room=>room.id),reservationCurrency:draft.currency||null}).then(result=>{if(cancelled)return;setDraftPricing(result);const byId=pricingRoomsById(result),ids=uniqueIds(draft.roomIds?.length?draft.roomIds:[draft.roomId]),rate=ids.reduce((sum,id)=>sum+(Number(byId.get(Number(id))?.average_net)||0),0);setDraft(current=>current?{...current,rate,currency:result.final_currency||result.configured_currency||current.currency}:current)}).catch(err=>{if(!cancelled)showFormError(err?.message||"No se pudieron resolver las tarifas del rango.")});return()=>{cancelled=true}},[propertyId,draft?.start,draft?.end,draft?.currency,data.rooms.map(room=>room.id).join("|")])
  const draftPricingByRoom=useMemo(()=>pricingRoomsById(draftPricing),[draftPricing])
  const availableRooms=useMemo(()=>draft?data.rooms.map(room=>{const price=draftPricingByRoom.get(Number(room.id));return{...room,pricing:price,pricing_net:Number(price?.average_net),pricing_configured:Number(price?.average_configured),pricing_final:Number(price?.average_final),precio:price?.is_complete?Number(price.average_net):0,available:Boolean(price?.is_complete)&&!availabilityReservations.some(item=>roomHas(item,room.id)&&overlaps(item,draft.start,draft.end))}}):data.rooms,[data.rooms,availabilityReservations,draft,draftPricingByRoom])
  const pricedRoomById=useMemo(()=>new Map(availableRooms.map(room=>[Number(room.id),room])),[availableRooms])
  const selectedRooms=useMemo(()=>selected?uniqueIds([...(selected._reservation_room_ids||[]),selected.habitacion_id,...(selected.habitaciones_ids||[])]).map(id=>roomById.get(Number(id))).filter(Boolean):[],[selected,roomById])
