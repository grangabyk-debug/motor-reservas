...
export function ReservationBlock({item,days,selected,onSelect,onDragStart,onResizeStart,settings,onPreview}){
  ...
  const first=days[0],dayUse=isDayUse(item),noShow=Boolean(item.no_show),effectiveEnd=resizeEnd||item.fecha_salida,entryOffset=diffDays(first,item.fecha_entrada),rooms=roomCount(item),group=rooms>1,pax=Math.max(1,Number(item.cantidad_huespedes)||1),channel=channelMeta(item.canal_reserva)
  const lock=movementDetail(item),movementLocked=Boolean(lock?.movement_locked),lockReason=String(lock?.movement_lock_reason||"").trim()
  const early=Boolean(item.early_checkin)||Number(item.early_checkin_importe)>0,late=(Boolean(item.late_checkout)||Number(item.late_checkout_importe)>0)&&item._room_checkout_segment!=="checked_out"
  ...
  const lockLabel=movementLocked?` · Movimiento de habitación bloqueado${lockReason?`: ${lockReason}`:""}`:""
  const stayLabel=(noShow?`${item.nombre_huesped}, No Show, estadía original ${item.fecha_entrada} a ${item.fecha_salida}`:dayUse?`${item.nombre_huesped}, ${pax} pasajeros, ${channel.label}, ${stageLabel}, ${payment.label}, day use`:group?`${item.nombre_huesped}, ${pax} pasajeros, ${channel.label}, ${stageLabel}, ${payment.label}, ${rooms} habitaciones, ${nights} noches`:`${item.nombre_huesped}, ${pax} pasajeros, ${channel.label}, ${stageLabel}, ${payment.label}, ${nights} noches`)+lockLabel
  ...
  return <div draggable={!group&&!resizing&&!noShow&&!movementLocked} role="button" tabIndex="0" aria-label={stayLabel} title={(noShow?`No Show · estadía original ${item.fecha_entrada} → ${item.fecha_salida} · ${money(item.precio_total,item.moneda)}`:`${channel.label} · ${pax} pax · ${stageLabel} · ${payment.label}${early?" · Early check-in":""}${late?" · Late check-out":""}`)+lockLabel} className={`${c.stay} ${l.stageBar} ${l[kind]} ${selected?c.selected:""}`} style={{left:`calc(${start} * var(--day-width) + 3px)`,width:`calc(${span} * var(--day-width) - 6px)`,minWidth:noShow?9:undefined,top:noShow?1:undefined,height:noShow?12:undefined,padding:noShow?"0 2px":undefined,borderRadius:noShow?4:undefined,transition:resizing?"none":undefined,zIndex:noShow?10:undefined,outline:movementLocked?"2px solid color-mix(in srgb,#111827 72%,transparent)":undefined,outlineOffset:movementLocked?-1:undefined}} onMouseEnter={showPreview} onMouseLeave={()=>onPreview?.(null)} onFocus={showPreview} onBlur={()=>onPreview?.(null)} onClick={()=>{if(resizing)return;onPreview?.(null);onSelect(item)}} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onPreview?.(null);onSelect(item)}}} onDragStart={event=>{if(resizing||noShow||movementLocked){event.preventDefault();return}onPreview?.(null);onDragStart(event,item)}}>
    <span className={c.stayContent}>{noShow?<span style={{fontSize:7,fontWeight:950,letterSpacing:"-.02em",whiteSpace:"nowrap",lineHeight:1}}>NS</span>:<><span style={paxStyle} title={`${pax} pasajero${pax===1?"":"s"}`}>{pax===1?"👤":"👥"}{pax}</span>{movementLocked?<span title={lockReason?`Habitación fija · ${lockReason}`:"Habitación fija · movimiento bloqueado"} style={{height:20,minWidth:20,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:5,background:"rgba(17,24,39,.14)",fontSize:10}}>🔒</span>:null}<ReservationChannelLogo value={item.canal_reserva}/><span className={c.stayText}>{settings.showId&&item.numero_reserva?<small>{item.numero_reserva}</small>:null}<b>{item.nombre_huesped}</b></span>{group?<span className={g.barGroupBadge}>{rooms} hab.</span>:settings.showPrice?<span className={c.stayPrice}>{money(item.precio_total,item.moneda)}</span>:null}</>}</span>
