...
export function ReservationBlock({item,days,selected,onSelect,onDragStart,onResizeStart,settings,onPreview}){
  ...
  return <div draggable={!group&&!resizing&&!noShow} role="button" tabIndex="0" aria-label={stayLabel} title={noShow?`No Show · estadía original ${item.fecha_entrada} → ${item.fecha_salida} · ${money(item.precio_total,item.moneda)}`:`${channel.label} · ${pax} pax · ${stageLabel} · ${payment.label}${early?" · Early check-in":""}${late?" · Late check-out":""}`} className={`${c.stay} ${l.stageBar} ${l[kind]} ${selected?c.selected:""}`} style={{left:`calc(${start} * var(--day-width) + 3px)`,width:`calc(${span} * var(--day-width) - 6px)`,minWidth:noShow?9:undefined,top:noShow?1:undefined,height:noShow?12:undefined,padding:noShow?"0 2px":undefined,borderRadius:noShow?4:undefined,transition:resizing?"none":undefined,zIndex:noShow?10:undefined}} onMouseEnter={showPreview} onMouseLeave={()=>onPreview?.(null)} onFocus={showPreview} onBlur={()=>onPreview?.(null)} onClick={()=>{if(resizing)return;onPreview?.(null);onSelect(item)}} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onPreview?.(null);onSelect(item)}}} onDragStart={event=>{if(resizing||noShow){event.preventDefault();return}onPreview?.(null);onDragStart(event,item)}}>
    <span className={c.stayContent}>{noShow?<span style={{fontSize:7,fontWeight:950,letterSpacing:"-.02em",whiteSpace:"nowrap",lineHeight:1}}>NS</span>:<><span style={paxStyle} title={`${pax} pasajero${pax===1?"":"s"}`}>{pax===1?"👤":"👥"}{pax}</span><ReservationChannelLogo value={item.canal_reserva}/><span className={c.stayText}>{settings.showId&&item.numero_reserva?<small>{item.numero_reserva}</small>:null}<b>{item.nombre_huesped}</b></span>{group?<span className={g.barGroupBadge}>{rooms} hab.</span>:settings.showPrice?<span className={c.stayPrice}>{money(item.precio_total,item.moneda)}</span>:null}</>}</span>
