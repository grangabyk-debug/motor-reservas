...
const planningSegments=rows=>(rows||[]).flatMap(item=>{
  const ids=reservationRooms(item),checked=ids.map(id=>({id,date:roomCheckoutDate(item,id)})).filter(value=>value.date&&value.date<item.fecha_salida)
  if(!checked.length)return[item]
  const checkedIds=new Set(checked.map(value=>value.id)),activeIds=ids.filter(id=>!checkedIds.has(id)),result=[]
  if(activeIds.length)result.push({...item,habitacion_id:activeIds[0],habitaciones_ids:activeIds,_room_checkout_segment:"active"})
  checked.forEach(({id,date})=>{if(item.fecha_entrada<date)result.push({...item,habitacion_id:id,habitaciones_ids:[id],fecha_salida:date,_room_checkout_segment:"checked_out",_room_checkout_date:date})})
  return result
})
...
export default function usePlanningData(propertyId,windowStart,windowEndExclusive){
  ...
  const load=useCallback(async(silent=false)=>{
    ...
    try{
      ...
      const enriched=attachPayments(planningSegments(resRes.data||[]),paymentRes.error?[]:paymentRes.data||[])
  ...
  const moveReservation=useCallback(async({reservationId,roomId,start,end,reprice=false})=>{
    ...
    try{
      ...
      return data
    }catch(err){setReservations(list=>{const withoutOptimistic=list.filter(item=>Number(item.id)!==numericId);return previous.fecha_entrada<windowEndExclusive&&previous.fecha_salida>=windowStart?[...withoutOptimistic,previous]:withoutOptimistic});toast({tone:"error",title:"Cambio revertido",message:"El servidor no pudo confirmar el movimiento y el Planning volvió al estado anterior.",duration:4200});throw err}
  ...
  const createReservation=useCallback(async draft=>{
    ...
    const[conflictRes,blockRes]=await Promise.all([
      supabase.from("reservas").select("id,numero_reserva,habitacion_id,habitaciones_ids,room_checkout_dates,fecha_entrada,fecha_salida,estado,no_show").eq("property_id",propertyId).neq("estado","cancelada").eq("no_show",false).lt("fecha_entrada",draft.end).gt("fecha_salida",draft.start),
    ...
    if(conflictRes.error)throw conflictRes.error;if(blockRes.error)throw blockRes.error
    const roomOverlaps=(item,id)=>reservationRooms(item).includes(id)&&item.fecha_entrada<draft.end&&effectiveRoomEnd(item,id)>draft.start
    const conflict=(conflictRes.data||[]).find(item=>roomIds.some(id=>roomOverlaps(item,id)))
    if(conflict){const conflictRoom=selectedRooms.find(room=>roomOverlaps(conflict,Number(room.id)));throw new Error(`La habitación ${conflictRoom?.nombre||"seleccionada"} ya tiene una reserva que se superpone con esas fechas.`)}
    ...
    const assignedGuests=details.reduce((sum,item)=>sum+item.huespedes,0);if(assignedGuests!==requestedGuests)throw new Error(`Distribuí los ${requestedGuests} huésped${requestedGuests===1?"":"es"} entre las habitaciones seleccionadas antes de crear la reserva.`)
    ...
    const payload={property_id:propertyId,user_id:userData?.user?.id||null,habitacion_id:roomIds[0],habitaciones_ids:roomIds,habitaciones_detalle:details,fecha_entrada:draft.start,fecha_salida:draft.end,tipo_estadia:"overnight",nombre_huesped:draft.guest.trim(),email_huesped:draft.email?.trim()||null,telefono_huesped:draft.phone?.trim()||null,pais_huesped:draft.country?.trim()||null,cantidad_huespedes:totalGuests,canal_reserva:draft.channel||"Walk-in",codigo_canal:draft.voucher?.trim()||null,tarifa_noche:rate,noches:nights,subtotal,descuento_tipo:discountAmount>0?discountType:"none",descuento_valor:discountAmount>0?discountValue:0,descuento_importe:discountAmount,descuento_motivo:discountReason,descuento_origen:discountAmount>0?"manual":null,precio_total:total,moneda:draft.currency||"ARS",notas:draft.notes?.trim()||null,...taxPayload,cancellation_policy_id:policy.id,cancellation_policy_snapshot:policySnapshot(policy),mascotas:[],mascotas_total:0,servicios:[],pasajeros:[],vehiculos:0,cochera_total:0,estado:draft.status||"confirmada",no_show:false}
    const{data,error:rpcError}=await supabase.rpc("hl_create_reservation_atomic",{p_reservation:payload,p_payments:[]});if(rpcError)throw rpcError
    const created={...data,payment_paid:0,payment_foreign_count:0,payment_last_id:null,payment_last_at:null};if(data.fecha_entrada<windowEndExclusive&&data.fecha_salida>=windowStart)setReservations(list=>[...list,created])
    const names=selectedRooms.map(room=>room.nombre).join(", ");toast({title:roomIds.length>1?"Reserva grupal creada":"Reserva creada",message:`${draft.guest.trim()} · ${roomIds.length>1?`${roomIds.length} habitaciones (${names})`:`Habitación ${names}`} · ${nights} noche${nights===1?"":"s"}${discountAmount>0?` · descuento ${discountType==="percent"?`${discountValue}%`:money(discountAmount,draft.currency)} (${reasonLabel})`:""} · ${policy.name}.`})
    if(typeof window!=="undefined")window.dispatchEvent(new CustomEvent("hl:pms-reservation-updated",{detail:{reservationId:data.id}}));return created
