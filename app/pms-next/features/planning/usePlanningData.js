...
import{roundInternalPrice,roundPrice}from"../../core/priceTax"
import resolveReservationRatePlanPricing from"./reservationRatePlanPricing"
...
export default function usePlanningData(propertyId,windowStart,windowEndExclusive){
  ...
  const createReservation=useCallback(async draft=>{
    ...
    const isGroup=roomIds.length>1,nights=nightsBetween(draft.start,draft.end)
    const{resolvedPricing,groupTaxes,pricing,details,requestedGuests,regimen}=await resolveReservationRatePlanPricing({propertyId,draft,roomIds,selectedRooms,maintenanceByRoom})
