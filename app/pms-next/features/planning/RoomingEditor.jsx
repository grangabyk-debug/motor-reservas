...
export default function RoomingEditor({draft,setDraft,rooms=[],currency="ARS"}){
  ...
  const grid={display:"grid",gridTemplateColumns:"minmax(120px,1.05fr) minmax(78px,.6fr) minmax(175px,1.2fr) minmax(88px,.65fr) minmax(110px,.75fr)",gap:9,alignItems:"end"}
  ...
  return <section style={shell} aria-label="Rooming por habitación">
    <header style={top}><div><small style={{display:"block",fontSize:9,fontWeight:900,letterSpacing:".1em",color:"var(--accent)"}}>ROOMING</small><b style={{display:"block",marginTop:2,fontSize:12}}>Configuración por habitación</b></div><span style={{fontSize:10,color:"var(--muted)"}}>{rooms.length} habitación{rooms.length===1?"":"es"}</span></header>
    {rooms.map(room=>{const id=String(room.id),assignment=assignments[id]||makeAssignment(room,Math.min(Number(draft.guests)||1,roomCapacity(room))),options=Array.from({length:roomCapacity(room)},(_,index)=>index+1),bedOptions=Array.from({length:roomCapacity(room)+1},(_,index)=>index);return <article key={id} style={row}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}><span style={{width:19,height:19,display:"grid",placeItems:"center",borderRadius:6,background:"color-mix(in srgb,#36a269 14%,transparent)",color:"#268357",fontSize:11,fontWeight:950}}>✓</span><b style={{fontSize:11}}>Hab. {room.nombre}</b><span style={{marginLeft:"auto",display:"inline-flex",alignItems:"center",gap:5,fontSize:9.5,color:"#2c8b58"}}><i style={{width:7,height:7,borderRadius:"50%",background:"#3cac6b",boxShadow:"0 0 0 3px color-mix(in srgb,#3cac6b 12%,transparent)"}}/>Libre</span></div>
      <div style={grid}>
        ...
        <div><span style={tinyLabel}>Instancia</span><div style={{...control,display:"flex",alignItems:"center",gap:6}}><i style={{width:7,height:7,borderRadius:"50%",background:"#3cac6b"}}/>Libre</div></div>
    ...
    <footer style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:14,padding:"11px 12px",background:"color-mix(in srgb,var(--bg) 34%,var(--panelSolid))"}}><span style={{fontSize:10,fontWeight:800,color:"var(--muted)"}}>Ingreso por noche</span><b style={{minWidth:110,padding:"8px 11px",border:"1px solid var(--line)",borderRadius:10,background:"var(--panelSolid)",fontSize:12,textAlign:"right"}}>{money(totalRate,currency)}</b></footer>
