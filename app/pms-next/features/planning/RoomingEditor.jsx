...
export default function RoomingEditor({draft,setDraft,rooms=[],categories=[],currency="ARS",editableRate=true,ratePlans=null,defaultRatePlanCode="",allowRatePlanOverride=false}){
  ...
  return <section style={shell} aria-label="Rooming por habitación">
    ...
    {effectiveRooms.map(room=>{const id=String(room.id),assignment=assignments[id]||makeAssignment(room,0),capacity=roomCapacity(room),options=Array.from({length:capacity+1},(_,index)=>index),matOptions=Array.from({length:Math.floor(capacity/2)+1},(_,index)=>index),indOptions=Array.from({length:capacity+1},(_,index)=>index),physical=clean(room.tipo)||"Habitación",sold=clean(assignment.soldAs)||physical,different=sold!==physical,effectiveNetRate=effectiveRateFor(room,assignment),selectedPlan=ratePlanByCode(planConfig,assignment.ratePlanCode||draft.ratePlanCode||defaultPlanCode),planNet=ratePlanAmounts(selectedPlan?.active?selectedPlan:defaultRatePlan(planConfig),assignment.guests,{enabled:draft?.impuestosDesglosados!==false,vat_rate:vatRate,price_tax_mode:draft?.priceTaxMode||"tax_included"},1).netPerNight;return <article key={id} style={row}>
      ...
      <div style={grid}>
        ...
        <label><span style={{...tinyLabel,color:"var(--accent)"}}>Huéspedes</span><select value={assignment.guests} onChange={event=>update(room,{guests:Number(event.target.value)})} style={control}>{options.map(value=><option key={value} value={value}>{value}</option>)}</select></label>
