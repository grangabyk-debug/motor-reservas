...
function makeAssignment(room,guests){const beds=defaultBeds(room,guests);return{soldAs:clean(room?.tipo)||"Habitación",guests:clamp(guests,0,roomCapacity(room)),matrimonial:beds.matrimonial,individual:beds.individual,rate:Number(room?.precio)||0,manualRate:false}}
...
function ChevronDownIcon(){return <svg viewBox="0 0 12 8" width="10" height="7" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="m1.5 1.5 4.5 4 4.5-4"/></svg>}

export default function RoomingEditor({draft,setDraft,rooms=[],categories=[],currency="ARS",editableRate=true}){
    ...
    setDraft(current=>{
      ...
      effectiveRooms.forEach(room=>{
        ...
        if(previous){
          ...
          next[id]={soldAs:clean(previous.soldAs)||clean(room.tipo)||"Habitación",guests:desired,matrimonial:beds.matrimonial,individual:beds.individual,rate:Number(previous.rate??room.precio)||0,manualRate:Boolean(previous.manualRate)}
      ...
      const sameAssignments=sameKeys&&ids.every(id=>{const a=existing[id],b=next[id];return clean(a.soldAs)===clean(b.soldAs)&&Number(a.guests||0)===Number(b.guests||0)&&Number(a.matrimonial||0)===Number(b.matrimonial||0)&&Number(a.individual||0)===Number(b.individual||0)&&Number(a.rate||0)===Number(b.rate||0)&&Boolean(a.manualRate)===Boolean(b.manualRate)})
  ...
  function update(room,patch){
    ...
    setDraft(current=>{
      ...
      next.soldAs=clean(next.soldAs)||clean(room.tipo)||"Habitación";next.guests=clamp(next.guests,0,capacity);next.rate=Math.max(0,Number(next.rate)||0);if(Object.prototype.hasOwnProperty.call(patch,"rate"))next.manualRate=true
  ...
  const grid={display:"grid",gridTemplateColumns:"minmax(108px,1.05fr) minmax(64px,.55fr) minmax(82px,.68fr) minmax(82px,.68fr) minmax(96px,.85fr)",gap:7,alignItems:"end"}
  ...
  const bedControl=tone=>({height:36,boxSizing:"border-box",display:"grid",gridTemplateColumns:"auto minmax(0,1fr) auto",alignItems:"center",gap:6,padding:"0 8px",border:"1px solid var(--line)",borderRadius:10,background:"color-mix(in srgb,var(--panelSolid) 88%,transparent)",color:tone,transition:"border-color .16s ease,box-shadow .16s ease"})
  const bedSelect={width:"100%",height:"100%",minWidth:0,border:0,background:"transparent",color:"var(--text)",font:"inherit",fontSize:11,fontWeight:850,outline:"none",boxShadow:"none",appearance:"none",WebkitAppearance:"none",padding:0,cursor:"pointer"}
  return <section style={shell} aria-label="Rooming por habitación">
    ...
    {effectiveRooms.map(room=>{const id=String(room.id),assignment=assignments[id]||makeAssignment(room,0),capacity=roomCapacity(room),options=Array.from({length:capacity+1},(_,index)=>index),matOptions=Array.from({length:Math.floor(capacity/2)+1},(_,index)=>index),indOptions=Array.from({length:capacity+1},(_,index)=>index),physical=clean(room.tipo)||"Habitación",sold=clean(assignment.soldAs)||physical,different=sold!==physical;return <article key={id} style={row}>
      ...
      <div style={grid}>
        ...
        <label><span style={tinyLabel}>Matrimonial</span><span data-bed-control title={`Cama matrimonial · ocupa 2 plazas · capacidad máxima ${capacity}`} style={bedControl("var(--accent)")}><DoubleBedIcon/><select aria-label={`Camas matrimoniales en habitación ${room.nombre}`} value={assignment.matrimonial} onChange={event=>update(room,{matrimonial:Number(event.target.value)})} style={bedSelect}>{matOptions.map(value=><option key={value} value={value}>{value}</option>)}</select><span aria-hidden="true" style={{display:"grid",placeItems:"center",color:"var(--muted)"}}><ChevronDownIcon/></span></span></label>
        <label><span style={tinyLabel}>Individual / twin</span><span data-bed-control title={`Cama individual / twin · ocupa 1 plaza · capacidad máxima ${capacity}`} style={bedControl("#7b65d8")}><SingleBedIcon/><select aria-label={`Camas individuales en habitación ${room.nombre}`} value={assignment.individual} onChange={event=>update(room,{individual:Number(event.target.value)})} style={bedSelect}>{indOptions.map(value=><option key={value} value={value}>{value}</option>)}</select><span aria-hidden="true" style={{display:"grid",placeItems:"center",color:"var(--muted)"}}><ChevronDownIcon/></span></span></label>
        <label><span style={tinyLabel}>Tarifa final</span><input type="number" min="0" value={displayRate(assignment.rate)} disabled={!editableRate} readOnly={!editableRate} onChange={editableRate?event=>update(room,{rate:netRate(event.target.value)}):undefined} title={editableRate?"Precio final por noche":"La tarifa se modifica con la lógica de cambio de habitación / estadía"} style={{...control,opacity:editableRate?1:.68,cursor:editableRate?"text":"not-allowed"}}/></label>
      </div>
    </article>})}
    ...
    <style>{`[aria-label="Rooming por habitación"] [data-bed-control]:focus-within{border-color:color-mix(in srgb,var(--accent) 52%,var(--line))!important;box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 7%,transparent)}@media(max-width:760px){[aria-label="Rooming por habitación"] article>div:nth-child(2){grid-template-columns:1fr 1fr!important}[aria-label="Rooming por habitación"] article>div:nth-child(2)>label:last-child{grid-column:1/-1}}`}</style>
  </section>
}
