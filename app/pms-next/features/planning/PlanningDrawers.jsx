...
const BED_OPTIONS=[
  ...
  {value:"matrimonial",label:"1 cama matrimonial"},
  {value:"twin",label:"2 camas twin"},
  {value:"individual",label:"1 cama individual"},
  {value:"matrimonial_twin",label:"1 matrimonial + 1 twin"},
  {value:"triple_twin",label:"3 camas twin"},
]
...
export function ReservationDetailDrawer({selected,room,rooms=[],onClose,onOpen}){
  ...
  return <aside className={s.detailDrawer}><header><div><small>{selected.canal_reserva||"Walk-in"} · {selected.numero_reserva||selected.id}</small><h2>{selected.nombre_huesped}</h2><p>{roomLabel}</p></div><button className={s.close} onClick={onClose}>×</button></header><div className={s.detailGrid}><div><small>Llegada</small><b>{longDate(selected.fecha_entrada)}</b></div><div><small>Salida</small><b>{longDate(selected.fecha_salida)}</b></div><div><small>Noches</small><b>{Math.max(1,diffDays(selected.fecha_entrada,selected.fecha_salida))}</b></div><div><small>Estado en Planning</small><b><span className={`${l.stagePill} ${l[stage]}`}>{planningStageLabel(selected)}</span></b></div><div><small>Huéspedes</small><b>{selected.cantidad_huespedes||1}</b></div><div><small>Tipo de cama</small><b>{bedLabel(selected.tipo_cama)}</b></div><div><small>Total</small><b>{money(selected.precio_total,selected.moneda)}</b></div></div>{selected.telefono_huesped||selected.email_huesped?<div className={s.contactBlock}>{selected.telefono_huesped?<p><small>Teléfono</small><b>{selected.telefono_huesped}</b></p>:null}{selected.email_huesped?<p><small>Email</small><b>{selected.email_huesped}</b></p>:null}</div>:null}{selected.notas?<div className={s.noteBlock}><small>Notas</small><p>{selected.notas}</p></div>:null}<footer><button onClick={onClose}>Cerrar</button><button className={s.primary} onClick={onOpen}>Abrir reserva</button></footer></aside>
}

export function CreateReservationDrawer({draft,setDraft,drawerStep,setDrawerStep,draftState,externalError="",availableRooms,roomById,nights,total,saving,onClose,onDiscard,onNext,onSave}){
  if(!draft)return null
  const guests=Math.max(1,Number(draft.guests)||1)
  const sortedRooms=[...availableRooms].sort((a,b)=>compareRooms(a,b,guests))
  const availabilityKey=sortedRooms.map(room=>`${room.id}:${room.available?1:0}:${room.estado||""}:${room.capacidad||1}`).join("|")
  const rawSelectedRoomIds=uniqueIds(draft.roomIds?.length?draft.roomIds:[draft.roomId])
  const selectedRoomIds=rawSelectedRoomIds.filter(id=>{const room=availableRooms.find(item=>String(item.id)===id);return room&&room.available&&room.estado!=="mantenimiento"})
  ...
  const netTotal=Math.max(0,grossTotal-discountAmount),visibleError=localError||externalError
  ...
  const checkRow={display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,padding:"9px 10px",border:"1px solid var(--line)",borderRadius:9,background:"var(--panelSolid)",fontSize:11}
  return <div className={s.drawerShade} onMouseDown={event=>event.target===event.currentTarget&&onClose()}><aside className={s.createDrawer} role="dialog" aria-modal="true" aria-label="Crear reserva"><header className={s.drawerHeader}><div><small>CREAR RESERVA</small><h2>{title}</h2><p>{draftState}</p></div><button className={s.close} onClick={onClose}>×</button></header><nav className={s.stepNav}>{STEPS.map((step,index)=><button key={step} className={index===drawerStep?s.stepActive:index<drawerStep?s.stepDone:""} onClick={()=>index<=drawerStep&&setDrawerStep(index)}><span>{index+1}</span>{step}</button>)}</nav><div className={s.drawerBody}>{visibleError?<div style={errorStyle} role="alert">{visibleError}</div>:null}
    {drawerStep===0?<div className={s.stepPanel}><div className={s.dateSummary}><span><small>Entrada</small><b>{longDate(draft.start)}</b></span><span className={s.nightsBadge}>{nights} noche{nights===1?"":"s"}</span><span><small>Salida</small><b>{longDate(draft.end)}</b></span></div><div className={s.formGrid}><label>Entrada<input type="date" value={draft.start} onChange={event=>setDraft(current=>({...current,start:event.target.value,end:current.end<=event.target.value?addDays(event.target.value,1):current.end,roomSelectionManual:false}))}/></label><label>Salida<input type="date" min={addDays(draft.start,1)} value={draft.end} onChange={event=>setDraft(current=>({...current,end:event.target.value,roomSelectionManual:false}))}/></label><label>Huéspedes<input type="number" min="1" value={draft.guests} onChange={event=>setDraft(current=>({...current,guests:Number(event.target.value)||1,roomSelectionManual:false}))}/></label><label>Moneda<select value={draft.currency} onChange={event=>setDraft(current=>({...current,currency:event.target.value}))}><option value="ARS">ARS</option><option value="USD">USD</option></select></label></div><p className={s.stepHelp}>Para una reserva grupal, arrastrá un rectángulo sobre varios días y varias habitaciones en el Planning. También podés sumar o quitar habitaciones en el paso siguiente.</p></div>:null}
    {drawerStep===1?<div className={s.stepPanel}><div className={g.groupSummary}><div><small>Habitaciones seleccionadas</small><b>{selectedRoomIds.length||0}</b><span>{roomNames.length?roomNames.join(", "):"No hay habitaciones disponibles para estas fechas"}</span>{selectedRoomIds.length>1?<span className={g.groupTag}>Reserva grupal</span>:null}</div><div><small>Capacidad combinada</small><b>{capacity} pax</b><span>{selectedRoomIds.length>1?"Una sola reserva, varias habitaciones":"Selección individual"}</span></div></div><p className={g.roomHint}>Preseleccionamos la habitación disponible con la capacidad más ajustada a {guests} huésped{guests===1?"":"es"}. Las disponibles aparecen primero; las ocupadas quedan debajo.</p><div className={s.roomOptions}>{sortedRooms.map(room=>{const unavailable=!room.available||room.estado==="mantenimiento",active=!unavailable&&selectedRoomIds.includes(String(room.id)),recommended=!draft.roomSelectionManual&&recommendedRoom&&String(recommendedRoom.id)===String(room.id);return <button key={room.id} disabled={unavailable} className={`${s.roomOption} ${active?s.roomOptionActive:""}`} onClick={()=>toggleRoom(room)}><span className={`${g.roomCheck} ${active?g.roomCheckSelected:""}`}>{active?"✓":""}</span><span><small>{room.tipo||"Sin tipo"}</small><b>{room.nombre}</b><em>{room.capacidad||1} pax · {room.estado==="mantenimiento"?"Mantenimiento":room.available?recommended?"Disponible · mejor ajuste":"Disponible":"Ocupada"}</em></span><strong>{money(room.precio,draft.currency)}<small>/ noche</small></strong></button>})}</div>{!selectedRoomIds.length?<p className={s.stepHelp}>No hay una habitación disponible seleccionable en este rango. Cambiá las fechas para continuar.</p>:null}<div className={s.formGrid} style={{marginTop:10}}><label>Configuración de cama<select value={draft.bedType||""} onChange={event=>setDraft(current=>({...current,bedType:event.target.value}))}>{BED_OPTIONS.map(option=><option key={option.value||"any"} value={option.value}>{option.label}</option>)}</select></label><label>Tarifa total por noche<input type="number" min="0" value={draft.rate} onChange={event=>setDraft(current=>({...current,rate:Number(event.target.value)||0}))}/></label></div><p className={g.roomListNote}>La cama queda como configuración/preferencia de la reserva. La tarifa total por noche es la suma de las habitaciones seleccionadas y se puede ajustar manualmente.</p></div>:null}
    ...
    {drawerStep===3?<div className={s.stepPanel}><div className={s.review}><div><small>Estadía</small><b>{longDate(draft.start)} — {longDate(draft.end)}</b><span>{nights} noche{nights===1?"":"s"}</span></div><div><small>{selectedRoomIds.length>1?"Habitaciones":"Habitación"}</small><b>{roomNames.slice(0,3).join(", ")||"—"}{roomNames.length>3?` +${roomNames.length-3}`:""}</b><span>{selectedRoomIds.length>1?`${selectedRoomIds.length} habitaciones · ${capacity} pax de capacidad`:selectedRooms[0]?.tipo||""} · {bedLabel(draft.bedType)}</span></div><div><small>Titular</small><b>{`${draft.firstName} ${draft.lastName}`.trim()||"Sin completar"}</b><span>{draft.phone||draft.email||"Sin contacto"}</span></div><div><small>Total</small><b>{money(netTotal,draft.currency)}</b><span>{discountAmount>0?`${money(grossTotal,draft.currency)} − ${money(discountAmount,draft.currency)}`:`${money(draft.rate,draft.currency)} × ${nights}`}</span></div></div><div className={s.formGrid}><label>Canal<select value={draft.channel||"Walk-in"} onChange={event=>setDraft(current=>({...current,channel:event.target.value}))}><option>Walk-in</option><option>WhatsApp</option><option>Telefónica</option><option>Motor</option><option>Booking.com</option><option>Expedia</option><option>Despegar</option><option>Airbnb</option><option>Agencia</option></select></label><label>Estado comercial<select value={draft.status} onChange={event=>setDraft(current=>({...current,status:event.target.value}))}><option value="confirmada">Venta · confirmada / garantizada</option><option value="pendiente">Preventa · pendiente de confirmación</option><option value="tentativa">Preventa · tentativa</option></select></label><label className={s.fullField}>Voucher / referencia<input value={draft.voucher||""} onChange={event=>setDraft(current=>({...current,voucher:event.target.value}))} placeholder="Código de OTA, voucher, referencia externa…"/></label></div><div style={{...sectionStyle,marginTop:10}}><b style={{fontSize:11}}>Descuento</b><div className={s.formGrid} style={{marginTop:8}}><label>Tipo<select value={discountType} onChange={event=>setDraft(current=>({...current,discountType:event.target.value,discountValue:event.target.value==="none"?0:current.discountValue||0}))}><option value="none">Sin descuento</option><option value="percent">Porcentaje</option><option value="amount">Importe fijo</option></select></label><label>Valor<input type="number" min="0" max={discountType==="percent"?100:undefined} disabled={discountType==="none"} value={draft.discountValue||0} onChange={event=>setDraft(current=>({...current,discountValue:Number(event.target.value)||0}))}/></label></div>{discountAmount>0?<small style={{display:"block",marginTop:6,color:"var(--muted)"}}>Descuento aplicado: {money(discountAmount,draft.currency)} · Total final {money(netTotal,draft.currency)}</small>:null}</div><div style={{...sectionStyle,marginTop:10}}><b style={{fontSize:11}}>Confirmación al huésped</b><p style={{margin:"4px 0 9px",fontSize:10,color:"var(--muted)"}}>Las opciones quedan preparadas dentro del flujo y se habilitan al conectar el servicio de mensajería.</p><div style={{display:"grid",gap:7}}><label style={{...checkRow,opacity:.58}}><span>Enviar por email{draft.email?` · ${draft.email}`:" · falta email"}</span><input type="checkbox" disabled/></label><label style={{...checkRow,opacity:.58}}><span>Enviar por WhatsApp{draft.phone?` · ${draft.phone}`:" · falta teléfono"}</span><input type="checkbox" disabled/></label></div></div><label className={s.fullField} style={{display:"grid",gap:5,marginTop:10}}>Notas<textarea rows="4" value={draft.notes} onChange={event=>setDraft(current=>({...current,notes:event.target.value}))} placeholder="Pedidos, observaciones o información interna"/></label></div>:null}
  </div><footer className={s.drawerFooter}><div><button className={s.dangerLink} onClick={onDiscard}>Descartar</button>{drawerStep>0?<button onClick={()=>{setLocalError("");setDrawerStep(step=>Math.max(0,step-1))}}>Atrás</button>:null}</div><div className={s.totalFooter}><span><small>Total</small><b>{money(netTotal,draft.currency)}</b></span>{drawerStep<3?<button className={s.primary} disabled={drawerStep===1&&(hasInvalidSelection||!selectedRoomIds.length)} onClick={next}>Continuar</button>:<button className={s.primary} disabled={saving||hasInvalidSelection||!selectedRoomIds.length} onClick={event=>{if(event.currentTarget.disabled)return;event.currentTarget.disabled=true;onSave()}}>{saving?"Creando…":"Crear reserva"}</button>}</div></footer></aside></div>
}