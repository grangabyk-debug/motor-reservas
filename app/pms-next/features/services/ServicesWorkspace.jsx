...
export default function ServicesWorkspace({propertyId}){
  ...
  const modeBox={margin:"0 0 14px",padding:"11px 12px",border:"1px solid color-mix(in srgb,var(--accent) 22%,var(--line))",borderRadius:12,background:"color-mix(in srgb,var(--accent) 5%,var(--panelSolid))",fontSize:11,lineHeight:1.45}
  return <section className={s.page}>
    ...
    <div style={modeBox}><b>{priceTaxCaption(taxes)}</b><div style={{marginTop:3,color:"var(--muted)"}}>{taxes.enabled&&taxes.price_tax_mode==="tax_included"?"Ingresás el precio final que querés cobrar. Habitación Llena separa internamente neto e IVA para folios y facturación.":taxes.enabled?"Ingresás el precio neto. El IVA se suma automáticamente cuando el cargo llega a la reserva y al folio.":"Los importes se tratan como finales porque los impuestos están desactivados."}</div></div>
