...
export default function ReservationEditPanel({item,assignedRooms=[],allRooms=[],saving=false,onCancel,onPreviewMove,onMove,onUpdate,onSaved}){
  ...
  const originalNightlyStayRate=originalNightlyRate(item,currentIds)
  ...
  async function saveMetadata(baseItem=item,rateOverride=null){
    ...
    const addedNightly=hasRoomAdditions?details.filter(detail=>addedRoomIds.includes(String(detail.habitacion_id))).reduce((sum,detail)=>sum+Math.max(0,Number(detail.tarifa_noche)||0),0):0
    const patch=buildReservationMetadataPatch({baseItem,draft,details,ids,effectiveNightly,addedStayAmount:roundMoney(addedNightly*newNights),earlyPercent,latePercent,newNights})
  ...
  return <>
    <div style={overlay} onMouseDown={event=>event.target===event.currentTarget&&!busy&&onCancel?.()}>
      <section style={shell} role="dialog" aria-modal="true" aria-label="Editar reserva">
        ...
        <ReservationAddRoomControl item={item} draft={draft} setDraft={setDraft} allRooms={allRooms} currentIds={currentIds} busy={busy} serviceLocked={serviceLocked} onPreviewMove={onPreviewMove} onStatus={({error:nextError="",ok=""})=>{setAvailabilityError(nextError);setAvailabilityOk(ok)}}/>
