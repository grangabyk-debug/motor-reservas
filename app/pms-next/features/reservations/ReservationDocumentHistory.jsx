...
function DocumentPreview({doc,reservation,onClose}){
  ...
  const receiptClass=String(billing.receipt_class||"").toUpperCase(),showTaxBreakdown=Boolean(billing.tax_breakdown_required)||receiptClass==="A"
  const taxPresentation=receiptClass==="B"?"IVA incluido · no discriminado":receiptClass==="C"?"Sin IVA discriminado":showTaxBreakdown?"IVA discriminado":""
  ...
  const preview=<div className={s.overlay} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}><div className={`${s.modal} ${s.invoiceModal}`} role="dialog" aria-modal="true" aria-label={title} style={{width:"min(1180px,calc(100vw - 40px))",maxHeight:"calc(100dvh - 40px)",padding:24}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.5fr) minmax(180px,.8fr)",gap:10,margin:"10px 0 12px"}}>
      <div style={{padding:"11px 12px",border:"1px solid var(--line)",borderRadius:12,background:"color-mix(in srgb,var(--bg) 42%,var(--panelSolid))",fontSize:10.5,lineHeight:1.55}}><b>{billing.name||reservation?.nombre_huesped||"Cliente sin identificar"}</b><div>{TAX_LABELS[billing.iva_condition]||billing.iva_condition||"Condición IVA no informada"}{receiptClass?` · Factura ${receiptClass}`:""}</div>{taxPresentation?<div style={{color:"var(--muted)"}}>{taxPresentation}</div>:null}{billing.iva_condition==="monotributo"&&receiptClass==="A"?<div style={{marginTop:3,color:"var(--muted)"}}>Operación con Monotributista: el comprobante A debe incluir la leyenda fiscal exigida por ARCA.</div>:null}{billing.folio_label?<div>Folio: {billing.folio_label}</div>:null}{billing.email?<div>{billing.email}</div>:null}<div style={{marginTop:4,color:"var(--muted)"}}>{doc.number?`Comprobante ${doc.number}`:"Todavía sin número fiscal"} · {fmtDateTime(doc.issued_at||doc.created_at)}</div></div>
    ...
    <div className={s.invoiceLines}><header><h3>Conceptos</h3><span style={{fontSize:9.5,color:"var(--muted)",fontWeight:800}}>{rows.length} línea{rows.length===1?"":"s"}</span></header><div style={{overflowX:"auto",paddingBottom:2}}><div style={{minWidth:showTaxBreakdown?690:570}}><div style={{display:"grid",gridTemplateColumns:showTaxBreakdown?"minmax(240px,2fr) 72px 118px 76px 120px":"minmax(260px,2fr) 72px 130px 130px",gap:8,padding:"7px 0 3px",fontSize:9,fontWeight:850,color:"var(--muted)"}}><span>Descripción</span><span>Cant.</span><span>{showTaxBreakdown?"Precio neto":"Precio final"}</span>{showTaxBreakdown?<span>IVA</span>:null}<span style={{textAlign:"right"}}>Total</span></div>{rows.length?rows.map((row,index)=><div key={`${index}-${row.folio_item_id||row.description||"line"}`} style={{display:"grid",gridTemplateColumns:showTaxBreakdown?"minmax(240px,2fr) 72px 118px 76px 120px":"minmax(260px,2fr) 72px 130px 130px",gap:8,alignItems:"center",padding:"9px 0",borderTop:"1px solid var(--line)",fontSize:10}}><span><b>{row.description||"Concepto"}</b>{row.detail?<small style={{display:"block",marginTop:2,color:"var(--muted)"}}>{row.detail}</small>:null}</span><span>{Number(row.quantity)||1}</span><span>{money(showTaxBreakdown?row.unit_price:(Number(row.unit_price)||0)*(1+(Number(row.tax_rate)||0)/100),currency)}</span>{showTaxBreakdown?<span>{Number(row.tax_rate)||0}%</span>:null}<strong style={{textAlign:"right"}}>{money(rowTotal(row),currency)}</strong></div>):<div style={{padding:"16px 0",color:"var(--muted)",fontSize:10}}>El documento no tiene líneas detalladas guardadas.</div>}</div></div></div>
    <div className={s.invoiceTotals}>{showTaxBreakdown?<><span>Precio neto <b>{money(doc.subtotal,currency)}</b></span><span>IVA discriminado <b>{money(doc.tax,currency)}</b></span></>:<span>{taxPresentation||"Precio final"}</span>}<strong>Total {money(total,currency)}</strong></div>
