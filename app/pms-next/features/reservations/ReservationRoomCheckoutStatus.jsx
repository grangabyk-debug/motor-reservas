...
export function roomCheckoutPresentation(item,roomingRows=[],assigned=[],fallbackRoom=null){
  ...
  const total=logicalIds.size||1,count=Object.keys(dates).length,earlyCount=activeDetails.filter(detail=>detail?.early_checkin_requested).length,lateCount=activeDetails.filter(detail=>detail?.late_checkout_requested).length
