...
export default function ReservationStayActions({item,saving=false,onPrimary,onNoShow,onCancel}){
  ...
  const mainLabel=item?.no_show?"No Show":isCheckout?"Check-out":"Check-in"
  ...
  const mainStyle={height:36,minWidth:108,padding:"0 12px",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:7,border:"1px solid color-mix(in srgb,var(--accent) 34%,var(--line))",borderRadius:10,background:item?.no_show?"linear-gradient(145deg,#4d5665,#303640)":isCheckout?"linear-gradient(145deg,#ec6370,#bd3947)":"linear-gradient(145deg,#31bc6b,#159447)",color:"#fff",font:"inherit",fontSize:11,fontWeight:900,cursor:menuDisabled?"not-allowed":"pointer",boxShadow:item?.no_show?"0 7px 18px rgba(48,54,64,.18)":isCheckout?"0 7px 18px rgba(189,57,71,.20)":"0 7px 18px rgba(21,148,71,.18)",opacity:menuDisabled?.5:1}
  ...
  return <div ref={rootRef} style={{position:"relative",display:"inline-flex"}}>
    <button type="button" aria-haspopup="menu" aria-expanded={open} disabled={menuDisabled} onClick={()=>setOpen(value=>!value)} style={mainStyle}>
      <span>{mainLabel}</span>
    ...
    {open?<div role="menu" style={{position:"absolute",right:0,top:"calc(100% + 7px)",zIndex:260,width:190,padding:6,border:"1px solid var(--line)",borderRadius:12,background:"color-mix(in srgb,var(--panelSolid) 97%,transparent)",boxShadow:"0 18px 46px rgba(18,30,52,.18)",backdropFilter:"blur(18px)",WebkitBackdropFilter:"blur(18px)"}}>
      <button type="button" role="menuitem" disabled={primaryDisabled} onClick={()=>run(onPrimary)} style={{...menuButton,opacity:primaryDisabled?.45:1,cursor:primaryDisabled?"not-allowed":"pointer"}}>{isCheckout?"Hacer check-out":"Hacer check-in"}</button>
