...
function Modal({title,subtitle,onClose,children}){
  return <div style={{position:"fixed",inset:0,zIndex:270,display:"grid",placeItems:"center",padding:16,background:"rgba(12,20,38,.38)"}} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}><section style={{width:"min(560px,calc(100vw - 28px))",maxHeight:"86vh",overflow:"auto",padding:16,border:"1px solid color-mix(in srgb,#fff 36%,var(--line))",borderRadius:20,background:"var(--panelSolid)",boxShadow:"0 28px 80px rgba(20,30,55,.26)"}}><header style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start"}}><div><small style={{fontSize:9.5,fontWeight:900,letterSpacing:".1em",color:"var(--accent)"}}>RESERVA</small><h3 style={{margin:"3px 0 0",fontSize:18}}>{title}</h3>{subtitle?<p style={{margin:"5px 0 0",fontSize:10.5,lineHeight:1.45,color:"var(--muted)"}}>{subtitle}</p>:null}</div><button type="button" onClick={onClose} style={{width:36,height:36,border:"1px solid var(--line)",borderRadius:10,background:"var(--panel)",color:"var(--text)",fontSize:18}}>×</button></header><div style={{marginTop:13}}>{children}</div></section></div>
}
