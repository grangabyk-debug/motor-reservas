...
function Icon({name,size=17}){
  ...
  if(name==="rules")return <svg {...common}><path d="M4 6h10M18 6h2M4 12h3M11 12h9M4 18h8M16 18h4"/><circle cx="16" cy="6" r="2"/><circle cx="9" cy="12" r="2"/><circle cx="14" cy="18" r="2"/></svg>
  return <svg {...common}><path d="M12 3 4.8 6v5.2c0 4.6 3 7.8 7.2 9.8 4.2-2 7.2-5.2 7.2-9.8V6L12 3Z"/><path d="m8.5 12 2.2 2.2 4.8-5"/></svg>
}
...
export default function GuestAttentionPanel({propertyId}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const today=todayKey(),attention=[]
      for(const guest of guests){
        ...
        const kind=birthday?"birthday":['vip','signature'].includes(vip)?"vip":"returning",benefit=clean(profile.preferences?.benefit_note)||(birthday?birthdayBenefit:returning?returnBenefit:"Revisar preferencias y nivel de servicio antes de atender.")
        attention.push({id:guest.id,kind,name:guest.full_name||profile.full_name||reservation.nombre_huesped||"Huésped",days,count,vip,reservation,room:roomById.get(Number(guest.room_id||reservation.habitacion_id))||"—",benefit})
      }
      attention.sort((a,b)=>(a.kind==="birthday"?0:a.kind==="vip"?1:2)-(b.kind==="birthday"?0:b.kind==="vip"?1:2)||(a.days??99)-(b.days??99));setRows(attention.slice(0,8))
    }catch(err){setMessage(err?.message||"No se pudo leer la atención de huéspedes.")}
  ...
  const prefs=settings?.preferences||{},autoNoShow=Boolean(prefs.auto_no_show_enabled),autoTime=prefs.auto_no_show_time||"02:05",dirtyPolicy=prefs.checkin_dirty_room_policy||"allow",birthdayDays=Number(prefs.birthday_notice_days??3)||0,returnThreshold=Number(prefs.returning_guest_threshold??2)||2
  const hasAttention=rows.length>0
  ...
  async function saveRules(nextPrefs){if(saving)return;setSaving(true);setMessage("");try{const next={...settings,preferences:{...(settings?.preferences||{}),...nextPrefs}};const{error}=await supabase.from("property_settings").upsert({property_id:propertyId,settings:next,updated_at:new Date().toISOString()},{onConflict:"property_id"});if(error)throw error;setSettings(next);setMessage("Reglas operativas guardadas.");setTimeout(()=>load(),50)}catch(err){setMessage(err?.message||"No se pudieron guardar las reglas.")}finally{setSaving(false)}}
  return <section style={shell} aria-label="Atención de huéspedes">
    <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"11px 13px",borderBottom:hasAttention||rulesOpen?"1px solid var(--line)":"0"}}><div style={{display:"flex",alignItems:"center",gap:9,minWidth:0}}><span style={{width:32,height:32,display:"grid",placeItems:"center",borderRadius:10,background:"color-mix(in srgb,var(--accent) 9%,var(--panelSolid))",color:"var(--accent)"}}><Icon name="occasion"/></span><div><small style={{display:"block",fontSize:9,fontWeight:900,letterSpacing:".09em",color:"var(--muted)"}}>ATENCIÓN DE HUÉSPEDES</small><b style={{display:"block",fontSize:12}}>{loading?"Leyendo huéspedes alojados…":hasAttention?`${rows.length} señal${rows.length===1?"":"es"} para recepción`:"Sin ocasiones especiales pendientes"}</b></div></div><button type="button" onClick={()=>setRulesOpen(v=>!v)} style={button}><Icon name="rules" size={15}/>Reglas</button></header>
    ...
    {rulesOpen?<div style={{padding:12,borderTop:hasAttention?"1px solid var(--line)":"0",display:"grid",gap:10}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:9}}><label style={{display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>No Show automático<select disabled={saving} value={autoNoShow?"on":"off"} onChange={e=>saveRules({auto_no_show_enabled:e.target.value==="on"})} style={input}><option value="off">Desactivado</option><option value="on">Activado</option></select></label><label style={{display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Hora de No Show<input type="time" disabled={saving||!autoNoShow} value={autoTime} onChange={e=>saveRules({auto_no_show_time:e.target.value})} style={input}/></label><label style={{display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Check-in con habitación sucia<select disabled={saving} value={dirtyPolicy} onChange={e=>saveRules({checkin_dirty_room_policy:e.target.value})} style={input}><option value="allow">Permitir con advertencia</option><option value="manager_only">Sólo con supervisor</option><option value="block">Bloquear</option></select></label><label style={{display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Avisar cumpleaños con anticipación<input type="number" min="0" max="30" defaultValue={birthdayDays} onBlur={e=>saveRules({birthday_notice_days:Math.max(0,Math.min(30,Number(e.target.value)||0))})} style={input}/></label><label style={{display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Considerar recurrente desde<input type="number" min="2" max="20" defaultValue={returnThreshold} onBlur={e=>saveRules({returning_guest_threshold:Math.max(2,Math.min(20,Number(e.target.value)||2))})} style={input}/></label><label style={{gridColumn:"1/-1",display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Beneficio sugerido por cumpleaños<input defaultValue={prefs.birthday_benefit||"Evaluar cortesía o beneficio de cumpleaños según disponibilidad y política del hotel."} onBlur={e=>saveRules({birthday_benefit:e.target.value.trim()})} style={input}/></label><label style={{gridColumn:"1/-1",display:"grid",gap:5,fontSize:9.5,fontWeight:850,color:"var(--muted)"}}>Reconocimiento sugerido para recurrentes<input defaultValue={prefs.returning_guest_benefit||"Revisar preferencias históricas y considerar reconocimiento de huésped recurrente."} onBlur={e=>saveRules({returning_guest_benefit:e.target.value.trim()})} style={input}/></label></div><small style={{fontSize:9.5,lineHeight:1.45,color:"var(--muted)"}}>El No Show automático queda desactivado hasta que el hotel lo habilite. Las cortesías son sugerencias configurables: recepción decide si aplicarlas según disponibilidad y política.</small></div>:null}
  </section>
}
