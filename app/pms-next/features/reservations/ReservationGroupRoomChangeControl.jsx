...
export default function ReservationGroupRoomChangeControl({item,allRooms=[],busy=false,onMoved}){
  const[localItem,setLocalItem]=useState(item),[sourceId,setSourceId]=useState(null),[options,setOptions]=useState([]),[loading,setLoading]=useState(false),[saving,setSaving]=useState(false),[error,setError]=useState(""),[query,setQuery]=useState(""),[mode,setMode]=useState("available"),[pending,setPending]=useState(null),[lockOpen,setLockOpen]=useState(false),[lockReason,setLockReason]=useState(""),[lockSelection,setLockSelection]=useState(()=>new Set())
  ...
  const portalTarget=typeof document!=="undefined"?(document.querySelector("[data-theme]")||document.body):null
  ...
  function openLockDialog(){
    setLockReason("")
    setLockSelection(new Set(anyLocked?activeRows.filter(row=>row.locked).map(row=>row.id):activeRows.map(row=>row.id)))
    setLockOpen(true)
  }
  function toggleLockRoom(id){setLockSelection(current=>{const next=new Set(current);next.has(id)?next.delete(id):next.add(id);return next})}

  async function saveGroupLock(){
    if(saving||busy)return
    setSaving(true);setError("")
    try{
      let latest=current,changed=0
      for(const row of activeRows){
        const shouldLock=lockSelection.has(row.id)
        if(Boolean(row.locked)===shouldLock)continue
        const{data,error:rpcError}=await supabase.rpc("hl_set_reservation_room_movement_lock",{p_reserva_id:Number(current.id),p_habitacion_id:Number(row.id),p_locked:shouldLock,p_reason:shouldLock?(lockReason.trim()||null):null})
        if(rpcError)throw rpcError
        latest=data||latest;changed+=1
      }
      setLocalItem(latest);setLockOpen(false);setLockReason("")
      if(typeof window!=="undefined"){
        const count=lockSelection.size
        window.dispatchEvent(new CustomEvent("hl:pms-toast",{detail:{title:count?"Bloqueo de habitaciones actualizado":"Grupo desbloqueado",message:count?`${count} de ${activeRows.length} habitación${activeRows.length===1?"":"es"} activa${activeRows.length===1?"":"s"} queda${count===1?"":"n"} fija${count===1?"":"s"}. Las demás siguen permitiendo movimientos.`:"Todas las habitaciones activas del grupo vuelven a permitir reasignaciones."}}))
        if(changed)window.dispatchEvent(new CustomEvent("hl:pms-reservation-updated",{detail:{reservationId:Number(current.id)}}))
      }
    }catch(err){setError(err?.message||"No se pudo actualizar el bloqueo de las habitaciones.")}
    finally{setSaving(false)}
  }
  ...
  const overlay={position:"fixed",inset:0,zIndex:278,display:"grid",placeItems:"center",padding:16,background:"rgba(10,18,34,.34)",backdropFilter:"blur(10px) saturate(1.14)",WebkitBackdropFilter:"blur(10px) saturate(1.14)"}
