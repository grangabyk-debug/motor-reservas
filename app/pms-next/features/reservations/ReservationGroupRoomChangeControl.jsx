...
export default function ReservationGroupRoomChangeControl({item,allRooms=[],busy=false,onMoved,historyMode=false}){
  ...
  useEffect(()=>{let cancelled=false;(async()=>{const{data}=await supabase.from("property_settings").select("settings").eq("property_id",item.property_id).maybeSingle();if(cancelled)return;setPropertySettings(data?.settings||{})})().catch(()=>{});return()=>{cancelled=true}},[item.property_id])
  const current=localItem||item,ids=useMemo(()=>roomIds(current),[current?.habitacion_id,JSON.stringify(current?.habitaciones_ids||[])]),today=todayKey(),isInHouse=inHouseState(current.estado),taxConfig=normalizeTaxSettings(propertySettings?.taxes||{}),rateCurrency=pricingFromSettings(propertySettings).rateCurrency||current.moneda||"ARS",currency=current.moneda||rateCurrency,displayRate=value=>commercialPriceFromNet(value,taxConfig)
  const rows=ids.map(id=>{const room=allRooms.find(value=>Number(value.id)===id),detail=detailFor(current,id),start=roomStart(current,id),end=roomPlannedEnd(current,id),release=current?.room_checkout_dates?.[String(id)],released=validDate(release),sameDayMove=isInHouse&&!released&&validDate(start)&&validDate(end)&&today===start&&today<end,midStay=isInHouse&&!released&&validDate(start)&&validDate(end)&&today>start&&today<end;return{id,room,detail,start,end,released,release,sameDayMove,midStay,moveStart:midStay?today:start,locked:Boolean(detail.movement_locked),rate:Math.max(0,Number(detail.tarifa_noche??room?.precio)||0)}})
  const movementRows=useMemo(()=>{const known=new Set(rows.map(row=>Number(row.id))),details=Array.isArray(current?.habitaciones_detalle)?current.habitaciones_detalle:[];return details.flatMap(detail=>Array.isArray(detail?.movement_history)?detail.movement_history:[]).map((entry,index)=>{const id=Number(entry?.habitacion_id);if(!id||known.has(id))return null;const room=allRooms.find(value=>Number(value.id)===id);return{key:`movement-${id}-${entry?.moved_at||index}`,id,room,detail:entry,start:validDate(entry?.fecha_entrada)?String(entry.fecha_entrada):current?.fecha_entrada,end:validDate(entry?.fecha_salida)?String(entry.fecha_salida):(validDate(entry?.fecha_entrada)?String(entry.fecha_entrada):current?.fecha_entrada),released:true,transient:true,movedAt:entry?.moved_at||entry?.physical_exit_at||"",locked:true,rate:Math.max(0,Number(entry?.tarifa_noche??room?.precio)||0)}}).filter(Boolean).sort((a,b)=>String(a.movedAt||a.start||"").localeCompare(String(b.movedAt||b.start||"")))},[JSON.stringify(current?.habitaciones_detalle||[]),allRooms,rows.map(row=>row.id).join("|")])
  const displayRows=[...rows.filter(row=>row.released||row.detail?.segment_role==="previous_room"),...movementRows,...rows.filter(row=>!row.released&&row.detail?.segment_role!=="previous_room")]
  ...
  async function applyMove(reprice,targetOverride=null){
    ...
    try{
      const rpcName=source.midStay?"hl_move_inhouse_group_room_segment_atomic":source.sameDayMove?"hl_move_inhouse_same_day_room_atomic":"hl_change_group_reservation_room_atomic"
  ...
  return <><section style={card} aria-label={historyMode?"Historial y habitación actual":"Reasignar habitación de reserva grupal"}>
    ...
    <div style={{display:"grid"}}>{displayRows.map(row=>{const historical=row.released&&row.detail?.segment_role==="previous_room",transient=Boolean(row.transient);return <article key={row.key||row.id} style={{padding:"10px 12px",display:"grid",gridTemplateColumns:"1fr auto",alignItems:"center",gap:12,borderTop:"1px solid color-mix(in srgb,var(--line) 72%,transparent)",opacity:row.released?.72:1}}><div style={{minWidth:0}}><b style={{fontSize:11.5}}>Hab. {row.room?.nombre||row.detail.nombre||row.id} · {row.room?.tipo||row.detail.categoria_asignada||"Habitación"}{transient?" · paso intermedio del mismo día":historical?" · tramo anterior":""}</b><small style={{display:"block",marginTop:3,fontSize:10,color:"var(--muted)"}}>{prettyDate(row.start)} → {prettyDate(row.end)} · Vendida como {row.detail.categoria_vendida||row.room?.tipo||"Habitación"} · {money(displayRate(row.rate),currency)}/noche{vatRate?` · IVA ${vatRate}% incluido`:""}</small>{row.midStay?<small style={{display:"block",marginTop:3,fontSize:10,color:"var(--accent)",fontWeight:800}}>Alojado · si lo movés, se corta hoy {prettyDate(row.moveStart)} y el tramo anterior queda guardado en el Planning.</small>:null}</div><button type="button" disabled={busy||saving||row.locked||row.released||transient} onClick={()=>openMove(row)} style={{...primary,opacity:(busy||saving||row.locked||row.released||transient)?0.5:1,cursor:busy||saving||row.locked||row.released||transient?"not-allowed":"pointer"}}>{transient?"Paso intermedio":historical?"Tramo anterior":row.released?"Salida realizada":row.locked?"🔒 Fija":"Cambiar habitación"}</button></article>})}</div>
  </section>

  {sourceId?<div style={overlay} onMouseDown={event=>event.target===event.currentTarget&&!loading&&!saving&&setSourceId(null)}><section style={shell} role="dialog" aria-modal="true" aria-label="Cambiar una habitación del grupo"><header style={{padding:"15px 16px",display:"flex",justifyContent:"space-between",gap:14,borderBottom:"1px solid var(--line)"}}><div><small style={{fontSize:10,fontWeight:900,letterSpacing:".09em",color:"var(--accent)"}}>{source?.midStay||source?.sameDayMove?"CAMBIO DURANTE LA ESTADÍA":"REASIGNACIÓN INDIVIDUAL DEL GRUPO"}</small><h3 style={{margin:"4px 0 0",fontSize:18}}>Hab. {source?.room?.nombre||source?.id} → elegir destino</h3><p style={{margin:"5px 0 0",fontSize:10.5,lineHeight:1.5,color:"var(--muted)"}}>{source?.midStay?<>El huésped ya está alojado. La Hab. {source?.room?.nombre||source?.id} queda registrada del {prettyDate(source?.start)} al {prettyDate(source?.moveStart)} y la nueva habitación se usará del <b>{prettyDate(source?.moveStart)}</b> al {prettyDate(source?.end)}.</>:source?.sameDayMove?<>El huésped ya está alojado y el cambio ocurre dentro del mismo día del tramo. La Hab. {source?.room?.nombre||source?.id} quedará guardada como paso intermedio y se marcará sucia; la nueva habitación pasa a ser la actual sin duplicar noches.</>:<>{prettyDate(source?.start)} → {prettyDate(source?.end)} · se modifica esta habitación para toda la estadía; las demás quedan intactas.</>}</p></div><button type="button" disabled={loading||saving} onClick={()=>setSourceId(null)} style={{...button,width:36,padding:0,fontSize:18}}>×</button></header>
