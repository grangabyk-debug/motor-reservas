...
export function buildFinanceInvoicePayload({propertyId,reservation,selected,paymentId,billingStatus,billingCurrency,billingName,billingEmail,billingPhone,billingTaxId,billingDueAt,billingNotes,invoiceCalc,invoiceLines,itemIds,billingMode,userId,taxCondition,fiscal}){
  ...
  return{
    ...
    billing_to:{name:billingName.trim(),email:billingEmail.trim()||null,phone:billingPhone.trim()||null,tax_id:String(billingTaxId||"").trim()||null,payer_type:selected.payer_type,folio_label:selected.label,iva_condition:taxCondition,iva_condition_code:fiscal.recipientCode||null,issuer_iva_condition:fiscal.issuerCondition||null,receipt_class:fiscal.receiptClass||null,receipt_type:fiscal.receiptType||null,tax_breakdown_required:Boolean(fiscal.taxBreakdownRequired)},
