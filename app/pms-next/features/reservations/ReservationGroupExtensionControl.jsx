  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      ...
      await Promise.all(rows.map(async row=>{
        ...
        }catch(err){next[row.id]={ok:false,message:err?.message||"No se pudo validar la extensión."}}
      }))
      if(cancelled)return
      setStates(next)
      setSelected(current=>new Set([...current].filter(id=>next[id]?.ok)))
    })().catch(err=>!cancelled&&setError(err?.message||"No se pudo validar la extensión.")).finally(()=>!cancelled&&setChecking(false))
  ...
  async function confirm(){
    ...
    try{
      ...
      if(typeof window!=="undefined"){
        ...
        window.dispatchEvent(new CustomEvent("hl:pms-toast",{detail:{title:"Estadía extendida",message:String(selected.size)+" habitación"+(selected.size===1?"":"es")+" extendida"+(selected.size===1?"":"s")+" hasta "+pretty(newEnd)+"."}}))
      }
      onSaved?.(result.data);setOpen(false)
    }catch(err){setError(err?.message||"No se pudieron extender las habitaciones.")}finally{setSaving(false)}
  }

  const field=<div style={{display:"grid",gap:5}}><span style={{fontSize:10,fontWeight:850,color:"var(--muted)"}}>Extensión</span><button type="button" disabled={disabled||!rows.length} onClick={()=>setOpen(true)} style={{height:39,width:"100%",border:"1px solid color-mix(in srgb,var(--accent) 30%,var(--line))",borderRadius:10,background:"color-mix(in srgb,var(--accent) 7%,var(--panelSolid))",color:"var(--accent)",font:"inherit",fontSize:10.5,fontWeight:850,cursor:disabled?"not-allowed":"pointer",opacity:disabled?.55:1}}>↔ Extender habitaciones</button></div>
