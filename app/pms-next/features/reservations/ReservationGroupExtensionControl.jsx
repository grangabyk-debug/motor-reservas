...
export default function ReservationGroupExtensionControl({item,allRooms=[],disabled=false,onPreviewMove,onSaved}){
  ...
  const rows=useMemo(()=>ids.map(id=>{const detail=reservationRoomDetail(item,id),room=roomMap.get(id);return{id,room,detail,end:String(detail?.fecha_salida||item?.fecha_salida||"").slice(0,10),start:String(detail?.fecha_entrada||item?.fecha_entrada||"").slice(0,10),late:roomSpecialStayEnabled(item,id,"late"),guests:Math.max(0,Number(detail?.huespedes)||0),planBookedPerPerson:Number(detail?.rate_plan_adjustment_booked_per_person)||0,planName:String(detail?.rate_plan_name||"")}}),[ids.join("|"),item?.habitaciones_detalle,item?.fecha_entrada,item?.fecha_salida,item?.room_checkout_dates,roomMap])
    ...
    ;(async()=>{
      ...
      await Promise.all(rows.map(async row=>{
        ...
        try{
          ...
          const quote=quoteRes.data||{},extraNights=Math.max(0,Math.round((new Date(newEnd+"T12:00:00")-new Date(row.end+"T12:00:00"))/86400000)),planBooked=round(row.planBookedPerPerson*row.guests*extraNights),planFinal=round(planBooked*factor),extensionFinal=Math.max(0,round((Number(quote.reservation_final_total)||0)+planFinal)),detailLate=Math.max(0,Number(row.detail?.late_checkout_net)||0),lateBooked=row.late?Math.max(detailLate,serviceLateBooked(item,row.id)):0,lateFinal=round(lateBooked*factor),additionalFinal=round(extensionFinal-lateFinal)
