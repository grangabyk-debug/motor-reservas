...
const diffDays=(start,end)=>Math.max(1,Math.round((new Date(`${end}T12:00:00`)-new Date(`${start}T12:00:00`))/86400000))
...
export default function ReservationRestoreCancelledDialog({item,rooms=[],reservations=[],saving=false,error="",onClose,onConfirm}){
  ...
  useEffect(()=>{if(!item)return;setStart(item.fecha_entrada||"");setEnd(item.fecha_salida||"");setSelectedRooms(uniqueRoomIds(item));setAckDuplicate(false);setRateChoice("");setPricePreview(null)},[item?.id])
  useEffect(()=>{
  ...
  const existingDetails=Array.isArray(item?.habitaciones_detalle)?item.habitaciones_detalle:[]
  const currentNightly=oldRoomIds.reduce((sum,id)=>{const detail=existingDetails.find(row=>Number(row?.habitacion_id)===Number(id));return sum+(Number(detail?.tarifa_noche)||Number(roomById.get(Number(id))?.precio)||0)},0)||Number(item?.tarifa_noche)||0
  const targetBaseNightly=selectedRooms.reduce((sum,id)=>sum+(Number(roomById.get(Number(id))?.precio)||0),0)
  const roomAssignmentChanged=selectedRooms.some((id,index)=>Number(id)!==Number(oldRoomIds[index]))
  const categoryChanged=selectedRooms.some((id,index)=>{const source=roomById.get(Number(oldRoomIds[index])),target=roomById.get(Number(id));return Number(id)!==Number(oldRoomIds[index])&&String(source?.tipo||"").trim().toLowerCase()!==String(target?.tipo||"").trim().toLowerCase()})
  const baseRateChanged=selectedRooms.some((id,index)=>{const source=roomById.get(Number(oldRoomIds[index])),target=roomById.get(Number(id));return Number(id)!==Number(oldRoomIds[index])&&Number(source?.precio||currentNightly/roomCount)!==Number(target?.precio||0)})
  const rateDecisionNeeded=roomAssignmentChanged&&(categoryChanged||baseRateChanged)
  ...
  const repriceAvailable=Boolean(pricePreview?.complete)
  const rateDecisionOk=!rateDecisionNeeded||rateChoice==="keep"||(rateChoice==="reprice"&&repriceAvailable)
  const canRestore=!saving&&!invalidDates&&selectionComplete&&!duplicateRoom&&!roomProblems.length&&(!requiresAck||ackDuplicate)&&rateDecisionOk
  ...
  const stayNights=invalidDates?0:diffDays(start,end)
  const sourceTypes=[...new Set(oldRoomIds.map(id=>roomById.get(Number(id))?.tipo).filter(Boolean))].join(" + ")||"Sin categoría"
  const targetTypes=[...new Set(selectedRooms.map(id=>roomById.get(Number(id))?.tipo).filter(Boolean))].join(" + ")||"Sin categoría"

  useEffect(()=>{
    if(!rateDecisionNeeded||invalidDates||!selectionComplete||!item?.property_id){setPricePreview(null);setLoadingPrice(false);return}
    let cancelled=false
    setLoadingPrice(true)
    ;(async()=>{
      const ids=selectedRooms.map(Number),nights=diffDays(start,end),expected=nights*ids.length
      const{data,error:priceError}=await supabase.from("hotel_rate_calendar").select("habitacion_id,stay_date,price").eq("property_id",item.property_id).in("habitacion_id",ids).gte("stay_date",start).lt("stay_date",end)
      if(cancelled)return
      if(priceError){setPricePreview({complete:false,nightly:targetBaseNightly,total:targetBaseNightly*nights,message:"No se pudo leer la tarifa vigente."});setLoadingPrice(false);return}
      const rows=data||[],complete=rows.length===expected,total=rows.reduce((sum,row)=>sum+Number(row.price||0),0)
      setPricePreview({complete,nightly:complete?total/Math.max(1,nights):targetBaseNightly,total:complete?total:targetBaseNightly*nights,message:complete?"":"Faltan tarifas del calendario para alguna noche de la habitación destino."})
      setLoadingPrice(false)
    })()
    return()=>{cancelled=true}
  },[rateDecisionNeeded,invalidDates,selectionComplete,item?.property_id,start,end,selectedRooms.join(","),targetBaseNightly])

  function setRoom(index,value){setRateChoice("");setPricePreview(null);setSelectedRooms(current=>{const next=[...current];next[index]=Number(value)||0;return next})}
  ...
  const dialog=<div role="dialog" aria-modal="true" aria-label="Restaurar reserva cancelada" style={{position:"fixed",inset:0,zIndex:120,display:"grid",placeItems:"center",padding:18,background:"rgba(9,14,25,.48)",backdropFilter:"blur(10px)"}}>
    <div style={{width:"min(860px,calc(100vw - 32px))",maxHeight:"calc(100dvh - 36px)",overflow:"auto",border:"1px solid color-mix(in srgb,var(--accent) 20%,var(--line))",borderRadius:22,background:"color-mix(in srgb,var(--panelSolid) 97%,transparent)",boxShadow:"0 28px 90px rgba(0,0,0,.28)",color:"var(--text)"}}>
      ...
      <div style={{padding:20,display:"grid",gap:14}}>
        ...
        {rateDecisionNeeded?<section style={{...card,padding:14,borderColor:"color-mix(in srgb,var(--accent) 28%,var(--line))",background:"color-mix(in srgb,var(--accent) 4%,var(--panelSolid))"}}><small style={{display:"block",fontSize:9.5,fontWeight:900,letterSpacing:".08em",color:"var(--accent)"}}>TARIFA DEL CAMBIO</small><b style={{display:"block",marginTop:3,fontSize:13}}>{sourceTypes} → {targetTypes}</b><span style={{display:"block",marginTop:3,fontSize:10.5,color:"var(--muted)"}}>Como cambiaste de habitación o categoría, elegí qué pasa con la tarifa antes de restaurar. Es el mismo criterio de un upgrade o downgrade.</span><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:11}}><button type="button" onClick={()=>setRateChoice("keep")} style={{textAlign:"left",padding:"12px 13px",border:rateChoice==="keep"?"2px solid var(--accent)":"1px solid var(--line)",borderRadius:12,background:rateChoice==="keep"?"color-mix(in srgb,var(--accent) 8%,var(--panelSolid))":"var(--panelSolid)",color:"var(--text)",font:"inherit",cursor:"pointer"}}><small style={{display:"block",fontSize:9.5,fontWeight:900,color:"var(--muted)"}}>MANTENER TARIFA</small><b style={{display:"block",marginTop:4,fontSize:14}}>{money(currentNightly,item.moneda)} / noche</b><span style={{display:"block",marginTop:3,fontSize:10,color:"var(--muted)"}}>Conserva la tarifa acordada. {stayNights} noche{stayNights===1?"":"s"}: {money(currentNightly*stayNights,item.moneda)} de alojamiento.</span></button><button type="button" disabled={loadingPrice||!repriceAvailable} onClick={()=>repriceAvailable&&setRateChoice("reprice")} style={{textAlign:"left",padding:"12px 13px",border:rateChoice==="reprice"?"2px solid var(--accent)":"1px solid var(--line)",borderRadius:12,background:rateChoice==="reprice"?"color-mix(in srgb,var(--accent) 8%,var(--panelSolid))":"var(--panelSolid)",color:"var(--text)",font:"inherit",cursor:loadingPrice||!repriceAvailable?"not-allowed":"pointer",opacity:(loadingPrice||!repriceAvailable)?0.58:1}}><small style={{display:"block",fontSize:9.5,fontWeight:900,color:"var(--accent)"}}>ACTUALIZAR TARIFA</small><b style={{display:"block",marginTop:4,fontSize:14}}>{loadingPrice?"Calculando…":money(pricePreview?.nightly||targetBaseNightly,item.moneda)} / noche</b><span style={{display:"block",marginTop:3,fontSize:10,color:"var(--muted)"}}>{loadingPrice?"Consultando el calendario tarifario…":repriceAvailable?<>Tarifa vigente del destino · {money(pricePreview?.total||0,item.moneda)} de alojamiento.</>:(pricePreview?.message||"No hay tarifa completa para recalcular.")}</span></button></div>{!rateChoice?<div style={{marginTop:8,fontSize:10.5,color:"#b87917",fontWeight:800}}>Elegí mantener o actualizar la tarifa para continuar.</div>:null}</section>:null}
