...
export default function ReservationChargePanel({reservation,propertyId,onClose,onSaved}){
  ...
  useEffect(()=>{let cancelled=false;(async()=>{setLoading(true);setError("");try{
    ...
    const rows=catalogRes.data||[],folioRows=folioRes.data||[];setCatalog(rows);setFolios(folioRows);setSettings(settingsRes.data?.settings||{});setSelectedId(rows[0]?.id||MANUAL_ID);const preferred=folioRows.find(row=>row.folio_type==="room")||folioRows.find(row=>row.folio_type==="master")||folioRows[0];setFolioId(preferred?.id||"")
  ...
  const shownUnitPrice=Math.max(0,Number(String(unitFinalPrice||"").replace(",","."))||0)
  const unitPrice=selected?netPriceFromCommercial(shownUnitPrice,taxes):0
  const total=selected?unitPrice*units*(timed?chargedNights:1):0
  const finalTotal=finalPriceFromNet(total,taxes)
  const unitLabel=selected?.category==="parking"?"Vehículos":selected?.charge_mode?.includes("person")?"Personas":"Cantidad"

  useEffect(()=>{if(!selected)return;const defaultQty=selected.charge_mode==="per_person"||selected.charge_mode==="per_person_night"?Math.max(1,Number(reservation.cantidad_huespedes)||1):1;setQuantity(defaultQty);setWholeStay(true);setFrom(stayStart);setTo(stayEnd);setNote("");setManualName("");setUnitFinalPrice(selected.id===MANUAL_ID?"":String(commercialPriceFromNet(selected.amount,taxes)))},[selectedId,taxes.enabled,taxes.vat_rate,taxes.price_tax_mode])
  ...
  async function addCharge(){
    ...
    const manual=selected.id===MANUAL_ID,chargeName=manual?manualName.trim():selected.name
    if(manual&&!chargeName){setError("Escribí qué consumo o cargo querés registrar.");return}
    if(shownUnitPrice<=0){setError("Ingresá un precio mayor a cero.");return}
    ...
    try{
      ...
      const charge={id:`charge-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,catalog_id:manual?null:selected.id,nombre:chargeName,categoria:selected.category,modo_cobro:selected.charge_mode,precio:unitPrice,precio_comercial:shownUnitPrice,cantidad:units,noches:timed?chargedNights:null,desde:timed?from:null,hasta:timed?to:null,toda_estadia:timed?wholeStay:null,folio_id:selectedFolio?.id||null,habitacion_id:selectedFolio?.room_id||null,detalle:detailParts.join(" · "),total,total_final:finalTotal,price_tax_mode:taxes.price_tax_mode,manual_price:true,created_at:new Date().toISOString()}
      ...
      const{data:event}=await supabase.from("hotel_reservation_events").insert({property_id:propertyId,reservation_id:reservation.id,event_type:"charge",title:`Cargo agregado · ${chargeName}`,detail:`${detailParts.length?`${detailParts.join(" · ")} · `:""}${money(taxEnabled?finalPriceFromNet(total,{enabled:true,vat_rate:vatRate,price_tax_mode:taxes.price_tax_mode}):total,currency)}`,payload:{catalog_id:manual?null:selected.id,name:chargeName,category:selected.category,charge_mode:selected.charge_mode,unit_price:unitPrice,quantity:units,nights:timed?chargedNights:null,from:timed?from:null,to:timed?to:null,folio_id:selectedFolio?.id||null,room_id:selectedFolio?.room_id||null,total,currency}}).select("id,event_type,title,detail,payload,actor_name,created_at").maybeSingle()
  ...
  return <div className={s.backdrop} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}><section className={s.modal} role="dialog" aria-modal="true" aria-label="Agregar cargo a la reserva">
    ...
    {loading?<div className={s.empty}>Cargando cargos disponibles…</div>:<>
      ...
      <label className={s.picker}><span>Cargo</span><div className={s.selectShell}><select value={selectedId} onChange={event=>setSelectedId(event.target.value)}>{catalog.map(row=><option key={row.id} value={row.id}>{row.name} · {money(commercialPriceFromNet(row.amount,taxes),currency)} · {priceTaxShortLabel(taxes)} · {MODE_LABEL[row.charge_mode]||row.charge_mode}</option>)}<option value={MANUAL_ID}>＋ Consumo manual / no catalogado</option></select></div>{selected?<small>{CATEGORY_LABEL[selected.category]||selected.category} · {MODE_LABEL[selected.charge_mode]||selected.charge_mode} · {money(shownUnitPrice,currency)} · {priceTaxShortLabel(taxes)}</small>:null}</label>
