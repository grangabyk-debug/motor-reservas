...
export default function ReservationMergeDialog({ item, propertyId, rooms = [], onMerge, onClose, onMerged }) {
  ...
  async function merge() {
    if (!eligible || !confirming || busy) return
    setBusy(true)
    setError("")
    try {
  ...
  return <div style={overlay} onMouseDown={event => event.target === event.currentTarget && !busy && onClose?.()}>
    <section style={panel} role="dialog" aria-modal="true" aria-label="Fusionar reservas">
      ...
      {confirming&&eligible?<div style={{marginTop:12,padding:12,border:"1px solid color-mix(in srgb,var(--accent) 35%,var(--line))",borderRadius:12,background:"color-mix(in srgb,var(--accent) 7%,var(--panelSolid))"}}><b style={{display:"block",fontSize:12}}>¿Deseás fusionar con {selected.nombre_huesped} · {selected.numero_reserva||selected.id}?</b><small style={{display:"block",marginTop:5,color:"var(--muted)",lineHeight:1.5}}>Se integrarán habitaciones, huéspedes, folios, cargos, pagos, documentos y mensajes en la reserva principal. La secundaria quedará archivada como fusionada.</small><div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:10}}><button type="button" disabled={busy} onClick={()=>setConfirming(false)} style={{height:36,padding:"0 12px",border:"1px solid var(--line)",borderRadius:9,background:"var(--panelSolid)",color:"var(--text)",font:"inherit",fontWeight:800}}>No</button><button type="button" disabled={busy} onClick={merge} style={{height:36,padding:"0 14px",border:0,borderRadius:9,background:"linear-gradient(145deg,var(--accent),var(--accent2))",color:"#fff",font:"inherit",fontWeight:900}}>{busy?"Fusionando…":"Sí, fusionar"}</button></div></div>:null}
      ...
      <footer style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--line)" }}><button type="button" onClick={onClose} disabled={busy} style={{ height: 39, padding: "0 14px", border: "1px solid var(--line)", borderRadius: 10, background: "var(--panelSolid)", color: "var(--text)", font: "inherit", fontWeight: 800 }}>Cancelar</button>{!confirming?<button type="button" onClick={()=>setConfirming(true)} disabled={!eligible||busy} style={{ height: 39, padding: "0 16px", border: 0, borderRadius: 10, background: "linear-gradient(145deg,var(--accent),var(--accent2))", color: "#fff", font: "inherit", fontWeight: 900, opacity: (!eligible||busy) ? .55 : 1 }}>Fusionar reservas</button>:null}</footer>
    </section>
  </div>
}
