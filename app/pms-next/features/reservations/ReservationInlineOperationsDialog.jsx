...
import{operationalRoomIds,sortOperationalRooms,operationalRoomLabel}from"./reservationOperationalRooms"
...
export default function ReservationInlineOperationsDialog({mode,item,rooms=[],propertyId,onClose}){
  ...
  const relatedRoomIds=useMemo(()=>operationalRoomIds(item,rooms),[item?.habitacion_id,item?.habitaciones_ids,item?.habitaciones_detalle,item?.room_checkout_dates,rooms])
  const activeRoomIds=useMemo(()=>new Set(roomIds),[roomIds.join("|")])
  const operationalRooms=useMemo(()=>sortOperationalRooms(freshRooms.length?freshRooms:rooms,activeRoomIds),[freshRooms,rooms,activeRoomIds])
  const roomMap=useMemo(()=>new Map(operationalRooms.map(room=>[Number(room.id),room])),[operationalRooms])
  ...
  async function load(){
    ...
    try{
      ...
      const roomRes=relatedRoomIds.length?await supabase.from("habitaciones").select("id,nombre,tipo,estado,housekeeping_zone").eq("property_id",propertyId).in("id",relatedRoomIds):{data:[],error:null}
      if(roomRes.error)throw roomRes.error
      setFreshRooms(roomRes.data||[])
      if(mode==="tasks"){
        ...
        if(taskRes.error)throw taskRes.error;setHkTasks(taskRes.data||[])
      }
    }catch(err){setError(err?.message||"No se pudo cargar la operación de esta reserva.")}
    finally{setLoading(false)}
  }

  useEffect(()=>{setFormOpen(false);setTaskDraft(v=>({...v,room_id:defaultRoom}));setRequestDraft(v=>({...v,room_id:defaultRoom}));setHkDraft(v=>({...v,room_id:defaultRoom}));load()},[mode,item?.id,propertyId,defaultRoom,relatedRoomIds.join("|")])
