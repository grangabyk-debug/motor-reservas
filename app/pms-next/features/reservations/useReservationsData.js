...
export default function useReservationsData(propertyId){
  ...
  const fetchPage=useCallback(async(page,{replace=false}={})=>{
    ...
    const resQuery=supabase.from("reservas").select("id,numero_reserva,nombre_huesped,email_huesped,telefono_huesped,habitacion_id,habitaciones_ids,habitaciones_detalle,room_checkout_dates,fecha_entrada,fecha_salida,estado,no_show,no_show_at,no_show_release_date,no_show_penalty_amount,no_show_penalty_status,no_show_note,garantia_tipo,garantia_marca,garantia_ultimos4,canal_reserva,codigo_canal,precio_total,precio_sin_impuestos_nacionales,iva_porcentaje,iva_importe,impuestos_desglosados,condicion_iva_huesped,subtotal,descuento_tipo,descuento_valor,descuento_importe,descuento_motivo,descuento_origen,tarifa_noche,noches,moneda,cantidad_huespedes,guest_profile_id,notas,created_at,tipo_estadia,servicios,mascotas_total,cochera_total,extra,extra_descripcion,early_checkin_importe,late_checkout_importe,regimen,hora_llegada_estimada,hora_salida_estimada,pais_huesped,nacionalidad_huesped,tipo_documento_huesped,dni_huesped,cancellation_policy_id,cancellation_policy_snapshot,cancelled_at,cancellation_penalty_amount,cancellation_penalty_status,cancellation_note,merged_into_id,merged_at,merge_note").eq("property_id",propertyId).neq("estado","fusionada").order("created_at",{ascending:false}).range(from,to)
  ...
  const moveReservation=useCallback(async({reservationId,roomId,start,end,reprice=false})=>{const{data,error:rpcError}=await supabase.rpc("hl_planning_move_reservation_with_plan_atomic",{p_reserva_id:Number(reservationId),p_habitacion_id:Number(roomId),p_fecha_entrada:start,p_fecha_salida:end,p_reprice:Boolean(reprice)});if(rpcError)throw rpcError;setReservations(list=>list.map(item=>Number(item.id)===Number(data.id)?{...item,...data}:item));notifyUpdate(data.id);return data},[])
  const checkin=useCallback(async(id,selectedRoomIds=[])=>{const roomIds=(selectedRoomIds||[]).map(Number).filter(Number.isFinite),rpc=roomIds.length?"hl_checkin_reservation_rooms_atomic":"hl_checkin_reservation_atomic",params=roomIds.length?{p_reserva_id:Number(id),p_room_ids:roomIds}:{p_reserva_id:Number(id)};const{data,error:rpcError}=await supabase.rpc(rpc,params);if(rpcError)throw rpcError;setReservations(list=>list.map(item=>item.id===data.id?{...item,...data}:item));notifyUpdate(id);return data},[])
  ...
  const markGroupRoomNoShow=useCallback(async(id,{roomId,releaseDate,penaltyAmount=0,penaltyStatus="none",note=""}={})=>{if(!roomId)throw new Error("Elegí la habitación que no se presentó.");const{data,error:rpcError}=await supabase.rpc("hl_mark_group_room_no_show_atomic",{p_reserva_id:Number(id),p_room_id:Number(roomId),p_release_date:releaseDate||null,p_penalty_amount:Number(penaltyAmount)||0,p_penalty_status:penaltyStatus||"none",p_note:note||null});if(rpcError)throw rpcError;setReservations(list=>list.map(item=>Number(item.id)===Number(data.id)?{...item,...data}:item));notifyUpdate(id);return data},[])
  ...
  const restoreCancelledReservation=useCallback(async(id,{start,end,roomIds=[],confirmDuplicate=false,reprice=false}={})=>{const{data,error:rpcError}=await supabase.rpc("hl_restore_cancelled_reservation_priced_atomic",{p_reserva_id:Number(id),p_fecha_entrada:start,p_fecha_salida:end,p_habitaciones_ids:(roomIds||[]).map(Number),p_confirm_duplicate:Boolean(confirmDuplicate),p_reprice:Boolean(reprice)});if(rpcError)throw rpcError;setReservations(list=>list.map(item=>Number(item.id)===Number(data.id)?{...item,...data}:item));notifyUpdate(id);return data},[])
  ...
  return{reservations,rooms,payments,paymentByReservation,loading,loadingMore,hasMore,error,setError,load,loadMore,updateReservation,previewMove,addRoomToReservation,mergeReservations,moveReservation,checkin,checkout,markNoShow,markGroupRoomNoShow,restoreNoShow,restoreCancelledReservation,cancelReservation}
}