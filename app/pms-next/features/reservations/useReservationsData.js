...
export default function useReservationsData(propertyId){
  ...
  const restoreCancelledReservation=useCallback(async(id,{start,end,roomIds=[],confirmDuplicate=false}={})=>{const{data,error:rpcError}=await supabase.rpc("hl_restore_cancelled_reservation_atomic",{p_reserva_id:Number(id),p_fecha_entrada:start,p_fecha_salida:end,p_habitaciones_ids:(roomIds||[]).map(Number),p_confirm_duplicate:Boolean(confirmDuplicate)});if(rpcError)throw rpcError;setReservations(list=>list.map(item=>Number(item.id)===Number(data.id)?{...item,...data}:item));notifyUpdate(id);return data},[])
