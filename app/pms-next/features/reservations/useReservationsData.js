...
export default function useReservationsData(propertyId){
  ...
  const fetchPage=useCallback(async(page,{replace=false}={})=>{
    ...
    const resQuery=supabase.from("reservas").select("id,numero_reserva,nombre_huesped,email_huesped,telefono_huesped,habitacion_id,habitaciones_ids,habitaciones_detalle,fecha_entrada,fecha_salida,estado,no_show,no_show_at,no_show_release_date,no_show_penalty_amount,no_show_penalty_status,no_show_note,garantia_tipo,garantia_marca,garantia_ultimos4,canal_reserva,codigo_canal,precio_total,subtotal,descuento_tipo,descuento_valor,descuento_importe,descuento_motivo,descuento_origen,tarifa_noche,noches,moneda,cantidad_huespedes,guest_profile_id,notas,created_at,tipo_estadia,servicios,mascotas_total,cochera_total,extra,extra_descripcion,early_checkin_importe,late_checkout_importe,regimen,hora_llegada_estimada,hora_salida_estimada,pais_huesped,nacionalidad_huesped,tipo_documento_huesped,dni_huesped,cancellation_policy_id,cancellation_policy_snapshot,cancelled_at,cancellation_penalty_amount,cancellation_penalty_status,cancellation_note").eq("property_id",propertyId).order("created_at",{ascending:false}).range(from,to)
