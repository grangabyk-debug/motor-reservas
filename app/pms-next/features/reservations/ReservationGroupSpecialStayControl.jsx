...
export default function ReservationGroupSpecialStayControl({item,allRooms=[],stayFees,disabled=false,onSaved}){
  ...
  const card=(kind,on,expected,actual,mismatch,pct,time)=>(
    <div style={{padding:"11px 12px",border:`1px solid ${on?"color-mix(in srgb,#2e9b61 48%,var(--line))":"var(--line)"}`,borderRadius:12,background:on?"color-mix(in srgb,#2e9b61 9%,var(--panelSolid))":"var(--panelSolid)",boxShadow:on?"inset 3px 0 #2e9b61,0 7px 18px color-mix(in srgb,#2e9b61 8%,transparent)":"none",display:"grid",gap:8}}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"start"}}><span><span style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><b style={{display:"block",fontSize:11.5}}>{kind==="early"?"Early check-in":"Late check-out"}</b>{on?<em style={{padding:"3px 6px",borderRadius:999,background:"#2e9b61",color:"#fff",fontStyle:"normal",fontSize:8.5,fontWeight:950,letterSpacing:".04em"}}>✓ ACTIVO</em>:null}</span><small style={{display:"block",marginTop:3,color:"var(--muted)",fontSize:10}}>{pct}% de la tarifa · {kind==="early"?"desde":"hasta"} {time}</small></span><strong style={{fontSize:12,color:on?"#26794d":"var(--text)",whiteSpace:"nowrap"}}>{money(on?actual:expected,current.moneda)}</strong></div>
