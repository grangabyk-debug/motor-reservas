...
export function quoteWithRatePlan({item,roomId,quote,start,end}){
  ...
  const pax=Math.max(0,Number(detail.rate_plan_booked_guests??detail.huespedes)||0),nights=planNights(start,end),basis=detail.rate_plan_adjustment_basis||detail.rate_plan_snapshot?.adjustment_basis||"per_person",bookedNight=Number(detail.rate_plan_adjustment_booked_per_night)||(Number(detail.rate_plan_adjustment_booked_per_person)||0)*pax,finalNight=Number(detail.rate_plan_adjustment_final_per_night)||(Number(detail.rate_plan_adjustment_final_per_person)||0)*pax,configuredValue=Number(detail.rate_plan_adjustment_configured_value)||(Number(detail.rate_plan_adjustment_configured_per_person)||0),configuredNight=configuredValue*(basis==="per_room"?1:pax)
