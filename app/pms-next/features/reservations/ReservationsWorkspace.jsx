...
export default function ReservationsWorkspace({propertyId,onNavigate,focusReservationId,onFocusHandled}){
  ...
  if(selected)return <ReservationRecord item={selected} room={selected.room} payments={data.payments} propertyId={propertyId} onBack={()=>setSelected(null)} onNavigate={onNavigate} saving={saving===String(selected.id)} onPrimaryAction={()=>selected.estado==="alojado"?checkout(selected):checkIn(selected)}/>
