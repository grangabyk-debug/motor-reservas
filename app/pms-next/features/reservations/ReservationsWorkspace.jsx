...
export default function ReservationsWorkspace({propertyId,onNavigate,focusReservationId,onFocusHandled}){
  ...
  const[query,setQuery]=useState(""),[mode,setMode]=useState("active"),[filterOpen,setFilterOpen]=useState(false),[paymentFilter,setPaymentFilter]=useState("all"),[selected,setSelected]=useState(null),[saving,setSaving]=useState("")
  ...
  const visible=useMemo(()=>items.filter(item=>{if(mode==="trash"&&item.estado!=="cancelada")return false;if(mode==="noshow"&&!item.no_show)return false;if(mode==="active"&&(item.estado==="cancelada"||item.no_show))return false;if(paymentFilter!=="all"&&item.payment!==paymentFilter)return false;const term=query.trim().toLowerCase(),roomText=(item.rooms||[]).map(room=>room.nombre).join(" ");return !term||`${item.numero_reserva||item.id} ${item.nombre_huesped} ${roomText} ${item.canal_reserva||""}`.toLowerCase().includes(term)}),[items,mode,paymentFilter,query])
