...
function CheckinConfirmDialog({item,onClose,onConfirm,onHousekeeping,saving,error}){
  if(!item)return null
...
export default function ReservationsWorkspace({propertyId,onNavigate,focusReservationId,onFocusHandled}){
  ...
  function closeRecord(){setEditOpen(false);setSelected(null);setCheckoutConfirm(null);setCheckinConfirm(null);setCheckinError("");syncRecordUrl(null)}
  ...
  const checkinDialog=<CheckinConfirmDialog item={checkinConfirm} saving={saving===String(checkinConfirm?.id)} error={checkinError} onClose={()=>{setCheckinConfirm(null);setCheckinError("")}} onHousekeeping={openHousekeepingFromCheckin} onConfirm={()=>performCheckIn(checkinConfirm)}/>
  ...
  if(selected)return <>
    ...
    <ReservationRecord item={selected} room={selected.room} rooms={selected.rooms} payments={data.payments} propertyId={propertyId} onBack={closeRecord} onNavigate={onNavigate} saving={saving===String(selected.id)} onPrimaryAction={()=>selected.estado==="alojado"?checkout(selected):requestCheckIn(selected)}/>
