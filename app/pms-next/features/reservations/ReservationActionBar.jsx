...
function GuestStayPortalShare({url,guestName,roomLabel,validUntil,phone,email,onClose}){
  ...
  return <div style={{position:"fixed",inset:0,zIndex:310,display:"grid",placeItems:"center",padding:16,background:"rgba(12,20,38,.4)"}} onMouseDown={event=>event.target===event.currentTarget&&onClose?.()}>
    <section style={{width:"min(720px,calc(100vw - 28px))",maxHeight:"90dvh",overflow:"auto",padding:20,border:"1px solid color-mix(in srgb,#fff 38%,var(--line))",borderRadius:24,background:"var(--panelSolid)",boxShadow:"0 30px 90px rgba(20,30,55,.3)"}}>
