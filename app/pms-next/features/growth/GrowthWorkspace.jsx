...
export default function GrowthWorkspace({propertyId,property}){
  ...
  return <section className={s.page}>
    <header className={s.header}><div><small>VENTAS Y CRECIMIENTO</small><h1>Vendé más sin complicar la operación</h1><p>{property?.name||"Propiedad activa"} · upsells y paquetes conectados a datos reales.</p></div><div className={s.tabs}><button className={tab==="upsells"?s.active:""} onClick={()=>setTab("upsells")}>Upsells</button><button className={tab==="packages"?s.active:""} onClick={()=>setTab("packages")}>Paquetes</button></div></header>
    {error&&<div className={s.error}>{error}</div>}{notice&&<div className={s.notice}>{notice}</div>}
    <div className={s.stats}><article><span>Upsells activos</span><b>{activeUps}</b><small>{upsells.length} configurados</small></article><article><span>Paquetes activos</span><b>{activePackages}</b><small>{packages.length} configurados</small></article><article><span>Moneda base</span><b>{currency}</b><small>Desde preferencias del hotel</small></article></div>
