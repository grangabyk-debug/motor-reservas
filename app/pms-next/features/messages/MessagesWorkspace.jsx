...
function OperationalContext({context,loading,onNavigate,allowedViews=[]}){
  ...
  function openRequests(){
    const request=guestRequests[0]||null
    if(typeof window!=="undefined"){
      const url=new URL(window.location.href)
      if(request?.id){url.searchParams.set("guest_request",String(request.id));url.searchParams.delete("request_reservation")}
      else if(reservation?.id){url.searchParams.set("request_reservation",String(reservation.id));url.searchParams.delete("guest_request")}
      window.history.replaceState(window.history.state||{},"",url)
      window.dispatchEvent(new CustomEvent("hl:guest-request-focus",{detail:{id:request?.id||"",reservationId:request?.id?"":reservation?.id||""}}))
    }
    onNavigate?.("requests",{restoreScroll:false})
  }
