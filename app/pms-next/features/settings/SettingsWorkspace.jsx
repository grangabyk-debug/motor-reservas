...
const DEFAULT_PREFS={currency:"ARS",timezone:"America/Argentina/Buenos_Aires",checkin_time:"15:00",checkout_time:"11:00",language:"es-AR",early_checkin_percent:35,late_checkout_percent:35}
...
export default function SettingsWorkspace({propertyId,property}){
  ...
  async function savePrefs(nextPrefs){setSaving("prefs");setError("");setNotice("");try{const normalized={...nextPrefs,early_checkin_percent:Math.max(0,Math.min(100,Number(nextPrefs.early_checkin_percent)||0)),late_checkout_percent:Math.max(0,Math.min(100,Number(nextPrefs.late_checkout_percent)||0))};await persistSettings({...settings,preferences:normalized},"Preferencias guardadas.")}catch(err){setError(err?.message||"No se pudieron guardar las preferencias.")}finally{setSaving("")}}
  async function saveTaxes(nextTaxes){if(!canManageTaxes)return;setSaving("taxes");setError("");setNotice("");try{await persistSettings({...settings,taxes:{...DEFAULT_TAXES,...nextTaxes,vat_rate:Math.max(0,Number(nextTaxes.vat_rate)||0)}},"Configuración de impuestos guardada. Se aplicará a nuevas reservas, folios, comprobantes y comunicaciones.")}catch(err){setError(err?.message||"No se pudo guardar la configuración de impuestos.")}finally{setSaving("")}}
...
function TaxSettingsForm({value,onSave,saving,canEdit}){
  ...
  return <div className={s.panel}><h2>Impuestos</h2><p className={s.panelIntro}>Define si la propiedad discrimina impuestos en tarifas, reservas, folios, facturas, presupuestos y comunicaciones. Argentina queda configurada por defecto con IVA 21%.</p>
    <div style={toggle}><div><b style={{display:"block",fontSize:13}}>Activar impuestos en la propiedad</b><small style={{display:"block",marginTop:4,color:"var(--muted)",lineHeight:1.45}}>Al activarlo, se mostrará Precio sin impuestos nacionales + IVA + Total. Al desactivarlo, se trabajará con un importe final sin desglose.</small></div><input type="checkbox" checked={Boolean(form.enabled)} disabled={!canEdit||saving} onChange={e=>setForm({...form,enabled:e.target.checked,show_breakdown:e.target.checked})}/></div>
    ...
    <div style={{marginTop:14,padding:"11px 12px",border:"1px solid color-mix(in srgb,var(--accent) 20%,var(--line))",borderRadius:11,background:"color-mix(in srgb,var(--accent) 5%,var(--panelSolid))",fontSize:11,lineHeight:1.5}}><b>Configuración recomendada para Argentina</b><div style={{marginTop:4,color:"var(--muted)"}}>Impuestos activos · IVA 21% · desglose visible · percepciones/impuestos adicionales disponibles en facturación. Los comprobantes conservarán precio neto, alícuota, IVA y total por concepto.</div></div>
    {!canEdit?<p className={s.panelIntro} style={{marginTop:12}}>Sólo el propietario puede modificar la configuración fiscal.</p>:null}<footer><button className={s.primary} disabled={saving||!canEdit} onClick={()=>onSave({...form,vat_rate:Math.max(0,Number(form.vat_rate)||0),show_breakdown:Boolean(form.enabled)})}>{saving?"Guardando…":"Guardar impuestos"}</button></footer></div>
}