...
export default function SettingsWorkspace({propertyId,property}){
  ...
  const taxes={...DEFAULT_TAXES,...(settings.taxes||{})}
  ...
  async function saveTaxes(nextTaxes){if(!canManageTaxes)return;setSaving("taxes");setError("");setNotice("");try{const mode=nextTaxes.price_tax_mode==="tax_included"?"tax_included":"tax_excluded";await persistSettings({...settings,taxes:{...DEFAULT_TAXES,...nextTaxes,price_tax_mode:mode,vat_rate:Math.max(0,Number(nextTaxes.vat_rate)||0)}},mode==="tax_included"?"Impuestos guardados. Vas a cargar precios finales con IVA incluido; el PMS separará neto e IVA internamente.":"Impuestos guardados. Vas a cargar precios netos y el PMS sumará el IVA automáticamente.")}catch(err){setError(err?.message||"No se pudo guardar la configuración de impuestos.")}finally{setSaving("")}}
...
function TaxSettingsForm({value,onSave,saving,canEdit}){
  ...
  const modeCard=active=>({display:"grid",gap:4,textAlign:"left",padding:"13px 14px",border:`1px solid ${active?"color-mix(in srgb,var(--accent) 48%,var(--line))":"var(--line)"}`,borderRadius:12,background:active?"color-mix(in srgb,var(--accent) 9%,var(--panelSolid))":"var(--panelSolid)",color:"var(--text)",cursor:canEdit&&!saving&&form.enabled?"pointer":"default",opacity:form.enabled?1:.58})
  const mode=form.price_tax_mode==="tax_included"?"tax_included":"tax_excluded",rate=Math.max(0,Number(form.vat_rate)||0)
  return <div className={s.panel}><h2>Impuestos</h2><p className={s.panelIntro}>Define cómo cargás los precios y cómo Habitación Llena separa base imponible, IVA y total en reservas, folios y facturación.</p>
    <div style={toggle}><div><b style={{display:"block",fontSize:13}}>Activar impuestos en la propiedad</b><small style={{display:"block",marginTop:4,color:"var(--muted)",lineHeight:1.45}}>Al activarlo, el PMS mantiene internamente precio neto + IVA + total. Vos elegís abajo si querés escribir precios finales o netos.</small></div><input type="checkbox" checked={Boolean(form.enabled)} disabled={!canEdit||saving} onChange={e=>setForm({...form,enabled:e.target.checked,show_breakdown:e.target.checked})}/></div>
    <div style={{marginBottom:14}}><b style={{display:"block",fontSize:13,marginBottom:5}}>¿Cómo cargás tus precios?</b><p className={s.panelIntro} style={{marginBottom:10}}>Esta regla se usa en Tarifas y disponibilidad y en Servicios y extras. Cambiarla no reescribe reservas históricas ni altera el precio final ya configurado: el PMS normaliza los importes internamente.</p><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:9}}><button type="button" disabled={!canEdit||saving||!form.enabled} style={modeCard(mode==="tax_included")} onClick={()=>setForm({...form,price_tax_mode:"tax_included"})}><b>Precio final · IVA incluido</b><small style={{color:"var(--muted)",lineHeight:1.45}}>Si escribís $ 60.000, el huésped paga $ 60.000. El sistema calcula automáticamente cuánto es neto y cuánto IVA.</small></button><button type="button" disabled={!canEdit||saving||!form.enabled} style={modeCard(mode==="tax_excluded")} onClick={()=>setForm({...form,price_tax_mode:"tax_excluded"})}><b>Precio neto · agregar IVA</b><small style={{color:"var(--muted)",lineHeight:1.45}}>Si escribís $ 60.000 con IVA {rate}%, el huésped paga $ {new Intl.NumberFormat("es-AR",{maximumFractionDigits:2}).format(60000*(1+rate/100))}.</small></button></div></div>
    ...
    <div style={{marginTop:14,padding:"11px 12px",border:"1px solid color-mix(in srgb,var(--accent) 20%,var(--line))",borderRadius:11,background:"color-mix(in srgb,var(--accent) 5%,var(--panelSolid))",fontSize:11,lineHeight:1.5}}><b>{mode==="tax_included"?"Modo recomendado · precio final con IVA incluido":"Modo profesional · precio neto + IVA"}</b><div style={{marginTop:4,color:"var(--muted)"}}>{mode==="tax_included"?"Es la forma más simple para recepción y comercial: cargás exactamente lo que le comunicás al pasajero. Folios y facturación siguen conservando neto, alícuota, IVA y total.":"Ideal si administración trabaja siempre con valores netos. Tarifas y servicios mostrarán además el total final para evitar confusiones."}</div></div>
    {!canEdit?<p className={s.panelIntro} style={{marginTop:12}}>Sólo el propietario puede modificar la configuración fiscal.</p>:null}<footer><button className={s.primary} disabled={saving||!canEdit} onClick={()=>onSave({...form,price_tax_mode:mode,vat_rate:rate,show_breakdown:Boolean(form.enabled)})}>{saving?"Guardando…":"Guardar impuestos"}</button></footer></div>
}
