...
export default function SettingsWorkspace({propertyId,property}){
  ...
  const tabs=[["property","Propiedad"],["rooms","Habitaciones"],["preferences","Preferencias"],["cancellation","Políticas"],["roles","Roles y permisos"]]
  return <section className={s.page}>
    <header className={s.header}><div><small>CONFIGURACIÓN</small><h1>Propiedad y operación</h1><p>Identidad, habitaciones, horarios, políticas y permisos del hotel.</p></div><div className={s.tabs}>{tabs.map(([id,label])=><button key={id} className={tab===id?s.active:""} onClick={()=>chooseTab(id)}>{label}</button>)}</div></header>
