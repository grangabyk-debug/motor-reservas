...
const SETTINGS_TABS=new Set(["property","rooms","preferences","cancellation","roles","subscription"])
...
export default function SettingsWorkspace({propertyId,property}){
  ...
  const tabs=[["property","Propiedad"],["rooms","Habitaciones"],["preferences","Preferencias"],["cancellation","Políticas"],["roles","Roles y permisos"],["subscription","Plan y módulos"]]
  return <section className={s.page}>
    <header className={s.header}><div><small>CONFIGURACIÓN</small><h1>Propiedad y operación</h1><p>Identidad, habitaciones, horarios, políticas, permisos y módulos del hotel.</p></div><div className={s.tabs}>{tabs.map(([id,label])=><button key={id} className={tab===id?s.active:""} onClick={()=>chooseTab(id)}>{label}</button>)}</div></header>
    ...
    {tab==="subscription"&&<SubscriptionPanel propertyId={propertyId} property={property}/>} 
  </section>
}
