...
export default function PropertyRoomsManager({propertyId,property,currency="ARS",taxes={}}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[roomRes,typeRes,floorRes,accessRes]=await Promise.all([
        supabase.from("habitaciones").select("id,nombre,tipo,room_type_id,color,capacidad,precio,activa,online_bookable,estado,floor_id,sort_order,descripcion,housekeeping_zone,bed_configuration").eq("property_id",propertyId).order("sort_order").order("nombre"),
  ...
  function openRoom(room){setMenu(null);setRoomEditor({...EMPTY_ROOM,...room,room_type_id:room.room_type_id||"",color:room.color||"",floor_id:room.floor_id||"",housekeeping_zone:room.housekeeping_zone||"",bed_configuration:room.bed_configuration||"",descripcion:room.descripcion||""})}
