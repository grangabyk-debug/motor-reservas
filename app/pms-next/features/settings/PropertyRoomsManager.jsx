...
export default function PropertyRoomsManager({propertyId,property,currency="ARS"}){
  ...
  const typeById=useMemo(()=>new Map(orderedTypes.map(type=>[type.id,type])),[orderedTypes]),floorById=useMemo(()=>new Map(floors.map(floor=>[floor.id,floor])),[floors])
  const orderedRooms=useMemo(()=>sortRoomsByHotelCategory(rooms,typeById),[rooms,typeById])
  ...
  return <div className={s.shell}><div className={s.subnav}><button className={section==="rooms"?s.subnavActive:""} onClick={()=>{setSection("rooms");setQuery("")}}><span>⌂</span> Habitaciones</button><button className={section==="types"?s.subnavActive:""} onClick={()=>{setSection("types");setQuery("")}}><span>▣</span> Tipos de habitación</button></div><header className={s.hero}><div><small>PROPIEDAD</small><h2>{section==="rooms"?"Habitaciones":"Tipos de habitación"}</h2><p>{section==="rooms"?"Administrá inventario, venta online, ubicación y acceso sin formularios eternos.":"Una definición única para Planning, Tarifas, Channel Manager, Motor y sitio web."}</p></div>{canManage?<button className={s.primary} onClick={section==="rooms"?newRoom:newType}>＋ {section==="rooms"?"Agregar habitación":"Agregar tipo"}</button>:<span className={s.readOnly}>Sólo lectura</span>}</header><div className={s.metrics}>{metrics.map(item=><article key={item.label} data-tone={item.tone}><span>{item.label}</span><b>{item.value}</b></article>)}</div><div className={s.toolbar}><label><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={section==="rooms"?"Buscar habitación, tipo, piso o zona":"Buscar tipo o código"}/></label><span>{canManage?"Orden hotelero automático por categoría":"Vista sin permisos de edición"}</span></div>{error&&<div className={s.error}><b>Revisar</b><span>{error}</span><button onClick={()=>setError("")}>×</button></div>}{notice&&<div className={s.notice}><span>✓</span>{notice}</div>}{loading?<div className={s.empty}>Cargando configuración…</div>:section==="rooms"?<RoomAdminRoomList rooms={filteredRooms} allRooms={orderedRooms} typeById={typeById} floorById={floorById} accessByRoom={accessByRoom} canManage={canManage} saving={saving} menu={menu} openRoom={openRoom} toggleRoom={toggleRoom} duplicateRoom={duplicateRoom} removeRoom={removeRoom} menuButton={menuButton}/>:<RoomAdminTypeList types={filteredTypes} rooms={orderedRooms} currency={currency} canManage={canManage} saving={saving} menu={menu} openType={openType} toggleType={toggleType} removeType={removeType} menuButton={menuButton}/>}{roomEditor&&<RoomAdminRoomDrawer form={roomEditor} setForm={setRoomEditor} types={orderedTypes} floors={floors} access={roomEditor.id?accessByRoom.get(Number(roomEditor.id)):null} canManage={canManage} saving={saving==="room-editor"} currency={currency} onClose={()=>setRoomEditor(null)} onSave={()=>saveRoom(roomEditor)} onDelete={roomEditor.id?()=>removeRoom(roomEditor):null}/>} {typeEditor&&<RoomAdminTypeDrawer form={typeEditor} setForm={setTypeEditor} canManage={canManage} saving={saving==="type-editor"} currency={currency} onClose={()=>setTypeEditor(null)} onSave={()=>saveType(typeEditor)} onDelete={typeEditor.id?()=>removeType(typeEditor):null}/>}</div>
}
