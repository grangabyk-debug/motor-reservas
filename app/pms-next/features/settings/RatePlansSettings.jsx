  ...
  return <div className={s.panel}>
    ...
    <div style={{display:"grid",gap:9}}>
      {form.plans.map(plan=>{const isDefault=plan.code===form.default_code;return <article key={plan.code} style={{padding:"11px 12px",border:"1px solid "+(isDefault?"color-mix(in srgb,var(--accent) 42%,var(--line))":"var(--line)"),borderRadius:11,background:isDefault?"color-mix(in srgb,var(--accent) 5%,var(--panelSolid))":"var(--panelSolid)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"minmax(150px,.7fr) minmax(220px,1.3fr)",gap:8,marginTop:9}}>
          <label style={{display:"grid",gap:5,fontSize:10,fontWeight:800,color:"var(--muted)"}}>Ajuste por persona / noche
            <input type="number" step={currency==="ARS"?"100":"0.5"} disabled={!canEdit||saving||isDefault||!plan.active} value={isDefault?0:plan.adjustment_per_person} onChange={e=>patchPlan(plan.code,{adjustment_per_person:Number(e.target.value)||0})}/>
          ...
          <label style={{display:"grid",gap:5,fontSize:10,fontWeight:800,color:"var(--muted)"}}>Detalle opcional
            <input disabled={!canEdit||saving} value={plan.description||""} onChange={e=>patchPlan(plan.code,{description:e.target.value})} placeholder="Qué incluye este régimen"/>
          </label>
        </div>
      </article>})}
    </div>
