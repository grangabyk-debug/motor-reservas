...
const DEFAULT_ACTIONS={admin:new Set(ALL_ACTIONS.map(item=>item.id)),reception:new Set(["payments.create_request","payments.verify_request"]),night_audit:new Set(["payments.create_request","payments.verify_request"]),housekeeping:new Set(),maintenance:new Set(),revenue:new Set(),member:new Set()}
...
function initialMode(){if(typeof window==="undefined")return"modules";return new URL(window.location.href).searchParams.get("permissions_mode")==="actions"?"actions":"modules"}

export default function RolePermissionsPanel({value,onSave,saving=false,canManage=true}){
  const[matrix,setMatrix]=useState(()=>initialMatrix(value)),[actions,setActions]=useState(()=>initialActions(value)),[activeRole,setActiveRole]=useState("reception"),[mode,setMode]=useState(initialMode)
  ...
  const chooseMode=next=>{setMode(next);if(typeof window!=="undefined"){const url=new URL(window.location.href);if(next==="actions")url.searchParams.set("permissions_mode","actions");else url.searchParams.delete("permissions_mode");window.history.replaceState(window.history.state,"",url)}}
  ...
  return <div className={s.layout}>
    ...
    <section className={s.matrix}><header><div><small>{mode==="modules"?"MÓDULOS":"ACCIONES"}</small><h2>{ROLE_LABELS[activeRole]}</h2><div className={s.modeTabs}><button type="button" className={mode==="modules"?s.selected:""} onClick={()=>chooseMode("modules")}>Módulos</button><button type="button" className={mode==="actions"?s.selected:""} onClick={()=>chooseMode("actions")}>Acciones</button></div></div>{canManage&&<div className={s.actions}>{mode==="modules"?<button type="button" onClick={clearRole}>Mínimo</button>:null}<button type="button" className={s.primary} disabled={saving} onClick={save}>{saving?"Guardando…":"Guardar"}</button></div>}</header>
