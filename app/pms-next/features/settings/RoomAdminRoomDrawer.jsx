...
import{commercialPriceFromNet,netPriceFromCommercial,normalizeTaxSettings}from"../../core/priceTax"
...
export default function RoomAdminRoomDrawer({form,setForm,types,floors,access,canManage,saving,currency,onClose,onSave,onDelete}){
  ...
  const[priceSuggestion,setPriceSuggestion]=useState(null),[taxConfig,setTaxConfig]=useState(()=>normalizeTaxSettings({enabled:false,vat_rate:0,price_tax_mode:"tax_excluded"})),priceTouched=useRef(false)

  useEffect(()=>{
    if(!form.room_type_id){setTaxConfig(normalizeTaxSettings({enabled:false,vat_rate:0,price_tax_mode:"tax_excluded"}));return}
    let cancelled=false
    ;(async()=>{
      const typeRes=await supabase.from("hotel_room_types").select("property_id").eq("id",form.room_type_id).maybeSingle();if(typeRes.error)throw typeRes.error
      if(!typeRes.data?.property_id)return
      const settingsRes=await supabase.from("property_settings").select("settings").eq("property_id",typeRes.data.property_id).maybeSingle();if(settingsRes.error)throw settingsRes.error
      if(!cancelled)setTaxConfig(normalizeTaxSettings(settingsRes.data?.settings?.taxes||{}))
    })().catch(()=>{})
  ...
  const taxIncluded=taxConfig.enabled&&taxConfig.price_tax_mode==="tax_included",displayPrice=commercialPriceFromNet(form.precio,taxConfig)
  const priceLabel=!taxConfig.enabled?`Precio · ${currency}`:taxIncluded?`Precio final · ${currency} · IVA ${taxConfig.vat_rate}% incluido`:`Precio base sin IVA · ${currency}`
  const suggestionCommercial=priceSuggestion?.value>0?commercialPriceFromNet(priceSuggestion.value,taxConfig):0
