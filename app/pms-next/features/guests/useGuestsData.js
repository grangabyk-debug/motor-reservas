...
export default function useGuestsData(propertyId,searchTerm=""){
  ...
  const createGuest=useCallback(async draft=>{
    ...
    if(existing){const patch={};for(const field of["email","phone","birth_date","document_type","document_number","country","nationality","language","address","city","province","notes"]){if(!existing?.[field]&&payload[field])patch[field]=payload[field]}if((String(existing.canonical_key||"").startsWith("manual:")||String(existing.canonical_key||"").startsWith("name:")||String(existing.canonical_key||"").startsWith("guest:"))&&!key.startsWith("manual:"))patch.canonical_key=key;if(!Object.keys(patch).length)return existing;const{data:updated,error:updateError}=await supabase.from("hotel_guest_profiles").update({...patch,updated_at:new Date().toISOString()}).eq("id",existing.id).eq("property_id",propertyId).is("merged_into_id",null).select().single();if(updateError)throw updateError;setProfiles(list=>list.some(item=>item.id===updated.id)?list.map(item=>item.id===updated.id?updated:item):[updated,...list].slice(0,SEARCH_LIMIT));return updated}
