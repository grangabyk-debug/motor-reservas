...
export default function GuestAttentionPanel({propertyId,onOpenGuest}){
  ...
  return <section style={shell} aria-label="Atención de huéspedes">
    ...
    {hasAttention?<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(245px,1fr))",gap:8,padding:11}}>{rows.map(row=><button type="button" key={row.id} disabled={!row.profileId||!onOpenGuest} onClick={()=>row.profileId&&onOpenGuest?.(row.profileId,row.name)} title={row.profileId?"Abrir ficha del huésped":"Este aviso todavía no tiene una ficha de huésped vinculada"} style={{width:"100%",padding:11,border:"1px solid var(--line)",borderRadius:12,background:"color-mix(in srgb,var(--bg) 28%,var(--panelSolid))",color:"var(--text)",font:"inherit",textAlign:"left",cursor:row.profileId&&onOpenGuest?"pointer":"default",transition:"border-color .14s ease,background .14s ease,transform .14s ease"}} onMouseEnter={event=>{if(row.profileId&&onOpenGuest){event.currentTarget.style.borderColor="color-mix(in srgb,var(--accent) 32%,var(--line))";event.currentTarget.style.background="color-mix(in srgb,var(--accent) 5%,var(--panelSolid))"}}} onMouseLeave={event=>{event.currentTarget.style.borderColor="var(--line)";event.currentTarget.style.background="color-mix(in srgb,var(--bg) 28%,var(--panelSolid))"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}><div style={{display:"flex",alignItems:"center",gap:7,color:row.kind==="birthday"?"#9b6b16":row.kind==="vip"?"#7257d9":"var(--accent)"}}><Icon name={row.kind==="birthday"?"occasion":"return"} size={15}/><b style={{fontSize:11,color:"var(--text)"}}>{row.name}</b></div><small style={{fontSize:8.8,fontWeight:900,color:"var(--muted)"}}>{row.kind==="birthday"?(row.days===0?"CUMPLE HOY":`CUMPLE EN ${row.days} DÍAS`):row.kind==="vip"?String(row.vip).toUpperCase():`${row.count} ESTADÍAS`}</small></div><p style={{margin:"6px 0 0",fontSize:9.8,color:"var(--muted)"}}>Hab. {row.room} · {row.reservation.numero_reserva||row.reservation.id}</p><p style={{margin:"7px 0 0",fontSize:10.2,lineHeight:1.45}}>{row.benefit}</p>{row.profileId?<small style={{display:"block",marginTop:7,fontSize:9.2,fontWeight:850,color:"var(--accent)"}}>Abrir ficha del huésped →</small>:null}</button>)}</div>:null}
