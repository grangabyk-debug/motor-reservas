...
import concierge from"./guestConcierge.module.css"
...
export default function GuestsWorkspace({propertyId}){
  ...
  return <section className={s.page}>
    ...
    {selected&&<div className={`${s.drawerShade} ${concierge.shade}`} onMouseDown={e=>e.target===e.currentTarget&&closeProfile()}><aside className={`${s.drawer} ${concierge.card}`}>
      <header className={`${s.profileHead} ${concierge.head}`}><div className={s.profileIdentity}><span className={s.avatar}>{initials(selected.full_name)}</span><div><small>FICHA DE CONCIERGE</small><h2>{selected.full_name}</h2><p>{selected.country||selected.nationality||"Sin país"} · {vipLabel(selected.vip_level)}</p></div></div><button className={s.close} onClick={closeProfile} aria-label="Cerrar ficha"><Icon name="close"/></button></header>
      <div className={`${s.profileMetrics} ${concierge.metrics}`}><div><small>Estadías</small><b>{selected.stays||0}</b></div><div><small>Noches</small><b>{selected.nights||0}</b></div><div><small>Próxima</small><b>{fmtDate(selected.nextStay)}</b></div><div><small>Valor</small><b>{spendLabel(selected.spentByCurrency)}</b></div></div>
      <nav className={`${s.profileTabs} ${concierge.tabs}`}>{[["summary","Resumen"],["contact","Contacto"],["loyalty","Fidelización"]].map(([id,label])=><button type="button" data-tab={id} key={id} className={`${concierge.tab} ${profileTab===id?`${s.profileTabActive} ${concierge.active}`:""}`} onClick={()=>setProfileTab(id)}>{label}</button>)}</nav>
      <div className={`${s.profileBody} ${concierge.body}`}>
