...
export default function SupportWorkspace({propertyId,property}){
  ...
  const load=useCallback(async()=>{
    ...
    const missing=[threadRes.error,feedbackRes.error].find(isMissingSupportSchema)
    if(missing){setUnavailable(true);setLoading(false);return}
    if(threadRes.error||feedbackRes.error){setError("No pudimos abrir Ayuda & feedback. Intentá de nuevo en unos minutos.");setLoading(false);return}
  ...
  const loadMessages=useCallback(async thread=>{
    ...
    if(messageError){if(isMissingSupportSchema(messageError))setUnavailable(true);else setError("No pudimos cargar esta conversación. Intentá nuevamente.");return}
    setMessages(data||[])
  },[propertyId])
  ...
  async function createThread(form){
    ...
    }catch(err){if(isMissingSupportSchema(err)){setNewThread(false);setUnavailable(true)}else setError(safeSupportError(err,"No se pudo abrir la consulta. Intentá nuevamente."))}finally{setSaving(false)}
  }
  async function sendMessage(){
    ...
    }catch(err){if(isMissingSupportSchema(err))setUnavailable(true);else setError(safeSupportError(err,"No se pudo enviar el mensaje. Intentá nuevamente."))}finally{setSaving(false)}
  }
  async function createFeedback(form){
    ...
    catch(err){if(isMissingSupportSchema(err)){setNewIdea(false);setUnavailable(true)}else setError(safeSupportError(err,"No se pudo enviar la idea. Intentá nuevamente."))}finally{setSaving(false)}
  }
