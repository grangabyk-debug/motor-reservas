...
export default async function prepareReservationFromQuote({propertyId,quote,group,quoteLines,checkAvailability,currency,onNavigate}){
  ...
  const[{data:crmLink},current]=await Promise.all([supabase.from("hotel_crm_opportunities").select("id").eq("property_id",propertyId).eq("quote_id",quote.id).maybeSingle(),checkAvailability(group.arrival_date,group.departure_date,{excludeGroupId:group.id})]),picks=picksFromSelection(selectionFromLines(quoteLines),current.types)
