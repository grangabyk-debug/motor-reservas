...
export default function QuotesWorkspace({propertyId,property,onNavigate,allowedViews=[]}){
  const[quotes,setQuotes]=useState([]),[groups,setGroups]=useState({}),[lines,setLines]=useState({}),[currency,setCurrency]=useState("ARS"),[taxes,setTaxes]=useState(()=>normalizeTaxSettings()),[ratePlans,setRatePlans]=useState(()=>normalizeRatePlans({}))
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const settings=setRes.data?.settings||{},rateCurrency=pricingFromSettings(settings).rateCurrency,plans=normalizeRatePlans(settings)
      setQuotes(qs);setGroups(Object.fromEntries((gRes.data||[]).map(g=>[g.id,g])));setLines(quoteLines.reduce((acc,line)=>{(acc[line.quote_id]??=[]).push(line);return acc},{}));setCurrency(rateCurrency);setTaxes(normalizeTaxSettings(settings?.taxes||{}));setRatePlans(plans);setForm(current=>({...current,currency:rateCurrency,ratePlanCode:current.ratePlanCode||plans.default_code}))
  ...
  function openNew(){const next=freshForm(currency,ratePlans.default_code);setForm(next);setNotice("");setError("");setFormOpen(true);refreshAvailability(next)}
