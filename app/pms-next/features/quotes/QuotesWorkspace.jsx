...
const hasIncludedTax=quoteLines=>(quoteLines||[]).some(line=>line.metadata?.tax_included===true)

export default function QuotesWorkspace({propertyId,property,onNavigate,allowedViews=[]}){
  const[quotes,setQuotes]=useState([]),[groups,setGroups]=useState({}),[lines,setLines]=useState({}),[currency,setCurrency]=useState("ARS"),[taxes,setTaxes]=useState(()=>normalizeTaxSettings())
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const settings=setRes.data?.settings||{},rateCurrency=pricingFromSettings(settings).rateCurrency
      setQuotes(qs);setGroups(Object.fromEntries((gRes.data||[]).map(g=>[g.id,g])));setLines(quoteLines.reduce((acc,line)=>{(acc[line.quote_id]??=[]).push(line);return acc},{}));setCurrency(rateCurrency);setTaxes(normalizeTaxSettings(settings?.taxes||{}))
  ...
  async function addToWaitlist(){
    if(!allowedViews.includes("crm"))return setError("El módulo CRM no está habilitado para esta propiedad.")
    if(form.crmOpportunityId){if(typeof window!=="undefined"){const url=new URL(window.location.href);url.searchParams.set("crm_opportunity",form.crmOpportunityId);window.history.replaceState(window.history.state||{},"",url)}setFormOpen(false);onNavigate?.("crm",{restoreScroll:false});return}
    ...
    }catch(err){setError(err?.message||"No se pudo agregar la consulta a Lista de espera.")}
  ...
  const estimate=useMemo(()=>availability.types.reduce((sum,type)=>{const qty=Number(form.selection?.[type.name])||0,rates=(type.finalRateCandidates||[]).slice(0,qty);return sum+rates.reduce((subtotal,rate)=>subtotal+(Number(rate)||0)*nights(form.start,form.end),0)},0),[availability.types,form.selection,form.start,form.end])
  ...
  async function openReservationFromQuote(quote,groupOverride=null,quoteLinesOverride=null){
    ...
    await prepareReservationFromQuote({propertyId,quote,group,quoteLines,checkAvailability,currency,onNavigate})
    setFormOpen(false);setSelected(null)
    emit({title:"Reserva preparada",message:`${quote.quote_number} · completá o revisá los datos y confirmá la reserva.`})
  }
  ...
  async function createQuote(mode="send"){
    ...
    setSaving(true);setError("");setNotice("");let groupId=null,quoteCreated=false
    try{
    ...
    }catch(err){if(groupId&&!quoteCreated)await supabase.from("hotel_groups").delete().eq("id",groupId).eq("property_id",propertyId);setError(quoteCreated?`El presupuesto quedó guardado, pero no pudimos abrir la reserva: ${err?.message||"revisá la disponibilidad."}`:err?.message||"No se pudo procesar el presupuesto.")}
    finally{setSaving(false)}
  }
  ...
  function sendQuoteEmail(quote){const group=groups[quote.group_id];if(!group?.contact_email)return emit({tone:"error",title:"Falta email",message:"Este presupuesto no tiene un email cargado."});const subject=`${property?.name||"Hotel"} · Presupuesto ${quote.quote_number}`;void touchQuoteContact(quote,"email");window.location.href=`mailto:${encodeURIComponent(group.contact_email)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(quoteText(quote))}`}
  function sendQuoteWhatsApp(quote){const group=groups[quote.group_id],phone=String(group?.contact_phone||"").replace(/\D/g,""),text=encodeURIComponent(quoteText(quote)),url=phone?`https://wa.me/${phone}?text=${text}`:`https://api.whatsapp.com/send?text=${text}`;window.open(url,"_blank","noopener,noreferrer");void touchQuoteContact(quote,"whatsapp")}
  function printQuote(quote){
    const group=groups[quote.group_id],quoteLines=lines[quote.id]||[],taxIncluded=hasIncludedTax(quoteLines),win=window.open("","_blank","width=900,height=760");if(!win)return setError("El navegador bloqueó la ventana de impresión.")
    ...
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(quote.quote_number)}</title><style>body{font-family:Arial,sans-serif;color:#17233b;padding:44px;line-height:1.4}header{display:flex;justify-content:space-between;gap:30px;border-bottom:3px solid #4967db;padding-bottom:20px}h1{margin:0;color:#3154c8}small{color:#6b7890}table{width:100%;border-collapse:collapse;margin:30px 0}th,td{text-align:left;padding:11px;border-bottom:1px solid #dfe5f0}th{color:#3154c8}.total{text-align:right;font-size:24px;font-weight:700}.tax{text-align:right;color:#6b7890;font-size:13px}.terms{margin-top:28px;padding:18px;background:#f4f7ff;border-radius:14px}@media print{body{padding:10mm}}</style></head><body><header><div><small>HABITACIÓN LLENA</small><h1>${esc(property?.name||"Hotel")}</h1></div><div><b>${esc(quote.quote_number)}</b><br><small>Válido hasta ${esc(quote.valid_until||"—")}</small></div></header><h2>Presupuesto para ${esc(group?.contact_name||group?.name||"Huésped")}</h2><p>${esc(group?.arrival_date)} → ${esc(group?.departure_date)} · ${esc(group?.estimated_pax||0)} huéspedes</p><table><thead><tr><th>Concepto</th><th>Cantidad</th><th>Precio final / hab.</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table><div class="total">Precio final: ${esc(money(quote.total,quote.currency))}</div>${taxIncluded?`<div class="tax">IVA incluido</div>`:""}${quote.terms?`<div class="terms"><b>Condiciones</b><p>${esc(quote.terms)}</p></div>`:""}<script>window.onload=()=>window.print()<\/script></body></html>`);win.document.close()
  }

  const selectedGroup=selected?groups[selected.group_id]:null,selectedLines=selected?(lines[selected.id]||[]):[],selectedTaxIncluded=hasIncludedTax(selectedLines)
  return <section className={s.page}>
    ...
    <div className={s.infoStrip}><span>◇ El presupuesto no ocupa habitaciones</span><span>✓ La disponibilidad se vuelve a validar al confirmar</span><span>↗ Un clic lo lleva a Crear reserva con los datos cargados</span></div>
    {loading?<div className={s.notice}>Cargando presupuestos…</div>:<div className={s.list}>{quotes.length?quotes.map(quote=>{const group=groups[quote.group_id];return <button type="button" key={quote.id} className={s.quoteRow} onClick={()=>setSelected(quote)}><div><small>{quote.quote_number||"SIN NÚMERO"}</small><b>{group?.contact_name||group?.name||"Sin contacto"}</b>{quote.follow_up_status==="pending"&&quote.follow_up_at?<small style={{display:"block",marginTop:3,color:new Date(quote.follow_up_at)<new Date()?"#c24850":"var(--accent)",fontWeight:850}}>Seguimiento · {followWhen(quote.follow_up_at)} · {FOLLOW_CHANNEL[quote.follow_up_channel]||"Contacto"}</small>:null}</div><span>{group?.arrival_date||"—"} → {group?.departure_date||"—"}</span><span>{group?.room_block||0} hab. · {group?.estimated_pax||0} pax</span><strong>{money(quote.total,quote.currency)}</strong><em data-status={quote.status}>{STATUS[quote.status]||quote.status}</em><i>›</i></button>}):<div className={s.empty}><b>Todavía no hay presupuestos</b><span>Creá uno desde una llamada o consulta sin alterar la disponibilidad del Planning.</span></div>}</div>}
    {formOpen&&<div className={s.backdrop} onMouseDown={e=>e.target===e.currentTarget&&setFormOpen(false)}><div className={s.modal}><button className={s.close} onClick={()=>setFormOpen(false)} aria-label="Cerrar">×</button><small>NUEVO PRESUPUESTO</small><h2>Buscar disponibilidad y cotizar</h2><div className={s.formGrid}><label>Nombre / grupo<input autoFocus value={form.name} onChange={e=>patch({name:e.target.value})} placeholder="Familia Fernández / Empresa"/></label><label>Huéspedes<input type="number" min="1" value={form.pax} onChange={e=>patch({pax:e.target.value})}/></label><label>Email<input type="email" value={form.email} onChange={e=>patch({email:e.target.value})}/></label><label>Teléfono<input value={form.phone} onChange={e=>patch({phone:e.target.value})}/></label><label>Entrada<input type="date" value={form.start} onChange={e=>changeDates({start:e.target.value})}/></label><label>Salida<input type="date" min={addDays(form.start,1)} value={form.end} onChange={e=>changeDates({end:e.target.value})}/></label><label>Moneda de tarifas<input value={form.currency} readOnly/></label><label>Válido hasta<input type="date" value={form.validUntil} onChange={e=>patch({validUntil:e.target.value})}/></label></div><div className={s.availabilityHead}><div><h3>Habitaciones disponibles</h3><p>{checking?"Comprobando Planning…":`${availability.types.reduce((sum,t)=>sum+t.available,0)} disponibles para ${nights(form.start,form.end)} noche${nights(form.start,form.end)===1?"":"s"}`}</p></div><button type="button" onClick={()=>refreshAvailability()}>Actualizar</button></div><div className={s.typeList}>{availability.types.map(type=><article key={type.name}><div><b>{type.name}</b><small>{type.available} disponibles · hasta {type.capacity} pax · desde {money(type.baseConfiguredPrice,form.currency)}/noche · {taxCaption}{taxes.enabled&&taxes.price_tax_mode==="tax_excluded"?` · final ${money(type.baseFinalPrice,form.currency)}`:""}</small></div><div className={s.stepper}><button type="button" onClick={()=>setQty(type.name,(form.selection[type.name]||0)-1)}>−</button><input type="number" min="0" max={type.available} value={form.selection[type.name]||0} onChange={e=>setQty(type.name,e.target.value)}/><button type="button" onClick={()=>setQty(type.name,(form.selection[type.name]||0)+1)}>＋</button></div></article>)}{!checking&&!availability.types.length&&<div className={s.noAvailability}><b>No hay inventario disponible para esas fechas.</b>{allowedViews.includes("crm")?<button type="button" className={s.primary} disabled={saving} onClick={addToWaitlist}>{saving?"Guardando…":form.crmOpportunityId?"Volver a lista de espera":"Agregar a lista de espera"}</button>:null}</div>}</div><div className={s.quoteSummary}><div><span>Habitaciones</span><b>{selectedRooms}</b></div><div><span>Capacidad seleccionada</span><b>{selectedCapacity} pax</b></div><div><span>Precio final{taxes.enabled?taxes.price_tax_mode==="tax_included"?" · IVA incluido":" · IVA agregado":""}</span><b>{money(estimate,form.currency)}</b></div></div><label className={s.fullLabel}>Condiciones<textarea rows="3" value={form.terms} onChange={e=>patch({terms:e.target.value})}/></label><label className={s.fullLabel}>Notas internas<textarea rows="2" value={form.notes} onChange={e=>patch({notes:e.target.value})}/></label>{selectedCapacity>0&&Number(form.pax)>selectedCapacity&&<div className={s.warning}>La capacidad seleccionada es menor que la cantidad de huéspedes.</div>}<footer><button onClick={()=>setFormOpen(false)}>Cancelar</button><button disabled={saving||checking||selectedRooms<1} onClick={()=>createQuote("send")}>{saving?"Procesando…":"Preparar para enviar"}</button><button className={s.primary} disabled={saving||checking||selectedRooms<1} onClick={()=>createQuote("reservation")}>{saving?"Preparando…":"Crear reserva"}</button></footer></div></div>}
    {selected&&<div className={s.backdrop} onMouseDown={e=>e.target===e.currentTarget&&setSelected(null)}><div className={`${s.modal} ${s.detailModal}`}><button className={s.close} onClick={()=>setSelected(null)}>×</button><small>{selected.quote_number}</small><h2>{selectedGroup?.contact_name||selectedGroup?.name}</h2><div className={s.detailMeta}><span><small>Estadía</small><b>{selectedGroup?.arrival_date} → {selectedGroup?.departure_date}</b></span><span><small>Huéspedes</small><b>{selectedGroup?.estimated_pax||0}</b></span><span><small>Estado</small><b>{STATUS[selected.status]||selected.status}</b></span><span><small>{selectedTaxIncluded?"Precio final · IVA incluido":"Total"}</small><b>{money(selected.total,selected.currency)}</b></span></div><div className={s.detailLines}>{selectedLines.map(line=><div key={line.id}><span><b>{line.description}</b><small>{line.quantity} × {money(line.unit_price,selected.currency)}{line.metadata?.tax_included?" · IVA incluido":""}</small></span><strong>{money(line.total,selected.currency)}</strong></div>)}</div>{selected.terms&&<div className={s.terms}><b>Condiciones</b><p>{selected.terms}</p></div>}<QuoteFollowUpPanel quote={selected} propertyId={propertyId} onUpdated={patch=>{setSelected(current=>current?{...current,...patch}:current);setQuotes(rows=>rows.map(row=>row.id===selected.id?{...row,...patch}:row))}}/><footer className={s.detailActions}><button onClick={()=>sendQuoteEmail(selected)}>✉ Email</button><button onClick={()=>sendQuoteWhatsApp(selected)}>WhatsApp</button><button onClick={()=>printQuote(selected)}>Imprimir / PDF</button>{!["accepted","rejected"].includes(selected.status)&&<button className={s.primary} disabled={saving} onClick={()=>convertToReservation(selected)}>{saving?"Preparando…":"Crear reserva"}</button>}{!["accepted","rejected"].includes(selected.status)&&<button className={s.reject} disabled={saving} onClick={()=>markQuote(selected,"rejected")}>Cancelar presupuesto</button>}</footer></div></div>}
  </section>
}