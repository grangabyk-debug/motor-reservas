...
export default function QuotesWorkspace({propertyId,property,onNavigate}){
  ...
  const taxCaption=taxes.enabled&&taxes.vat_rate?`IVA ${taxes.vat_rate}% incluido`:"Precio final"
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      setQuotes(qs);setGroups(Object.fromEntries((gRes.data||[]).map(g=>[g.id,g])));setLines(quoteLines.reduce((acc,line)=>{(acc[line.quote_id]??=[]).push(line);return acc},{}));setCurrency(settings?.preferences?.currency||"ARS");setTaxes(normalizeTaxSettings(settings?.taxes||{}))
  ...
  const estimate=useMemo(()=>availability.types.reduce((sum,type)=>sum+(Number(form.selection?.[type.name])||0)*finalPriceFromNet(type.basePrice,taxes)*nights(form.start,form.end),0),[availability.types,form.selection,form.start,form.end,taxes])
  ...
  async function createQuote(mode="send"){
    ...
    try{
      ...
      const stayNights=nights(form.start,form.end),quoteLines=latest.types.flatMap(type=>{const qty=Number(form.selection[type.name])||0;if(!qty)return[];const nightlyNet=Number(type.basePrice)||0,nightlyFinal=finalPriceFromNet(nightlyNet,taxes);return[{category:"room",description:type.name,quantity:qty,unit_price:nightlyFinal*stayNights,sort_order:0,metadata:{room_type:type.name,nights:stayNights,nightly_rate:nightlyNet,nightly_rate_final:nightlyFinal,vat_rate:taxes.enabled?taxes.vat_rate:0,tax_included:Boolean(taxes.enabled&&taxes.vat_rate),quoted_available:type.available}}]}),quoteTotal=quoteLines.reduce((sum,line)=>sum+(Number(line.quantity)||0)*(Number(line.unit_price)||0),0)
