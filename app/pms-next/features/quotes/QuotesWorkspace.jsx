...
export default function QuotesWorkspace({propertyId,property,onNavigate}){
  ...
  async function createReservationFromQuote(quote,groupOverride=null,quoteLinesOverride=null,latestOverride=null){
    const group=groupOverride||groups[quote.group_id],quoteLines=quoteLinesOverride||(lines[quote.id]||[]).filter(line=>line.category==="room")
    if(!group?.arrival_date||!group?.departure_date||!quoteLines.length)throw new Error("Este presupuesto no tiene estadía o habitaciones convertibles.")
    ...
    const payload={property_id:propertyId,user_id:userData?.user?.id||null,habitacion_id:roomIds[0],habitaciones_ids:roomIds,habitaciones_detalle:details,fecha_entrada:group.arrival_date,fecha_salida:group.departure_date,tipo_estadia:"overnight",nombre_huesped:group.contact_name||group.name,email_huesped:group.contact_email||null,telefono_huesped:group.contact_phone||null,cantidad_huespedes:Math.max(1,Number(group.estimated_pax)||1),canal_reserva:"Directa · Presupuesto",codigo_canal:quote.quote_number||null,tarifa_noche:rate,noches:stayNights,precio_total:Number(quote.total)||rate*stayNights,moneda:quote.currency||currency,notas:`Convertida desde ${quote.quote_number}${group.notes?` · ${group.notes}`:""}`,group_id:group.id,mascotas:[],mascotas_total:0,servicios:[],pasajeros:[],vehiculos:0,cochera_total:0,estado:"confirmada",no_show:false}
    ...
    const{error:acceptError}=await supabase.rpc("hl_group_mark_quote_atomic",{p_property_id:propertyId,p_quote_id:quote.id,p_status:"accepted"});if(acceptError)emit({tone:"warning",title:"Reserva creada",message:"La reserva se creó, pero el presupuesto no pudo marcarse como aceptado automáticamente."})
  ...
  async function createQuote(mode="send"){
    ...
    try{
      ...
      if(mode==="reservation")return await createReservationFromQuote(quote,groupRecord,quoteLines,latest)
    ...
    }catch(err){if(groupId){const{data:reservation}=await supabase.from("reservas").select("id").eq("property_id",propertyId).eq("group_id",groupId).limit(1).maybeSingle();if(!reservation)await supabase.from("hotel_groups").delete().eq("id",groupId).eq("property_id",propertyId)}setError(err?.message||"No se pudo procesar el presupuesto.")}
