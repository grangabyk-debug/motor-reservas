...
export function buildQuoteText({quote,group,quoteLines,propertyName}){
  const hotel=propertyName||"Hotel",guest=group?.contact_name||group?.name||"huésped",roomLines=(quoteLines||[]).filter(line=>line.category==="room"),planName=roomLines.find(line=>line.metadata?.rate_plan_name)?.metadata?.rate_plan_name||"",taxIncluded=roomLines.some(line=>line.metadata?.price_tax_mode==="tax_included"&&Number(line.metadata?.vat_rate)>0),taxExcluded=roomLines.some(line=>line.metadata?.price_tax_mode==="tax_excluded"&&Number(line.metadata?.vat_rate)>0),concepts=roomLines.map(line=>`${Number(line.quantity)||1} × ${line.description}: ${money(line.total||line.unit_price,quote.currency)}`).join("\n"),taxNote=taxIncluded?" · IVA incluido":taxExcluded?" · total con IVA agregado":""
  return`Hola ${guest},\n\nTe enviamos el presupuesto para tu estadía en ${hotel}.\n\nPresupuesto: ${quote.quote_number}\nEntrada: ${group?.arrival_date||"—"}\nSalida: ${group?.departure_date||"—"}\n${concepts?`Habitación: ${concepts}\n`:""}${planName?`Plan tarifario: ${planName}\n`:""}Pasajeros: ${group?.estimated_pax||0}\nTotal del presupuesto: ${money(quote.total,quote.currency)}${taxNote}\nVálido hasta: ${quote.valid_until||"—"}\n\n${quote.terms||"Tarifas sujetas a disponibilidad al momento de confirmar."}\n\nSi necesitás hacer algún cambio o agregar una solicitud, respondé este correo.\n\nSaludos,\n${hotel}`
}