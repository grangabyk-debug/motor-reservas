import{activeRatePlans,configuredAmountToFinal,defaultRatePlan,ratePlanBasis,ratePlanByCode}from"../../core/ratePlans"
...
export function estimateQuoteWithPlan({availability,selection,start,end,pax,selectedPlan,taxes}){
  ...
  const units=ratePlanBasis(selectedPlan)==="per_room"?roomCount:Math.max(1,Number(pax)||1),adjustment=configuredAmountToFinal(Number(selectedPlan?.adjustment_per_person)||0,taxes)*units*stayNights
