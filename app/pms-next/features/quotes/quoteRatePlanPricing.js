import{activeRatePlans,configuredAmountToFinal,defaultRatePlan,ratePlanByCode}from"../../core/ratePlans"
...
export function estimateQuoteWithPlan({availability,selection,start,end,pax,selectedPlan,taxes}){
  const stayNights=nights(start,end)
  ...
  const adjustment=configuredAmountToFinal(Number(selectedPlan?.adjustment_per_person)||0,taxes)*Math.max(1,Number(pax)||1)*stayNights
