...
import{nightlyPricingByRoomDate,resolvePricingQuote}from"../../core/pricingContract"
...
export default function RatesWorkspace({propertyId,property}){
  ...
  const[rooms,setRooms]=useState([]),[rates,setRates]=useState([]),[resolvedPricing,setResolvedPricing]=useState(null),[propertySettings,setPropertySettings]=useState({})
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const pricingEnd=iso(addDays(new Date(`${dataEnd}T12:00:00`),1)),resolved=await resolvePricingQuote({propertyId,start:dataStart,end:pricingEnd,roomIds:(roomsRes.data||[]).map(room=>room.id)})
      setResolvedPricing(resolved)
  ...
  const rateMap=useMemo(()=>{const map=new Map();for(const row of rates)map.set(`${row.habitacion_id}:${row.stay_date}`,row);return map},[rates]),resolvedRateMap=useMemo(()=>nightlyPricingByRoomDate(resolvedPricing),[resolvedPricing]),selectedDates=useMemo(()=>Array.from(selected).sort(),[selected])
  ...
  async function switchCurrency(target){if(!isOwner||target===pricing.rateCurrency)return;if(!effectiveFx)return setError("Falta una cotización USD/ARS válida.");const message=`Convertir tarifas y calendario de ${pricing.rateCurrency} a ${target} con 1 USD = ${formatCurrency(effectiveFx,"ARS")}. Las reservas existentes no cambian. ¿Continuar?`;if(typeof window!=="undefined"&&!window.confirm(message))return;setSaving("currency");setError("");try{const factor=pricing.rateCurrency==="ARS"&&target==="USD"?1/effectiveFx:effectiveFx,convertedPlans=convertRatePlans(normalizeRatePlans(propertySettings),factor),{data,error:rpcError}=await supabase.rpc("hl_change_rate_currency_with_plans",{p_property_id:propertyId,p_target_currency:target,p_usd_ars_rate:effectiveFx,p_rate_plans:convertedPlans,p_fx_source:pricing.fxMode==="manual"?"Manual":fx?.source||"Banco Nación · dólar billete venta",p_fx_as_of:pricing.fxMode==="manual"?iso(new Date()):fx?.asOf||iso(new Date())});if(rpcError)throw rpcError;const next={...propertySettings,pricing:{...(propertySettings.pricing||{}),...(data||{})},rate_plans:convertedPlans};setPropertySettings(next);await load();if(typeof window!=="undefined"){window.dispatchEvent(new CustomEvent("hl:property-settings-updated",{detail:{propertyId,settings:next}}));window.dispatchEvent(new CustomEvent("hl:pms-toast",{detail:{title:`Tarifas en ${target}`,message:`Cambio guardado con 1 USD = ${formatCurrency(effectiveFx,"ARS")}. Los ajustes por régimen se convirtieron junto con el calendario.`}}))}}catch(err){setError(err?.message||"No se pudo cambiar la moneda base.")}finally{setSaving("")}}
  ...
  async function mutateRooms(targetRooms,{percent=null,open=null,minStay=null,commercialPrice=null}={},key="bulk"){
    ...
    try{
      const{error:rateError}=await supabase.rpc("hl_set_rate_calendar_pricing",{p_property_id:propertyId,p_room_ids:targetRooms.map(room=>Number(room.id)),p_dates:selectedDates,p_configured_amount:commercialPrice!=null?Number(commercialPrice):null,p_percent:percent!=null?Number(percent):null,p_min_stay:minStay!=null?Math.max(1,Number(minStay)||1):null,p_open:open==null?null:Boolean(open),p_round_up:roundUp});if(rateError)throw rateError
      await load()
  ...
  return <section className={s.page}>
    ...
    <div className={s.layout}><aside className={s.selector}>
      ...
      <div className={s.selection}><b>{selectedDates.length}</b><span>fecha{selectedDates.length===1?"":"s"} seleccionada{selectedDates.length===1?"":"s"}</span>{selectedDates.length>0&&<small>{dateLabel(selectedDates[0])}{selectedDates.length>1?` → ${dateLabel(selectedDates.at(-1))}`:""}</small>}<small style={{marginTop:3,lineHeight:1.35}}>Si falta una tarifa resoluble, no se publica un precio hasta configurarla.</small></div>
      ...
      <div style={taxCard}><small style={{display:"block",fontSize:10,fontWeight:900,letterSpacing:".08em",color:"#278452"}}>CONTRATO DE PRECIOS</small><b style={{display:"block",marginTop:5,fontSize:12}}>{priceTaxCaption(taxes)}</b><p style={{margin:"5px 0 0",fontSize:10,lineHeight:1.45,color:"var(--muted)"}}>{!taxes.enabled?`Importes sin IVA en ${pricing.rateCurrency}.`:taxes.price_tax_mode==="tax_included"?`Ingresás el precio final en ${pricing.rateCurrency}; el PMS separa IVA internamente.`:`Ingresás la tarifa neta en ${pricing.rateCurrency}; el PMS agrega IVA al total final.`}</p></div>
