...
export default function RatesWorkspace({propertyId,property}){
  ...
  const bulkBusy=saving==="bulk-price"||saving==="bulk-percent",monthLabel=new Intl.DateTimeFormat("es-AR",{month:"long",year:"numeric"}).format(anchor),bulkPercent=Number(String(bulkValue).replace(",",".")),bulkPercentText=String(bulkValue).trim()&&Number.isFinite(bulkPercent)?`${bulkPercent>0?"+":""}${bulkPercent}%`:"ajuste %"
  ...
  return <section className={s.page}>
    ...
    <div className={s.layout}><aside className={s.selector}>
      ...
      <div className={s.selection}><b>{selectedDates.length}</b><span>fecha{selectedDates.length===1?"":"s"} seleccionada{selectedDates.length===1?"":"s"}</span>{selectedDates.length>0&&<small>{dateLabel(selectedDates[0])}{selectedDates.length>1?` → ${dateLabel(selectedDates.at(-1))}`:""}</small>}<small style={{marginTop:3,lineHeight:1.35}}>Los días sin tarifa específica usan la tarifa base de la habitación. Cambiar una fecha no modifica las demás.</small></div>
