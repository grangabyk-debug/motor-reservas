...
export default function RatesWorkspace({propertyId,property}){
  ...
  const[anchor,setAnchor]=useState(today),[selected,setSelected]=useState(new Set([iso(today)])),[rangeStart,setRangeStart]=useState(null),[rangeMode,setRangeMode]=useState(true)
  ...
  const visibleDates=useMemo(()=>Array.from({length:35},(_,i)=>iso(addDays(anchor,i))),[anchor]),rangeEnd=visibleDates.at(-1)
  ...
  const selectionLabel=explicitlySelected.length?`${explicitlySelected.length} seleccionada${explicitlySelected.length===1?"":"s"}`:typeFilter!=="all"?`${visibleRooms.length} ${typeFilter}`:"Seleccioná habitaciones o una categoría"

  function chooseDate(day){if(!rangeMode){setSelected(current=>{const next=new Set(current);next.has(day)?next.delete(day):next.add(day);return next});return}if(!rangeStart){setRangeStart(day);setSelected(new Set([day]));return}const a=new Date(`${rangeStart}T12:00:00`),b=new Date(`${day}T12:00:00`),from=a<=b?a:b,to=a<=b?b:a,next=new Set();for(let d=new Date(from);d<=to;d=addDays(d,1))next.add(iso(d));setSelected(next);setRangeStart(null)}
  ...
  function toggleWeekday(index){setSelected(current=>{const next=new Set(current),matching=visibleDates.filter(day=>new Date(`${day}T12:00:00`).getDay()===index),all=matching.every(day=>next.has(day));matching.forEach(day=>all?next.delete(day):next.add(day));return next})}
  ...
  async function switchCurrency(target){if(!isOwner||target===pricing.rateCurrency)return;if(!effectiveFx)return setError("Necesitamos una cotización USD/ARS válida antes de cambiar la moneda base.");const message=`Se convertirán todas las tarifas maestras y el calendario de ${pricing.rateCurrency} a ${target} usando 1 USD = ${formatCurrency(effectiveFx,"ARS")}. Las reservas ya creadas no cambian. ¿Continuar?`;if(typeof window!=="undefined"&&!window.confirm(message))return;setSaving("currency");setError("");try{const{data,error:rpcError}=await supabase.rpc("hl_change_rate_currency",{p_property_id:propertyId,p_target_currency:target,p_usd_ars_rate:effectiveFx,p_fx_source:pricing.fxMode==="manual"?"Manual":fx?.source||"Banco Nación · dólar billete venta",p_fx_as_of:pricing.fxMode==="manual"?iso(new Date()):fx?.asOf||iso(new Date())});if(rpcError)throw rpcError;const next={...propertySettings,pricing:{...(propertySettings.pricing||{}),...(data||{})}};setPropertySettings(next);await load();if(typeof window!=="undefined"){window.dispatchEvent(new CustomEvent("hl:property-settings-updated",{detail:{propertyId,settings:next}}));window.dispatchEvent(new CustomEvent("hl:pms-toast",{detail:{title:`Tarifas en ${target}`,message:`Moneda base cambiada usando 1 USD = ${formatCurrency(effectiveFx,"ARS")}.`}}))}}catch(err){setError(err?.message||"No se pudo cambiar la moneda base.")}finally{setSaving("")}}
  ...
  async function mutateRooms(targetRooms,{percent=null,open=null,minStay=null,commercialPrice=null}={},key="bulk"){
    ...
    try{
      ...
      await load()
      if(typeof window!=="undefined"&&targetRooms.length>1)window.dispatchEvent(new CustomEvent("hl:pms-toast",{detail:{title:"Tarifas actualizadas",message:`Se aplicó el cambio a ${targetRooms.length} habitaciones y ${selectedDates.length} fecha${selectedDates.length===1?"":"s"}.`}}))
  ...
  function applyBulkChange(){
    ...
    if(!bulkTargets.length)return setError("Elegí una categoría o seleccioná habitaciones.")
    if(bulkMode==="percent"){
  ...
  const bulkBusy=saving==="bulk-price"||saving==="bulk-percent"

  return <section className={s.page}>
    <header className={s.header}><div><small>REVENUE</small><h1>Tarifas y disponibilidad</h1><p>{property?.name||"Propiedad activa"} · precios, restricciones y venta por fecha.</p></div><div className={s.headerActions}><button className={s.open} onClick={()=>bulkOpen(true)} disabled={!selectedDates.length||!!saving}>Abrir venta</button><button className={s.closeSale} onClick={()=>bulkOpen(false)} disabled={!selectedDates.length||!!saving}>Cerrar venta</button></div></header>
    ...
    <div className={s.layout}><aside className={s.selector}>
      ...
      <div className={s.weekdays}>{weekdays.map(index=><button key={index} onClick={()=>toggleWeekday(index)}>{["D","L","M","M","J","V","S"][index]}</button>)}</div><div className={s.calendar}>{visibleDates.map(day=>{const d=new Date(`${day}T12:00:00`);return <button key={day} className={`${selected.has(day)?s.daySelected:""} ${day===iso(today)?s.today:""}`} onClick={()=>chooseDate(day)}><small>{new Intl.DateTimeFormat("es-AR",{weekday:"short"}).format(d).slice(0,2)}</small><b>{d.getDate()}</b></button>})}</div>
      <label className={s.mode}><input type="checkbox" checked={rangeMode} onChange={e=>{setRangeMode(e.target.checked);setRangeStart(null)}}/><span>Selección por rango</span></label><div className={s.selection}><b>{selectedDates.length}</b><span>fecha{selectedDates.length===1?"":"s"} seleccionada{selectedDates.length===1?"":"s"}</span>{selectedDates.length>0&&<small>{dateLabel(selectedDates[0])}{selectedDates.length>1?` → ${dateLabel(selectedDates.at(-1))}`:""}</small>}</div>
      <div style={card}><small style={{display:"block",fontSize:10,fontWeight:900,letterSpacing:".08em",color:"var(--accent)"}}>MONEDA DE TARIFAS</small><b style={{display:"block",marginTop:5,fontSize:13}}>Base {pricing.rateCurrency}</b><p style={{margin:"5px 0 9px",fontSize:10,lineHeight:1.45,color:"var(--muted)"}}>La moneda base queda fija. La otra moneda se muestra como equivalencia con la cotización vigente.</p><div style={{display:"flex",gap:6}}>{["ARS","USD"].map(code=><button key={code} type="button" style={{...tinyButton,flex:1,borderColor:pricing.rateCurrency===code?"color-mix(in srgb,var(--accent) 55%,var(--line))":"var(--line)",background:pricing.rateCurrency===code?"color-mix(in srgb,var(--accent) 10%,var(--panelSolid))":"var(--panelSolid)",color:pricing.rateCurrency===code?"var(--accent)":"var(--text)"}} disabled={!isOwner||saving==="currency"} onClick={()=>switchCurrency(code)}>{code}</button>)}</div></div>
      ...
      <div style={taxCard}><small style={{display:"block",fontSize:10,fontWeight:900,letterSpacing:".08em",color:"#278452"}}>PRECIOS E IVA</small><b style={{display:"block",marginTop:5,fontSize:12}}>{priceTaxCaption(taxes)}</b><p style={{margin:"5px 0 0",fontSize:10,lineHeight:1.45,color:"var(--muted)"}}>Cargá el precio final en {pricing.rateCurrency}. El PMS separa neto e IVA internamente.</p></div>
