...
export default function CrmWorkspace({propertyId,onNavigate}){
  ...
  async function changeStage(stage){
    ...
    setSaving(true);data.setError("")
    try{await data.updateOpportunity(selected.id,{stage,waitlist_until:stage==="waitlist"?(selected.waitlist_until||selected.desired_check_in):null},`Estado comercial: ${STAGES[stage]||stage}.`);toast({title:"Estado actualizado",message:STAGES[stage]||stage})}
  ...
  return <section className={s.page}>
    ...
    <div className={s.workspace}>
      ...
      <section className={s.detail}>{selected?<><div className={s.detailTop}><div><small className={s.eyebrow}>{STAGES[selected.stage]||selected.stage}</small><h2>{selected.name}</h2><p>{[selected.email,selected.phone,CHANNELS[selected.source_channel]].filter(Boolean).join(" · ")}</p></div><div className={s.actions}>{selected.guest_profile_id?<button type="button" className={s.soft} onClick={()=>{if(typeof window!=="undefined"){const url=new URL(window.location.href);url.searchParams.set("guest_profile",selected.guest_profile_id);window.history.replaceState(window.history.state||{},"",url)}onNavigate?.("guests",{restoreScroll:false})}}>Abrir huésped</button>:null}<button type="button" className={s.soft} onClick={()=>{setEditTarget(selected);setFormOpen(true)}}>Editar</button>{selected.email?<button type="button" className={s.soft} onClick={()=>contact("email")}>Email</button>:null}{selected.phone?<button type="button" className={s.soft} onClick={()=>contact("whatsapp")}>WhatsApp</button>:null}{selected.phone?<button type="button" className={s.soft} onClick={()=>contact("phone")}>Llamar</button>:null}</div></div>
