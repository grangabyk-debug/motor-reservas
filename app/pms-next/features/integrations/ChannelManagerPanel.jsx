...
const OTAS=[
  {id:"booking",name:"Booking.com",mark:"B",aliases:["booking","booking.com"],color:"#1769d2",surface:"#edf5ff"},
  {id:"airbnb",name:"Airbnb",mark:"A",aliases:["airbnb"],color:"#e75b67",surface:"#fff0f2"},
  {id:"expedia",name:"Expedia",mark:"E",aliases:["expedia"],color:"#d29a21",surface:"#fff7df"},
  {id:"despegar",name:"Despegar",mark:"D",aliases:["despegar","decolar"],color:"#6858d9",surface:"#f1efff"},
  {id:"agoda",name:"Agoda",mark:"Ag",aliases:["agoda"],color:"#8b5bc9",surface:"#f6efff"},
  {id:"hotelbeds",name:"Hotelbeds",mark:"H",aliases:["hotelbeds"],color:"#2f9a69",surface:"#eaf9f1"},
]
...
export default function ChannelManagerPanel({propertyId,property,connections=[],mappings=[],onReload,onMessage}){
  ...
  return <div className={s.wrap}>
    ...
    <div className={s.cards}>{cards.map(card=>{const connection=card.connection,menuOpen=menu===card.id;return <article className={s.card} key={card.id} data-provider={card.id}><div className={s.cardTop}><div className={s.brand}><span className={s.mark} style={{color:card.color,background:`linear-gradient(145deg,${card.surface},color-mix(in srgb,${card.surface} 58%,var(--panelSolid)))`,borderColor:`color-mix(in srgb,${card.color} 22%,var(--line))`,boxShadow:`0 8px 20px color-mix(in srgb,${card.color} 10%,transparent),inset 0 1px rgba(255,255,255,.55)`}}>{card.mark}</span><div><small>CANAL DE VENTA</small><h3>{card.name}</h3></div></div><span className={s.state} data-state={card.status}>{STATUS[card.status]||card.status}</span></div><div className={s.info}><div><small>Última sync</small><b>{fmt(connection?.last_sync_at)}</b></div><div><small>Mapeos</small><b>{card.mappingCount}</b></div><div><small>Ajuste tarifa</small><b>{Number(connection?.diagnostics?.rate_adjustment||0)>0?"+":""}{Number(connection?.diagnostics?.rate_adjustment||0)}%</b></div></div>{connection?.last_error?<div className={s.errorBox}>{connection.last_error}</div>:null}<div className={s.actions}><button data-main={Boolean(connection)} onClick={()=>openEditor(card)}>{connection?"Administrar":"Preparar conexión"}</button>{connection?<div className={s.moreWrap} onClick={e=>e.stopPropagation()}><button className={s.more} onClick={e=>{e.stopPropagation();setMenu(menuOpen?null:card.id)}}>⋯</button>{menuOpen?<div className={s.menu}><button onClick={()=>openEditor(card)}>Editar configuración</button><button onClick={()=>setStatus(card,card.status==="paused"?"sandbox":"paused")}>{card.status==="paused"?"Reanudar configuración":"Pausar"}</button>{canDelete?<button className={s.danger} onClick={()=>remove(card)}>Desconectar</button>:null}</div>:null}</div>:null}</div></article>})}</div>
