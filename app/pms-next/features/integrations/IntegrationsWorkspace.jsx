...
const OTAS = [
  { id: "booking", name: "Booking.com", aliases: ["booking", "booking.com"] },
  { id: "expedia", name: "Expedia", aliases: ["expedia"] },
  { id: "airbnb", name: "Airbnb", aliases: ["airbnb"] },
  { id: "despegar", name: "Despegar", aliases: ["despegar", "decolar"] },
  { id: "agoda", name: "Agoda", aliases: ["agoda"] },
  { id: "hotelbeds", name: "Hotelbeds", aliases: ["hotelbeds"] },
]
...
const INTEGRATION_TABS = new Set(["engine","channels","rooms","history"])
function readTab(){if(typeof window==="undefined")return"engine";const value=new URL(window.location.href).searchParams.get("integration_tab");return INTEGRATION_TABS.has(value)?value:"engine"}
...
export default function IntegrationsWorkspace({ propertyId, property }) {
  ...
  async function copy(value, label) {
    ...
    catch { setError("No se pudo copiar automáticamente.") }
  }
  ...
  async function createEngine() {
    ...
    try {
      const name = property?.name || "Mi hotel", base = slugify(name)
      ...
      if (insertError?.code === "23505") { payload = { ...payload, slug: `${base}-${String(propertyId).slice(0, 6)}` }; const retry = await supabase.from("hotel_booking_engines").insert(payload).select(ENGINE_FIELDS).single(); data = retry.data; insertError = retry.error }
      if (insertError) throw insertError
      setEngine(data);setDraft({ ...data });setNotice("Motor creado y conectado al inventario real del hotel.")
    } catch (err) { setError(err?.message || "No se pudo crear el motor.") }
  ...
  async function saveEngine() {
    ...
    try {
      ...
      setEngine(data);setDraft({ ...data });setNotice("Configuración del motor guardada.")
    } catch (err) { setError(err?.message || "No se pudo guardar el motor.") }
    finally { setSaving(false) }
  }
  ...
  async function uploadHero(file) {
    ...
    try {
      ...
      const ext=(file.name.split(".").pop()||"jpg").replace(/[^a-z0-9]/gi,""),path=`${propertyId}/booking/hero-${Date.now()}.${ext}`
      const { data, error: upError } = await supabase.storage.from("hotel-media").upload(path, file, { contentType: file.type, upsert: false });if(upError)throw upError
      const { data: publicData } = supabase.storage.from("hotel-media").getPublicUrl(data.path);setDraft(current => ({ ...current, hero_url: publicData.publicUrl }));setNotice("Imagen cargada. Guardá los cambios para publicarla.")
  ...
  return <section className={s.page}>
    <header className={s.header}><div><small>INTEGRACIONES</small><h1>Canales y venta directa</h1><p>Un solo inventario para OTAs, web propia y motor de reservas.</p></div><div className={s.tabs}>{[["engine","Motor web"],["channels","Canales"],["rooms","Mapeos"],["history","Actividad"]].map(([id,name])=><button key={id} className={tab===id?s.active:""} onClick={()=>chooseTab(id)}>{name}</button>)}</div></header>
    {error && <div className={s.notice}>{error}</div>}{notice && <div className={s.security}><span>✓</span><div><b>{notice}</b></div></div>}
    {loading ? <div className={s.notice}>Cargando…</div> : tab === "engine" ? <div style={{ display: "grid", gap: 12 }}>
      {!engine ? <section style={box}><small>MOTOR WEB</small><h2>Activá la reserva directa</h2><p style={{ color: "var(--muted)", fontSize: 12, maxWidth: 720 }}>Se conecta a las mismas habitaciones, tarifas y cierres de venta del PMS.</p><button style={{ ...button, background: "linear-gradient(145deg,var(--accent),var(--accent2))", color: "#fff", border: 0 }} disabled={saving} onClick={createEngine}>{saving ? "Creando…" : "Crear motor"}</button></section> : <>
        ...
        <section style={{ ...box, display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 12 }}>
          <label style={label}>Nombre público<input style={field} value={draft?.display_name || ""} onChange={event => setDraft(value => ({ ...value, display_name: event.target.value }))} /></label><label style={label}>Plantilla<select style={field} value={draft?.template || "classic"} onChange={event => setDraft(value => ({ ...value, template: event.target.value }))}><option value="classic">Clásica</option><option value="boutique">Boutique</option><option value="minimal">Minimal</option></select></label><label style={label}>Moneda<select style={field} value={draft?.currency || "ARS"} onChange={event => setDraft(value => ({ ...value, currency: event.target.value }))}><option>ARS</option><option>USD</option><option>EUR</option><option>BRL</option><option>UYU</option></select></label>
          <label style={label}>Estadía mínima<input style={field} type="number" min="1" value={draft?.min_nights || 1} onChange={event => setDraft(value => ({ ...value, min_nights: event.target.value }))} /></label><label style={label}>Anticipación mínima<input style={field} type="number" min="0" value={draft?.min_advance_days || 0} onChange={event => setDraft(value => ({ ...value, min_advance_days: event.target.value }))} /></label><label style={label}>Anticipación máxima<input style={field} type="number" min="1" value={draft?.max_advance_days || 730} onChange={event => setDraft(value => ({ ...value, max_advance_days: event.target.value }))} /></label>
          <label style={{ ...label, gridColumn: "1/-1" }}>Mensaje de venta<textarea style={area} value={draft?.booking_message || ""} onChange={event => setDraft(value => ({ ...value, booking_message: event.target.value }))} /></label><label style={{ ...label, gridColumn: "1/-1" }}>Política de cancelación<textarea style={area} value={draft?.cancellation_policy || ""} onChange={event => setDraft(value => ({ ...value, cancellation_policy: event.target.value }))} placeholder="Ej. Cancelación sin cargo hasta 48 h antes. Luego se cobra una noche." /></label>
          <label style={{ ...label, display: "flex", alignItems: "center", gap: 9, padding: "11px 12px", border: "1px solid var(--line)", borderRadius: 10, color: "var(--text)" }}><input type="checkbox" checked={Boolean(draft?.allow_self_cancel)} onChange={event => setDraft(value => ({ ...value, allow_self_cancel: event.target.checked }))} /><span><b style={{ display: "block", fontSize: 12 }}>Permitir cancelación desde el portal</b><small style={{ color: "var(--muted)" }}>Sólo reservas web y dentro de la ventana configurada.</small></span></label><label style={label}>Límite para cancelar (horas antes)<input style={field} type="number" min="0" max="720" disabled={!draft?.allow_self_cancel} value={draft?.self_cancel_cutoff_hours ?? 48} onChange={event => setDraft(value => ({ ...value, self_cancel_cutoff_hours: event.target.value }))} /></label><div />
          <label style={label}>Cobro<select style={field} value={draft?.payment_mode || "at_property"} onChange={event => setDraft(value => ({ ...value, payment_mode: event.target.value }))}><option value="at_property">Pago en el hotel</option><option value="deposit" disabled={!paymentConnected}>Seña online{!paymentConnected ? " · requiere Mercado Pago" : ""}</option><option value="full" disabled={!paymentConnected}>Pago total online{!paymentConnected ? " · requiere Mercado Pago" : ""}</option></select></label>{draft?.payment_mode === "deposit" ? <label style={label}>Seña %<input style={field} type="number" min="1" max="100" value={draft?.deposit_percent || 0} onChange={event => setDraft(value => ({ ...value, deposit_percent: event.target.value }))} /></label> : <div />}
          <label style={label}>Teléfono<input style={field} value={draft?.contact_phone || ""} onChange={event => setDraft(value => ({ ...value, contact_phone: event.target.value }))} /></label><label style={label}>Email<input style={field} value={draft?.contact_email || ""} onChange={event => setDraft(value => ({ ...value, contact_email: event.target.value }))} /></label><label style={label}>Términos URL<input style={field} value={draft?.terms_url || ""} onChange={event => setDraft(value => ({ ...value, terms_url: event.target.value }))} /></label><label style={label}>Privacidad URL<input style={field} value={draft?.privacy_url || ""} onChange={event => setDraft(value => ({ ...value, privacy_url: event.target.value }))} /></label>
        </section>
