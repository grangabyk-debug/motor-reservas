...
export default function IntegrationsWorkspace({propertyId,section="apps",onNavigate}){
  ...
  if(section==="api")return <section className={s.catalogPage}><header className={s.catalogHeader}><div><small>{meta.eyebrow}</small><h1>{meta.title}</h1><p>{meta.description}</p></div><span className={s.connectedCount}>API v1</span></header><ApiAccessPanel propertyId={propertyId}/></section>
