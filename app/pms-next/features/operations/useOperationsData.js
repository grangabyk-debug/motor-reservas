...
export default function useOperationsData(propertyId){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const[ticketRes,requestRes,reservationRes,roomRes,catalogRes,hkRes,membersRes]=await Promise.all([
        supabase.from("hotel_maintenance_tickets").select("id,reservation_id,room_id,resource_id,title,description,priority,status,assigned_to,reported_by,due_at,started_at,completed_at,cost,photos,notes,created_at,updated_at").eq("property_id",propertyId).order("created_at",{ascending:false}),
        supabase.from("hotel_guest_requests").select("id,reservation_id,room_id,title,detail,status,priority,assigned_area,assigned_to,requested_by,due_at,created_by,resolved_at,created_at,updated_at").eq("property_id",propertyId).order("created_at",{ascending:false}).limit(300),
        ...
        supabase.from("hotel_housekeeping_tasks").select("id,reservation_id,room_id,task_type,priority,status,assigned_to,scheduled_for,checklist,notes,updated_at").eq("property_id",propertyId).gte("scheduled_for",start.toISOString()).lt("scheduled_for",end.toISOString()).order("scheduled_for"),
      ...
      const ids=(membersRes.data||[]).map(item=>item.user_id),roleByUser=new Map((membersRes.data||[]).map(item=>[item.user_id,item.role]));let profileRows=[]
      if(ids.length){const{data,error:profileError}=await supabase.from("profiles").select("id,full_name,role").in("id",ids);if(profileError)throw profileError;profileRows=(data||[]).map(row=>({...row,member_role:roleByUser.get(row.id)||row.role}))}
  ...
  useEffect(()=>{load()},[load])
  usePmsAutoRefresh(propertyId,load,["reservas","habitaciones","hotel_housekeeping_tasks","hotel_maintenance_tickets","hotel_guest_requests","hotel_operational_events"])
  ...
  const createGuestRequest=useCallback(async draft=>{const{data:userData,error:userError}=await supabase.auth.getUser();if(userError)throw userError;const reservation=draft.reservation_id?reservations.find(item=>Number(item.id)===Number(draft.reservation_id)):null;const roomId=draft.room_id?Number(draft.room_id):reservation?.habitacion_id?Number(reservation.habitacion_id):null;const requestedBy=draft.requested_by==="staff"?"other":draft.requested_by||"guest";const payload={property_id:propertyId,reservation_id:draft.reservation_id?Number(draft.reservation_id):null,room_id:roomId,title:draft.title.trim(),detail:draft.detail?.trim()||null,status:"open",priority:draft.priority||"normal",assigned_area:draft.assigned_area||"reception",assigned_to:draft.assigned_to||null,requested_by:requestedBy,due_at:draft.due_at||null,created_by:userData?.user?.id||null};const{data,error:insertError}=await supabase.from("hotel_guest_requests").insert(payload).select().single();if(insertError)throw insertError;setGuestRequests(list=>[data,...list]);return data},[propertyId,reservations])
  ...
  const getTimeline=useCallback(async(entityType,entityId)=>{const{data,error:timelineError}=await supabase.from("hotel_operational_events").select("id,event_type,from_status,to_status,actor_id,metadata,created_at").eq("property_id",propertyId).eq("entity_type",entityType).eq("entity_id",entityId).order("created_at",{ascending:true});if(timelineError)throw timelineError;return data||[]},[propertyId])
  ...
  return{tickets,guestRequests,reservations,rooms,profiles,checklistCatalog,housekeepingTasks,roomById,profileById,reservationById,loading,error,setError,load,createTicket,updateTicket,createGuestRequest,updateGuestRequest,getTimeline,createChecklistItem,toggleChecklistItem}
}
