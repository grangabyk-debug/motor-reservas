...
function Placeholder({view}){
  return <section className={s.placeholder}><small>MÓDULO EN CONSTRUCCIÓN</small><h1>{NAV_LABELS[view]||"Módulo"}</h1><p>Este módulo pertenece al PMS nuevo y se va a construir acá, sin reutilizar la interfaz anterior.</p></section>
}
...
export default function PmsNextApp(){
  ...
  return <div className={s.app} data-theme={theme}>
    ...
    <main className={s.workspace}>
      ...
      {view==="dashboard"?<DashboardWorkspace onPlanning={()=>setView("planning")}/>:view==="planning"?<PlanningWorkspace/>:<Placeholder view={view}/>} 
    </main>
  </div>
}
