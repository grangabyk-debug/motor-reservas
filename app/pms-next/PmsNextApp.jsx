...
import PmsDialogHost from"./components/system/PmsDialogHost"
import PaymentAlertWatcher from"./components/system/PaymentAlertWatcher"
import PmsNotificationCenter from"./components/system/PmsNotificationCenter"
...
import DailyControlWorkspace from"./features/reception/DailyControlWorkspace"
...
function WorkspacePane({id,active,mounted,children}){if(!mounted||!active)return null;return <div className={s.workspacePane} data-workspace={id}>{children}</div>}

export default function PmsNextApp({buildId="local"}){
  const[view,setView]=useState("dashboard"),[mountedViews,setMountedViews]=useState(()=>new Set(["dashboard"])),[theme,setTheme]=useState("light"),[bootChecked,setBootChecked]=useState(false),[bootDone,setBootDone]=useState(false),[reservationFocus,setReservationFocus]=useState(null),[cashReservationFocus,setCashReservationFocus]=useState(null),[newReservationRequest,setNewReservationRequest]=useState(0),[searchOpen,setSearchOpen]=useState(false),[messageUnread,setMessageUnread]=useState(0),[notificationsOpen,setNotificationsOpen]=useState(false),[notificationCount,setNotificationCount]=useState(0)
  ...
  const rolePermissions=useMemo(()=>featureState.settings?.role_permissions||{},[featureState.settings])
  ...
  useEffect(()=>{const handler=e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="k"){e.preventDefault();setSearchOpen(true)}else if(e.key==="Escape")setNotificationsOpen(false)};window.addEventListener("keydown",handler);return()=>window.removeEventListener("keydown",handler)},[])
  useEffect(()=>{if(typeof document==="undefined")return;const html=document.documentElement,body=document.body,previousHtmlOverflow=html.style.overflow,previousBodyOverflow=body.style.overflow;if(view==="planning"){html.style.overflow="hidden";body.style.overflow="hidden"}else{html.style.overflow=previousHtmlOverflow;body.style.overflow=previousBodyOverflow}return()=>{html.style.overflow=previousHtmlOverflow;body.style.overflow=previousBodyOverflow}},[view])
  ...
  const activateView=useCallback((requested,{historyMode="push",restoreScroll=true,reservationId=null,cashReservationId=null}={})=>{const next=canOpenView(role,requested,featureState.flags,rolePermissions)?requested:"dashboard",previous=viewRef.current,sameView=next===previous,resetReceptionReports=sameView&&next==="receptionreports"&&historyMode==="push";if(next==="reservations"&&reservationId!=null)setReservationFocus(Number(reservationId));if(next==="dailycash"&&cashReservationId!=null)setCashReservationFocus(Number(cashReservationId));if(typeof window!=="undefined"&&!sameView)scrollPositions.current[previous]=window.scrollY;viewRef.current=next;setMountedViews(current=>{if(current.has(next))return current;const nextSet=new Set(current);nextSet.add(next);return nextSet});setView(next);if(typeof window!=="undefined"){const url=new URL(window.location.href);url.searchParams.set("view",next);if(resetReceptionReports)url.searchParams.delete("report");if(next==="dailycash"&&cashReservationId!=null)url.searchParams.set("cash_reservation",String(cashReservationId));else if(next!=="dailycash"||cashReservationId==null)url.searchParams.delete("cash_reservation");if(historyMode==="replace")window.history.replaceState({pmsView:next},"",url);else if(historyMode==="push"&&(!sameView||resetReceptionReports))window.history.pushState({pmsView:next},"",url);if(resetReceptionReports)window.dispatchEvent(new CustomEvent("hl:reception-reports-home"));if(restoreScroll&&!sameView)requestAnimationFrame(()=>window.scrollTo({top:scrollPositions.current[next]||0,behavior:"auto"}))}},[role,featureState.flags,rolePermissions])
  ...
  return <div className={`${s.app} ${visual.system}`} data-theme={theme}><style>{`[data-workspace] > section > header p{display:none!important}`}</style><PmsSidebar view={view} onView={activateView} property={session.property} properties={session.properties} onProperty={session.selectProperty} user={session.user} featureFlags={featureState.flags} rolePermissions={rolePermissions} buildId={buildId} badges={{messages:messageUnread}}/><main className={`${s.workspace} ${glass.mobileSafeArea}`} style={view==="planning"?{height:"100dvh",overflow:"hidden"}:undefined}><PmsTopbar title={NAV_LABELS[view]||"Dashboard"} info={VIEW_DESCRIPTIONS[view]||""} theme={theme} onTitleClick={view==="receptionreports"?()=>activateView("receptionreports"):null} onToggleTheme={toggleTheme} onNewReservation={allowedSet.has("planning")?startNewReservation:null} onNewQuote={allowedSet.has("quotes")?()=>activateView("quotes"):null} onOpenSearch={()=>setSearchOpen(true)} onOpenActivity={allowedSet.has("audit")?()=>activateView("audit"):null} onOpenNotifications={()=>setNotificationsOpen(true)} notificationCount={notificationCount} onOpenSupport={allowedSet.has("support")?()=>activateView("support"):null} timeZone={featureState.settings?.preferences?.timezone}/><ReservationReturnBar view={view} onReturn={reservationId=>activateView("reservations",{reservationId,restoreScroll:false})}/>{pane("dashboard",<DashboardWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("portfolio",<PortfolioWorkspace properties={session.properties} activePropertyId={session.propertyId} onProperty={session.selectProperty} onNavigate={activateView}/>)}{pane("planning",<PlanningWorkspace {...shared} onNavigate={activateView} newReservationRequest={newReservationRequest}/>)}{pane("reservations",<ReservationsWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews} focusReservationId={reservationFocus} onFocusHandled={()=>setReservationFocus(null)}/>)}{pane("quotes",<QuotesWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("guests",<GuestsWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("crm",<CrmWorkspace {...shared} onNavigate={activateView}/>)}{pane("messages",<MessagesWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("maintenance",<OperationsWorkspace {...shared} initialTab="maintenance" onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("tasks",<OperationsWorkspace {...shared} initialTab="tasks" onNavigate={activateView} allowedViews={allowedViews}/>)}{featureState.flags.guest_requests&&pane("requests",<OperationsWorkspace {...shared} initialTab="requests" onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("housekeeping",<HousekeepingWorkspace {...shared} onNavigate={activateView} allowedViews={allowedViews}/>)}{pane("inventory",<InventoryWorkspace {...shared}/>)}{pane("services",<ServicesWorkspace {...shared}/>)}{pane("dailycash",<DailyCashWorkspace {...shared} onNavigate={activateView} focusReservationId={cashReservationFocus} onFocusHandled={()=>setCashReservationFocus(null)}/>)}{pane("dailycontrol",<DailyControlWorkspace {...shared}/>)}{pane("finance",<FinanceWorkspace {...shared}/>)}{pane("receptionreports",<ReceptionReportsWorkspace {...shared}/>)}{pane("rates",<RatesWorkspace {...shared}/>)}{pane("onboarding",<OnboardingWorkspace {...shared} onNavigate={activateView}/>)}{pane("website",<WebsitePanel {...shared}/>)}{pane("growth",<GrowthWorkspace {...shared}/>)}{pane("revenue",<RevenueWorkspace {...shared} onNavigate={activateView}/>)}{pane("intelligence",<IntelligenceWorkspace {...shared}/>)}{pane("channelmanager",<ChannelManagerWorkspace {...shared}/>)}{pane("reports",<ReportsWorkspace {...shared}/>)}{pane("audit",<AuditWorkspace {...shared} onNavigate={activateView}/>)}{pane("staff",<StaffWorkspace {...shared}/>)}{pane("integrations",<IntegrationsWorkspace {...shared} section="apps" onNavigate={activateView}/>)}{pane("integrationapi",<IntegrationsWorkspace {...shared} section="api" onNavigate={activateView}/>)}{pane("integrationmessages",<IntegrationsWorkspace {...shared} section="messages" onNavigate={activateView}/>)}{pane("integrationpayments",<IntegrationsWorkspace {...shared} section="payments" onNavigate={activateView}/>)}{pane("subscription",<SubscriptionWorkspace {...shared}/>)}{pane("settings",<SettingsWorkspace {...shared}/>)}{pane("help",<HelpWorkspace onNavigate={activateView}/>)}{pane("support",<SupportWorkspace {...shared}/>)}</main><PmsMobileNav view={view} onView={activateView} allowedViews={allowedViews}/><PmsGlobalSearch open={searchOpen} onClose={()=>setSearchOpen(false)} propertyId={session.propertyId} onNavigate={activateView} allowedViews={allowedViews}/><PmsNotificationCenter open={notificationsOpen} onClose={()=>setNotificationsOpen(false)} propertyId={session.propertyId} onNavigate={activateView} onCountChange={setNotificationCount} timeZone={featureState.settings?.preferences?.timezone}/><PaymentAlertWatcher propertyId={session.propertyId} onNavigate={activateView} onUnreadChange={setMessageUnread}/><PmsDialogHost/><PmsToastHost/><PmsUpdateNotice buildId={buildId}/></div>
}
