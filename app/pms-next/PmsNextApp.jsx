...
import{canOpenView,getAllowedViews,NAV_LABELS}from"./core/navigation"
...
export default function PmsNextApp({buildId="local"}){
  ...
  const activateView=useCallback((requested,{historyMode="push",restoreScroll=true,reservationId=null}={})=>{
    const next=canOpenView(role,requested,featureState.flags,rolePermissions)?requested:"dashboard"
  ...
  },[role,featureState.flags,rolePermissions])
  const startNewReservation=useCallback(()=>{activateView("planning");setNewReservationRequest(value=>value+1)},[activateView])
