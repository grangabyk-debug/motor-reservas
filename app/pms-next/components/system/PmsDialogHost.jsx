...
export default function PmsDialogHost(){
  ...
  useEffect(()=>{
    if(!active)return
    const key=event=>{if(event.key==="Escape"){event.preventDefault();finish(active.kind==="alert"?true:false)}}
  ...
  function finish(value){
    if(!active)return
    try{active.resolve(Boolean(value))}catch{}
  ...
  const alertOnly=active.kind==="alert"
  return <div role="presentation" style={{position:"fixed",inset:0,zIndex:2000,display:"grid",placeItems:"center",padding:16,background:"rgba(12,20,38,.34)",backdropFilter:"blur(12px) saturate(1.12)",WebkitBackdropFilter:"blur(12px) saturate(1.12)"}}>
    <section role={alertOnly?"alertdialog":"dialog"} aria-modal="true" aria-label={active.title||"Confirmar acción"} style={{width:"min(440px,calc(100vw - 28px))",overflow:"hidden",border:"1px solid color-mix(in srgb,#fff 34%,var(--line))",borderRadius:20,background:"color-mix(in srgb,var(--panelSolid) 96%,transparent)",boxShadow:"inset 0 1px color-mix(in srgb,#fff 55%,transparent),0 28px 80px rgba(14,24,46,.3)",color:"var(--text)"}}>
      ...
      <footer style={{padding:"12px 16px 16px",display:"flex",justifyContent:"flex-end",gap:8,borderTop:"1px solid var(--line)"}}>
        {!alertOnly?<button type="button" autoFocus onClick={()=>finish(false)} style={{height:39,padding:"0 14px",border:"1px solid var(--line)",borderRadius:10,background:"var(--panel)",color:"var(--text)",font:"inherit",fontSize:10.5,fontWeight:850,cursor:"pointer"}}>{active.cancelLabel||"Cancelar"}</button>:null}
        <button type="button" autoFocus={alertOnly} onClick={()=>finish(true)} style={{height:39,padding:"0 15px",border:0,borderRadius:10,background:active.tone==="danger"?"var(--red)":"linear-gradient(145deg,var(--accent),var(--accent2))",color:"#fff",font:"inherit",fontSize:10.5,fontWeight:900,cursor:"pointer",boxShadow:"0 8px 20px color-mix(in srgb,var(--accent) 20%,transparent)"}}>{active.confirmLabel||"Confirmar"}</button>
      </footer>
    </section>
  </div>
}
