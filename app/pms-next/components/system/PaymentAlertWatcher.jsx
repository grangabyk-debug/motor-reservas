...
export default function PaymentAlertWatcher({propertyId}){
  const known=useRef(new Set()),primed=useRef(false)
  useEffect(()=>{
    ...
    async function check(){
      ...
      window.dispatchEvent(new CustomEvent("hl:pms-payment-updated",{detail:{reservationId:latest.reserva_id,paymentIds:fresh.map(row=>row.id)}}))
    }
