...
function NavButton({item,active,onView}){return <button type="button" style={{"--nav-icon":NAV_COLORS[item.id]||"var(--accent)"}} className={`${s.navItem} ${glass.navItemGlass} ${active?`${s.navItemActive} ${glass.navItemGlassActive}`:""}`} onClick={()=>onView(item.id)}><span className={glass.navIcon}><PmsIcon name={item.icon} size={15}/></span><span>{item.label}</span></button>}
function NavGroup({title,items,view,onView}){if(!items.length)return null;return <section className={s.navGroup}>{title&&<small className={s.navLabel}>{title}</small>}{items.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}</section>}
function NavFolder({label,icon,color,items,view,onView,defaultOpen=false}){
...
export default function PmsSidebar({view,onView,property,properties=[],onProperty,user,featureFlags={},rolePermissions={},buildId="local"}){
  ...
  return <aside className={`${s.sidebar} ${glass.sidebarGlass}`}>
    ...
    <nav className={s.navScroll}><NavGroup items={primary} view={view} onView={onView}/><section className={s.navGroup}><small className={s.navLabel}>Operación</small><NavFolder label="Recepción" icon="booking" color="#4b5fd5" items={reception} view={view} onView={onView}/><NavFolder label="Housekeeping" icon="clean" color="#21a8a0" items={housekeeping} view={view} onView={onView}/>{operations.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}</section><section className={s.navGroup}><small className={s.navLabel}>Gestión</small>{management.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}<NavFolder label="Integraciones" icon="link" color="#557ed1" items={integrations} view={view} onView={onView}/></section></nav>
    <div style={{display:"grid",gap:8,padding:"0 10px 8px"}}>{["owner","manager","admin"].includes(role)?<SidebarSubscriptionCard propertyId={property?.id} onManage={()=>onView("subscription")}/>:null}</div>
