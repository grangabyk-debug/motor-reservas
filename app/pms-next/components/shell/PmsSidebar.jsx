...
export default function PmsSidebar({view,onView,property,properties=[],onProperty,user,featureFlags={},buildId="local"}){
  const propertyName=property?.name||"Seleccionar propiedad",operations=OPERATIONS_NAV.filter(item=>item.id!=="requests"||featureFlags.guest_requests===true),version=buildId&&buildId!=="local"?buildId.slice(0,7):"local",roleLabel=ROLE_LABELS[property?.role]||"Equipo"
  return <aside className={s.sidebar}>
    ...
    <nav className={s.navScroll}><NavGroup items={PRIMARY_NAV} view={view} onView={onView}/><NavGroup title="Operación" items={operations} view={view} onView={onView}/><NavGroup title="Gestión" items={MANAGEMENT_NAV} view={view} onView={onView}/></nav>
    <div className={s.sidebarFooter}><span><b>{user?.email||"Usuario autenticado"}</b><small>{roleLabel} · v{version}</small></span><button type="button" title={`Configuración · versión ${version}`} onClick={()=>onView("settings")}>•••</button></div>
  </aside>
}
