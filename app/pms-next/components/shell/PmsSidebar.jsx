...
export default function PmsSidebar({view,onView,property,properties=[],onProperty,user,featureFlags={},rolePermissions={},buildId="local"}){
  ...
  return <aside className={`${s.sidebar} ${glass.sidebarGlass}`}>
    ...
    <nav className={s.navScroll}><NavGroup items={primary} view={view} onView={onView}/><section className={s.navGroup}><small className={s.navLabel}>Operación</small><NavFolder label="Recepción" icon="booking" color="#4b5fd5" items={reception} view={view} onView={onView}/>{housekeeping.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}{operations.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}</section><section className={s.navGroup}><small className={s.navLabel}>Gestión</small>{management.map(item=><NavButton key={item.id} item={item} active={view===item.id} onView={onView}/>)}<NavFolder label="Integraciones" icon="link" color="#557ed1" items={integrations} view={view} onView={onView}/></section></nav>
