...
export default function HotelSite({slug}){
  ...
  const params=typeof window!=="undefined"?new URLSearchParams(window.location.search):null,candidate=params?.get("template_preview"),template=window?.location?.hostname?.endsWith(".vercel.app")&&TEMPLATE_IDS.includes(candidate)?candidate:data.template
  return <HotelWebsiteCanvas data={{...data,template}} slug={slug}/>
}
