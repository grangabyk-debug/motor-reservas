...
function diffRaw(a, b) {
  return Math.round((parseDate(b) - parseDate(a)) / 86400000)
}

function diffDays(a, b) {
  return Math.max(1, diffRaw(a, b))
}
...
function statusMeta(status) {
  return {
    alojado: ["Alojado", "emerald"],
    confirmada: ["Confirmada", "gold"],
...
function roomMeta(status) {
  return {
    limpia: ["Lista", "emerald"],
    libre: ["Lista", "emerald"],
    sucia: ["Sucia", "rose"],
    inspeccion: ["Inspección", "amber"],
    mantenimiento: ["Mantenimiento", "muted"],
    ocupada: ["Ocupada", "gold"],
...
export default function HotelOS() {
  ...
  const [aiMessages, setAiMessages] = useState([
    {
      role: "assistant",
      text: "Estoy conectada a la operación de este hotel. Preguntame por ocupación, reservas, housekeeping o revenue.",
    },
  ])
  ...
  useEffect(() => {
    ...
    setNewForm((current) => ({ ...current, start: now, end: addDays(now, 2) }))
  }, [])
  ...
  async function loadProperties(uid = user?.id) {
    ...
    const ids = Array.from(new Set([
      ...(member || []).map((item) => item.property_id),
    ...
    const { data: list, error } = await supabase
      .from("properties")
      .select("id,name,city,description,owner_id")
      .in("id", ids)
      .order("created_at")
    ...
    setPropertyId((current) => {
      if (current && ids.map(String).includes(String(current))) return current
  ...
  async function createHotel(event) {
    ...
    await supabase.from("alojamientos").insert({
      nombre: name,
      user_id: user.id,
      property_id: property.id,
    })

    await supabase.from("hotel_os_settings").insert({
      property_id: property.id,
      hotel_name: name,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`hotel-os-${propertyId}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "reservas", filter: `property_id=eq.${propertyId}` },
        () => loadHotel({ quiet: true })
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "habitaciones", filter: `property_id=eq.${propertyId}` },
        () => loadHotel({ quiet: true })
      )
      .subscribe()
  ...
  async function loadHotel({ quiet = false } = {}) {
    ...
    const [accommodationsResult, roomsResult, reservationsResult, paymentsResult, settingsResult, automationsResult, connectionsResult] = await Promise.all([
    ...
    const results = [accommodationsResult, roomsResult, reservationsResult, paymentsResult, settingsResult, automationsResult, connectionsResult]
    results.forEach((result) => { if (result.error) console.warn(result.error) })

    setAlojamientos(accommodationsResult.data || [])
    ...
    setSettings({
      ...DEFAULT_SETTINGS,
      hotel_name: property?.name || DEFAULT_SETTINGS.hotel_name,
      city: property?.city || "",
      ...(settingsResult.data || {}),
    })
  ...
  const activeReservations = useMemo(
    () => reservations.filter((reservation) => normalize(reservation.estado) !== "cancelada" && !reservation.no_show),
    [reservations]
  )
  ...
  const todayOccupied = useMemo(
    () => activeReservations.filter((reservation) =>
      today &&
      reservation.fecha_entrada <= today &&
      reservation.fecha_salida > today &&
      normalize(reservation.estado) !== "finalizada"
    ),
    [activeReservations, today]
  )

  const arrivals = useMemo(
    () => activeReservations.filter((reservation) => reservation.fecha_entrada === today && normalize(reservation.estado) !== "finalizada"),
    [activeReservations, today]
  )

  const departures = useMemo(
    () => activeReservations.filter((reservation) => reservation.fecha_salida === today && normalize(reservation.estado) !== "finalizada"),
    [activeReservations, today]
  )

  const sellableRooms = useMemo(
    () => rooms.filter((room) => room.activa !== false && normalize(room.estado) !== "mantenimiento"),
    [rooms]
  )

  const occupancy = sellableRooms.length ? Math.round((todayOccupied.length / sellableRooms.length) * 100) : 0
  const gross = activeReservations.reduce((sum, reservation) => sum + Number(reservation.precio_total || 0), 0)
  const nightsSold = activeReservations.reduce((sum, reservation) => sum + (Number(reservation.noches) || diffDays(reservation.fecha_entrada, reservation.fecha_salida)), 0)
  const adr = nightsSold ? gross / nightsSold : 0
  const revpar = sellableRooms.length ? gross / Math.max(1, sellableRooms.length * 30) : 0

  const calendarDays = useMemo(
    () => calendarStart ? Array.from({ length: 14 }, (_, index) => addDays(calendarStart, index)) : [],
    [calendarStart]
  )

  const selectedReservation = reservations.find((reservation) => String(reservation.id) === String(selectedReservationId)) || null
  const selectedRoom = rooms.find((room) => String(room.id) === String(selectedRoomId)) || null

  function paidFor(reservation) {
    return payments
      .filter((payment) => String(payment.reserva_id) === String(reservation?.id))
      .reduce((sum, payment) => sum + Number(payment.monto || 0), 0)
  }

  function balanceFor(reservation) {
    return Math.max(0, Number(reservation?.precio_total || 0) - paidFor(reservation))
  }
  ...
  const guestProfiles = useMemo(() => {
    ...
    activeReservations.forEach((reservation) => {
      const key = normalize(reservation.email_huesped) || normalize(reservation.telefono_huesped) || normalize(reservation.nombre_huesped)
      if (!key) return

      const current = profiles.get(key) || {
        key,
        name: reservation.nombre_huesped,
        email: reservation.email_huesped,
        phone: reservation.telefono_huesped,
        stays: 0,
        nights: 0,
        lifetime: 0,
        last: null,
      ...
      current.stays += 1
      current.nights += Number(reservation.noches) || diffDays(reservation.fecha_entrada, reservation.fecha_salida)
      current.lifetime += Number(reservation.precio_total || 0)
      current.last = !current.last || reservation.fecha_entrada > current.last ? reservation.fecha_entrada : current.last
      if (reservation.notas) current.notes.add(reservation.notas)
      if (reservation.canal_reserva) current.channels.add(reservation.canal_reserva)
      profiles.set(key, current)
    })

    return Array.from(profiles.values())
      .map((profile) => ({ ...profile, notes: Array.from(profile.notes), channels: Array.from(profile.channels) }))
  ...
  const filteredReservations = useMemo(() => {
    ...
    return reservations.filter((reservation) => {
      const roomName = rooms.find((room) => String(room.id) === String(reservation.habitacion_id))?.nombre
      return [
  ...
  async function moveReservation(id, roomId, start, end = null) {
    const { error } = await supabase.rpc("hl_move_reservation_atomic", {
      p_reserva_id: Number(id),
      p_habitacion_id: Number(roomId),
      p_fecha_entrada: start,
      p_fecha_salida: end,
    })

    if (error) {
      const collision = error.code === "23P01" || normalize(error.message).includes("ocup") || normalize(error.message).includes("reserva")
      return notify(collision ? "Ese movimiento genera un cruce de reservas." : error.message)
    }

    await loadHotel({ quiet: true })
    notify("Reserva actualizada sin conflictos.")
  }
  ...
  async function splitReservation() {
    ...
    const { error } = await supabase.rpc("hl_split_reservation_atomic", {
      p_reserva_id: Number(selectedReservation.id),
      p_split_date: splitForm.date,
      p_target_room_id: Number(splitForm.roomId),
    })

    if (error) return notify(error.message)

    setSplitForm({ date: "", roomId: "" })
    setSelectedReservationId(null)
    await loadHotel({ quiet: true })
    notify("Estadía dividida correctamente.")
  }

  async function createReservation(event) {
    event.preventDefault()
    const room = rooms.find((item) => String(item.id) === String(newForm.roomId))

    if (!room || !newForm.guest.trim() || !newForm.start || !newForm.end || newForm.end <= newForm.start) {
      return notify("Revisá huésped, habitación y fechas.")
    }

    const nights = diffDays(newForm.start, newForm.end)
    const total = Number(room.precio || 0) * nights
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        nombre_huesped: newForm.guest.trim(),
        email_huesped: newForm.email.trim() || null,
        telefono_huesped: newForm.phone.trim() || null,
        fecha_entrada: newForm.start,
        fecha_salida: newForm.end,
        cantidad_huespedes: Number(newForm.pax) || 1,
        estado: "confirmada",
        notas: newForm.notes.trim() || null,
        user_id: user.id,
        tarifa_noche: Number(room.precio || 0),
        noches: nights,
        precio_total: total,
        subtotal: total,
        moneda: "ARS",
        property_id: propertyId,
        canal_reserva: newForm.channel,
        numero_reserva: `HL-${Date.now().toString(36).toUpperCase()}`,
      })
      .select("*")
      .single()
  ...
  async function checkIn(reservation) {
    ...
    if (error) return notify(error.message)
    await loadHotel({ quiet: true })
    notify(`Check-in de ${reservation.nombre_huesped} realizado.`)
  }

  async function checkout(reservation) {
    const { error } = await supabase.rpc("hl_checkout_reservation_atomic", {
    ...
    setSelectedReservationId(null)
    await loadHotel({ quiet: true })
    notify("Check-out realizado. Habitación enviada a limpieza.")
  }
  ...
  async function registerPayment(event) {
    ...
    const { error } = await supabase.from("pagos").insert({
      ...
      moneda: selectedReservation.moneda || "ARS",
      property_id: propertyId,
    })

    if (error) return notify(error.message)

    setPaymentForm({ amount: "", method: "Efectivo" })
    await loadHotel({ quiet: true })
  ...
  async function setRoomStatus(roomId, status) {
    ...
    setRooms((list) => list.map((room) => String(room.id) === String(roomId) ? { ...room, estado: status } : room))
    notify("Estado de habitación actualizado.")
  }

  async function applyRate(percent) {
    const allowed = rooms.filter((room) => room.activa !== false && Number(room.precio || 0) > 0)

    const results = await Promise.all(allowed.map((room) =>
      supabase
        .from("habitaciones")
        .update({ precio: Math.round(Number(room.precio) * (1 + percent / 100)) })
        .eq("id", room.id)
        .eq("property_id", propertyId)
    ))
    ...
    await loadHotel({ quiet: true })
    notify(`Tarifas ajustadas ${percent > 0 ? "+" : ""}${percent}%.`)
  }
  ...
  async function saveIdentity(event) {
    ...
    const payload = {
      property_id: propertyId,
      hotel_name: settings.hotel_name || "Mi hotel",
      city: settings.city || null,
      motto: settings.motto || DEFAULT_SETTINGS.motto,
  ...
  function readLogo(file) {
    ...
    reader.onload = () => setSettings((current) => ({ ...current, logo_data_url: String(reader.result) }))
  ...
  async function createAutomation(event) {
    ...
    const { error } = await supabase.from("hotel_automations").insert({
      property_id: propertyId,
      name: text.slice(0, 46),
      trigger_text: text,
      action_text: "Ejecutar esta regla sobre la operación del hotel",
      enabled: true,
      runs: 0,
      created_by: user.id,
    })
  ...
  async function toggleAutomation(item) {
    ...
    setAutomations((list) => list.map((automation) => automation.id === item.id ? { ...automation, enabled: !automation.enabled } : automation))
  }

  async function connectChannel(provider) {
    const current = connections.find((connection) => connection.provider === provider)
    const payload = {
      property_id: propertyId,
      provider,
      status: "sandbox",
      mode: "sandbox",
      account_ref: current?.account_ref || "sandbox",
    ...
    await loadHotel({ quiet: true })
    notify(`${provider}: entorno de integración preparado.`)
  }
  ...
  async function verifyChannel(item) {
    ...
    await loadHotel({ quiet: true })
    notify("Health-check sandbox correcto.")
  }
  ...
  async function runAI(event) {
    ...
    const rateMatch = question.match(/(?:subi|subí|aumenta|aumentá).*?(\d{1,2})\s*%/i)
    if (rateMatch) {
      const percent = Number(rateMatch[1])
      setAiProposal({ type: "rate", percent })
      setAiMessages((messages) => [...messages, {
        role: "assistant",
        text: `Puedo aumentar ${percent}% las tarifas activas. Te lo dejo listo para confirmar; no voy a cambiar precios sin tu aprobación.`,
      }])
      return
    }

    if (/sucia|limpieza/i.test(question)) {
      const dirty = rooms.filter((room) => normalize(room.estado) === "sucia")
      setAiMessages((messages) => [...messages, {
        role: "assistant",
        text: dirty.length
          ? `${dirty.length} habitaciones requieren limpieza: ${dirty.map((room) => room.nombre).join(", ")}.`
          : "No hay habitaciones marcadas como sucias.",
      }])
      return
    }
    ...
    try {
      ...
      const response = await fetch("/api/assistant", {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            reservas: activeReservations.slice(0, 180).map((reservation) => ({
              id: reservation.id,
              huesped: reservation.nombre_huesped,
              habitacion: reservation.habitacion_id,
              entrada: reservation.fecha_entrada,
      ...
      setAiMessages((messages) => [...messages, {
        role: "assistant",
        text: "No pude consultar la capa de IA en este momento; la operación del hotel sigue funcionando normalmente.",
      }])
    }
  }
  ...
  if (loading || !today) {
    return (
      <div className="loading">
        ...
        <strong>Preparando recepción…</strong>
        <span>Habitación Llena OS</span>
      </div>
    )
  }
    ...
    return (
      <div className="onboarding">
        ...
        <div className="onboardingCard">
          <div className="seal">HL</div>
          <small>BIENVENIDO A HABITACIÓN LLENA OS</small>
          <h1>Tu hotel empieza acá.</h1>
          <p>Creá la propiedad principal. Después vas a poder sumar habitaciones, identidad, equipo y canales desde el mismo escritorio.</p>
          <form onSubmit={createHotel}>
            <input value={creatingHotel} onChange={(event) => setCreatingHotel(event.target.value)} placeholder="Nombre del hotel" />
            <button>Crear mi hotel</button>
          </form>
        </div>
      </div>
    )
  ...
  return (
    <div className={`hotelApp theme-${settings.theme || "forest"}`}>
      <FiveStarStyles />
      ...
      <aside className="side">
        <div className="brandLockup">
          <div className="brandSeal">
            {settings.logo_data_url
              ? <img src={settings.logo_data_url} alt="Logo del hotel" />
              : <span>HL</span>}
          </div>
          <div>
            <strong>{settings.hotel_name || "Habitación Llena"}</strong>
            <small>{settings.city || "Hospitality Operating System"}</small>
          </div>
        </div>

        <div className="propertySwitch">
        ...
        <div className="sideStory">
          <span className="lamp" />
          <small>LA CASA HOY</small>
          <p>{settings.motto}</p>
        </div>

        <div className="sideFoot">
          <button onClick={() => setAiOpen(true)}>✦ Llena Intelligence</button>
          <button onClick={async () => { await supabase.auth.signOut(); window.location.href = "/login" }}>Salir</button>
        </div>
      </aside>
      ...
      <main className="workspace">
        <header className="topbar">
          <div>
            <small>{view === "overview" ? "FRONT DESK · HOY" : NAV.find((item) => item[0] === view)?.[1]?.toUpperCase()}</small>
            <h1>{view === "overview" ? settings.welcome_message : NAV.find((item) => item[0] === view)?.[1]}</h1>
          </div>

          <div className="topActions">
            <label className="search">
              <span>⌕</span>
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Huésped, reserva, habitación…" />
            </label>
            <button className="glass" onClick={() => setIdentityOpen(true)}>Personalizar</button>
            <button className="primary" onClick={() => setNewOpen(true)}>＋ Nueva reserva</button>
          </div>
        </header>
      ...
      {identityOpen && (
        <div className="modalShade" onClick={() => setIdentityOpen(false)}>
          <div className="identityModal" onClick={(event) => event.stopPropagation()}>
            <Identity compact />
            <button className="closeFloating" onClick={() => setIdentityOpen(false)}>×</button>
          </div>
        </div>
      )}
  ...
  function Overview() {
    const nextArrival = arrivals[0] || activeReservations
      .filter((reservation) => reservation.fecha_entrada > today)
      .sort((a, b) => a.fecha_entrada.localeCompare(b.fecha_entrada))[0]

    return (
      <div className="content overviewGrid">
        <section className="lobbyHero">
          <div className="heroShade" />
          <div className="heroCopy">
            <small>{new Date().getHours() < 13 ? "BUENOS DÍAS" : "BUEN TURNO"} · {settings.hotel_name}</small>
            <h2>{occupancy >= 80 ? "La casa está llena de vida." : "Una operación tranquila también se diseña."}</h2>
            <p>{arrivals.length} llegada{arrivals.length === 1 ? "" : "s"} · {departures.length} salida{departures.length === 1 ? "" : "s"} · {sellableRooms.filter((room) => roomMeta(room.estado)[0] === "Lista").length} habitaciones listas.</p>
            <div>
              <button onClick={() => setView("calendar")}>Abrir Command Center</button>
              <button onClick={() => setView("housekeeping")}>Ver Housekeeping</button>
            </div>
          </div>
          <div className="occupancyMedallion"><span>{occupancy}%</span><small>ocupación</small></div>
        </section>

        <section className="metricsRail">
          {[
            ["Llegadas", arrivals.length, "Front desk"],
            ["Salidas", departures.length, "Housekeeping"],
            ["ADR", money(adr), "Revenue"],
            ["RevPAR", money(revpar), "Performance"],
          ].map(([label, value, detail]) => (
            <article key={label}><small>{label}</small><strong>{value}</strong><span>{detail}</span></article>
          ))}
        </section>

        <section className="todayDesk paperCard">
          <div className="sectionHead">
            <div><small>EL ESCRITORIO DE HOY</small><h3>Lo importante, antes de que lo pidan.</h3></div>
            <button onClick={() => setView("reservations")}>Todas las reservas →</button>
          </div>
          <div className="deskColumns">
            <DeskColumn title="Llegan" items={arrivals} />
            <DeskColumn title="En casa" items={todayOccupied} />
            <DeskColumn title="Salen" items={departures} />
          </div>
        </section>

        <section className="humanCard">
          <img src="https://images.unsplash.com/photo-1556740749-887f6717d7e4?auto=format&fit=crop&w=900&q=82" alt="Recepción hotelera" />
          <div>
            <small>HOSPITALIDAD HUMANA</small>
            <h3>El sistema trabaja para que el equipo mire más al huésped y menos a la pantalla.</h3>
            <p>Menos pasos, menos ruido operativo y contexto visible justo donde recepción lo necesita.</p>
          </div>
        </section>

        <section className="conciergeCard paperCard">
          <div className="conciergePortrait">
            <img src="https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=500&q=80" alt="Concierge" />
          </div>
          <div>
            <small>MOMENTO DEL DÍA</small>
            <h3>{nextArrival ? `${nextArrival.nombre_huesped} es la próxima historia.` : "La recepción está preparada."}</h3>
            <p>{nextArrival ? `Llega ${shortDate(nextArrival.fecha_entrada)} · Habitación ${rooms.find((room) => String(room.id) === String(nextArrival.habitacion_id))?.nombre || "—"}. ${nextArrival.notas || "Sin pedidos especiales."}` : "No hay una llegada próxima cargada."}</p>
            {nextArrival && <button onClick={() => setSelectedReservationId(nextArrival.id)}>Abrir ficha</button>}
          </div>
        </section>

        <section className="intel paperCard">
          <div className="sectionHead">
            <div><small>LLENA INTELLIGENCE</small><h3>Un hotel que se anticipa.</h3></div>
            <button onClick={() => setAiOpen(true)}>Hablar con IA ✦</button>
          </div>
          <div className="intelRows">
            <article><span>01</span><div><b>{occupancy > 75 ? "Demanda alta detectada" : "Ritmo de venta saludable"}</b><p>{occupancy > 75 ? "Podés revisar una suba controlada de tarifas para las próximas noches." : "No hace falta forzar precios hoy. Priorizá conversión directa."}</p></div><button onClick={() => setView("revenue")}>Revenue</button></article>
            <article><span>02</span><div><b>Housekeeping con prioridad</b><p>{rooms.filter((room) => normalize(room.estado) === "sucia").length} habitaciones esperan limpieza.</p></div><button onClick={() => setView("housekeeping")}>Resolver</button></article>
            <article><span>03</span><div><b>Distribución supervisada</b><p>{connections.filter((connection) => connection.status === "healthy").length} conexiones pasaron health-check sandbox.</p></div><button onClick={() => setView("distribution")}>Distribution</button></article>
          </div>
        </section>
      </div>
    )
  ...
  function DeskColumn({ title, items }) {
    return (
      <div className="deskColumn">
        <h4>{title}<span>{items.length}</span></h4>
        {items.length ? items.slice(0, 4).map((reservation) => (
          <button key={reservation.id} onClick={() => setSelectedReservationId(reservation.id)}>
            <b>{reservation.nombre_huesped}</b>
            <span>Hab. {rooms.find((room) => String(room.id) === String(reservation.habitacion_id))?.nombre || "—"}</span>
            <small>{reservation.fecha_entrada === today ? "Hoy" : shortDate(reservation.fecha_entrada)}</small>
          </button>
        )) : <p>Sin movimientos.</p>}
      </div>
    )
      ...
      <div className="content">
        <section className="calendarShell">
          <div className="calendarToolbar">
            <div><small>ROOM DIARY · TIEMPO REAL</small><h2>La operación completa en una sola superficie.</h2></div>
            <div>
              <button onClick={() => setCalendarStart(addDays(calendarStart, -7))}>← 7 días</button>
              <button onClick={() => setCalendarStart(today)}>Hoy</button>
              <button onClick={() => setCalendarStart(addDays(calendarStart, 7))}>7 días →</button>
            </div>
          </div>

          <div className="calendarLegend">
            <span><i className="lg inhouse" />Alojado</span>
            <span><i className="lg confirmed" />Confirmada</span>
            <span><i className="lg pending" />Pendiente</span>
            <small>Arrastrá una reserva para moverla · abrila para extender, cobrar, dividir o hacer check-in/out.</small>
          </div>

          <div className="calendarScroller">
            <div className="calendarHeader" style={{ gridTemplateColumns: `190px repeat(${calendarDays.length}, minmax(78px,1fr))` }}>
              <div className="roomHeading">Habitación</div>
              {calendarDays.map((day) => (
                <div key={day} className={day === today ? "todayDay" : ""}>
                  <small>{parseDate(day).toLocaleDateString("es-AR", { weekday: "short" })}</small>
                  <b>{parseDate(day).getDate()}</b>
                  <span>{day === today ? "HOY" : ""}</span>
                </div>
              ))}
            </div>

            {rooms.filter((room) => room.activa !== false).map((room) => <CalendarRow key={room.id} room={room} />)}
          </div>
        </section>
      </div>
    )
  ...
  function CalendarRow({ room }) {
    const list = activeReservations.filter((reservation) =>
      String(reservation.habitacion_id) === String(room.id) &&
      reservation.fecha_salida > calendarStart &&
      reservation.fecha_entrada < addDays(calendarStart, 14)
    )

    return (
      <div className="calendarRow" style={{ gridTemplateColumns: `190px repeat(${calendarDays.length}, minmax(78px,1fr))` }}>
        <button className="roomLabel" style={{ gridColumn: 1, gridRow: 1 }} onClick={() => setSelectedRoomId(room.id)}>
          <strong>{room.nombre}</strong>
          <span>{room.tipo || "Habitación"}</span>
          <small className={`tone ${roomMeta(room.estado)[1]}`}>{roomMeta(room.estado)[0]}</small>
        </button>

        {calendarDays.map((day, index) => (
          <div
            key={day}
            className={`dayCell ${day === today ? "todayCell" : ""}`}
            style={{ gridColumn: index + 2, gridRow: 1 }}
            onDragOver={(event) => event.preventDefault()}
            onDrop={(event) => {
              event.preventDefault()
              const id = event.dataTransfer.getData("reservation")
              if (id) moveReservation(id, room.id, day)
            }}
            onDoubleClick={() => {
              setNewForm((form) => ({ ...form, roomId: String(room.id), start: day, end: addDays(day, 2) }))
              setNewOpen(true)
            }}
          />
        ))}
        ...
        {list.map((reservation) => {
          const start = Math.max(0, diffRaw(calendarStart, reservation.fecha_entrada))
          const end = Math.min(14, diffRaw(calendarStart, reservation.fecha_salida))
          const span = Math.max(1, end - start)
          const tone = statusMeta(reservation.estado)[1]

          return (
            <button
              draggable
              onDragStart={(event) => event.dataTransfer.setData("reservation", String(reservation.id))}
              onClick={() => setSelectedReservationId(reservation.id)}
              key={reservation.id}
              className={`reservationBar ${tone}`}
              style={{ gridColumn: `${start + 2} / span ${span}`, gridRow: 1 }}
            >
              <strong>{reservation.nombre_huesped}</strong>
              <small>{statusMeta(reservation.estado)[0]} · {balanceFor(reservation) > 0 ? `${money(balanceFor(reservation))} saldo` : "pagado"}</small>
            </button>
          )
        })}
      </div>
  ...
  function Reservations() {
    return (
      <div className="content">
        <section className="paperCard reservationDesk">
          <div className="sectionHead">
            <div><small>RESERVATION DESK</small><h3>Reservas sin perder contexto.</h3></div>
            <button className="primaryMini" onClick={() => setNewOpen(true)}>＋ Crear</button>
          </div>
          <div className="reservationList">
            {filteredReservations.length ? filteredReservations.map((reservation) => (
              <button key={reservation.id} onClick={() => setSelectedReservationId(reservation.id)}>
                <div className="guestAvatar">{String(reservation.nombre_huesped || "?").split(" ").slice(0, 2).map((part) => part[0]).join("")}</div>
                <div><b>{reservation.nombre_huesped}</b><span>{reservation.numero_reserva || `#${reservation.id}`} · {reservation.canal_reserva || "Directa"}</span></div>
                <div><small>Habitación</small><b>{rooms.find((room) => String(room.id) === String(reservation.habitacion_id))?.nombre || "—"}</b></div>
                <div><small>Estadía</small><b>{shortDate(reservation.fecha_entrada)} → {shortDate(reservation.fecha_salida)}</b></div>
                <div><small>Total</small><b>{money(reservation.precio_total)}</b></div>
                <div className={`statusPill ${statusMeta(reservation.estado)[1]}`}>{statusMeta(reservation.estado)[0]}</div>
              </button>
            )) : <div className="empty">No encontramos reservas.</div>}
          </div>
        </section>
      </div>
    )
  ...
  function Distribution() {
    return (
      <div className="content distributionGrid">
        <section className="distributionHero">
          <small>LLENA DISTRIBUTION HUB</small>
          <h2>Una sola disponibilidad. Todos los canales.</h2>
          <p>La capa está preparada para conectar un channel manager certificado sin duplicar lógica dentro del PMS. Hasta disponer de credenciales reales, cada proveedor queda claramente identificado como sandbox.</p>
          <div className="health"><span>{connections.filter((connection) => connection.status === "healthy").length}/{CHANNELS.length}</span><small>health-checks listos</small></div>
        </section>

        <section className="paperCard channelPanel">
          <div className="sectionHead"><div><small>CHANNEL HEALTH</small><h3>Distribución supervisada</h3></div></div>
          <div className="channelList">
            {CHANNELS.map((name) => {
              const connection = connections.find((item) => item.provider === name)
              return (
                <article key={name}>
                  <div className="channelLogo">{name.slice(0, 2).toUpperCase()}</div>
                  <div><b>{name}</b><span>{name === "Motor directo" ? "Canal nativo" : "Conector externo"}</span></div>
                  <div className={`statusPill ${connection?.status === "healthy" ? "emerald" : connection ? "amber" : "muted"}`}>
                    {connection?.status === "healthy" ? "Sandbox OK" : connection ? "Preparado" : "Sin configurar"}
                  </div>
                  <small>{connection?.last_sync_at ? new Date(connection.last_sync_at).toLocaleString("es-AR") : "—"}</small>
                  {connection
                    ? <button onClick={() => verifyChannel(connection)}>Verificar</button>
                    : <button onClick={() => connectChannel(name)}>Preparar</button>}
                </article>
              )
            })}
          </div>
        ...
        <section className="paperCard distributionMap">
          <small>ARQUITECTURA</small>
          <h3>El PMS sigue siendo la fuente de verdad.</h3>
          <div className="flow"><span>OTAs</span><i>→</i><span>Channel layer</span><i>↔</i><b>Habitación Llena OS</b><i>↔</i><span>Motor directo</span></div>
          <p>Booking.com, Expedia, Airbnb y otros requieren acuerdos, credenciales o certificación del proveedor. El sistema no muestra una conexión externa como real hasta que exista.</p>
        </section>
      </div>
    )
  }
  ...
  function Revenue() {
    ...
    activeReservations.forEach((reservation) => {
      const key = reservation.canal_reserva || "Directa"
      channelTotals.set(key, (channelTotals.get(key) || 0) + Number(reservation.precio_total || 0))
    })

    return (
      <div className="content">
        <section className="revenueHero">
          <div><small>LLENA REVENUE BRAIN</small><h2>Precio, demanda y margen sin abrir otra herramienta.</h2><p>La recomendación combina ocupación propia y ritmo de venta. La inteligencia de mercado externa se suma cuando conectemos fuentes certificadas.</p></div>
          <div className="revenueActions"><button onClick={() => applyRate(8)}>Aplicar +8%</button><button onClick={() => applyRate(-5)}>Aplicar −5%</button></div>
        </section>

        <section className="metricGrid">
          {[["Ocupación", `${occupancy}%`], ["ADR", money(adr)], ["RevPAR", money(revpar)], ["Ingresos", money(gross)]].map(([label, value]) => (
            <article className="paperCard" key={label}><small>{label}</small><strong>{value}</strong><span>operación actual</span></article>
          ))}
        </section>

        <div className="revenueLayout">
          <section className="paperCard revenueCard">
            <div className="sectionHead"><div><small>RENTABILIDAD</small><h3>Mix de canales</h3></div></div>
            <div className="bars">
              {Array.from(channelTotals.entries()).sort((a, b) => b[1] - a[1]).map(([name, value]) => (
                <div key={name}><div><b>{name}</b><span>{money(value)}</span></div><i><em style={{ width: `${gross ? Math.max(4, value / gross * 100) : 0}%` }} /></i></div>
              ))}
            </div>
          </section>

          <section className="paperCard rateBoard">
            <small>RATE BOARD</small><h3>Tarifa viva por habitación</h3>
            {rooms.filter((room) => room.activa !== false).slice(0, 8).map((room) => (
              <button key={room.id} onClick={() => setSelectedRoomId(room.id)}><span>{room.nombre} · {room.tipo}</span><b>{money(room.precio)}</b></button>
            ))}
          </section>
        </div>
      </div>
  ...
  function Guests() {
    return (
      <div className="content">
        <section className="guestHero"><small>GUEST GRAPH</small><h2>Recordar bien también es hospitalidad.</h2><p>Cada estadía suma contexto. El objetivo es que recepción reconozca a un huésped aunque vuelva por otro canal.</p></section>
        <section className="guestGrid">
          {guestProfiles.map((profile) => (
            <article className="guestCard" key={profile.key}>
              <div className="guestPortrait">{profile.name?.split(" ").slice(0, 2).map((part) => part[0]).join("")}</div>
              <div><small>{profile.stays > 2 ? "HUÉSPED RECURRENTE" : "PERFIL DE HUÉSPED"}</small><h3>{profile.name}</h3><p>{profile.email || profile.phone || "Sin contacto cargado"}</p></div>
              <div className="guestStats"><span><b>{profile.stays}</b> estadías</span><span><b>{profile.nights}</b> noches</span><span><b>{money(profile.lifetime)}</b> lifetime</span></div>
              <div className="tags">{profile.channels.map((channel) => <span key={channel}>{channel}</span>)}{profile.notes.slice(0, 2).map((note, index) => <span key={index}>{note.slice(0, 34)}</span>)}</div>
            </article>
          ))}
        </section>
      </div>
  ...
  function Housekeeping() {
    const rank = { sucia: 0, inspeccion: 1, mantenimiento: 2, limpia: 3, libre: 3 }
    const priority = rooms
      .filter((room) => room.activa !== false)
      .sort((a, b) => (rank[normalize(a.estado)] ?? 4) - (rank[normalize(b.estado)] ?? 4))

    return (
      <div className="content">
        <section className="houseHero">
          <div><small>HOUSEKEEPING INTELLIGENCE</small><h2>La próxima llegada define la prioridad.</h2><p>Recepción y pisos comparten el mismo estado, sin llamadas ni papelitos.</p></div>
          <div className="linenBadge"><span>{rooms.filter((room) => normalize(room.estado) === "sucia").length}</span><small>requieren atención</small></div>
        </section>

        <section className="houseGrid">
          {priority.map((room) => {
            const next = activeReservations
              .filter((reservation) => String(reservation.habitacion_id) === String(room.id) && reservation.fecha_entrada >= today)
              .sort((a, b) => a.fecha_entrada.localeCompare(b.fecha_entrada))[0]

            return (
              <article className={`houseCard ${roomMeta(room.estado)[1]}`} key={room.id}>
                <div className="doorNo">{room.nombre}</div>
                <div><small>{room.tipo || "Habitación"}</small><h3>{roomMeta(room.estado)[0]}</h3><p>{next ? `Próxima llegada: ${next.nombre_huesped} · ${shortDate(next.fecha_entrada)}` : "Sin llegada próxima"}</p></div>
                <select value={normalize(room.estado) || "libre"} onChange={(event) => setRoomStatus(room.id, event.target.value)}>
                  <option value="limpia">Lista</option><option value="sucia">Sucia</option><option value="inspeccion">Inspección</option><option value="mantenimiento">Mantenimiento</option>
                </select>
              </article>
            )
          })}
    ...
    return (
      <div className="content">
        <section className="automationHero">
          <small>WORKFLOWS</small><h2>Describí la operación como la dirías en recepción.</h2>
          <form onSubmit={createAutomation}><textarea value={automationDraft} onChange={(event) => setAutomationDraft(event.target.value)} placeholder="Ej. Si una reserva supera $500.000, pedir garantía y avisar a recepción." /><button>Crear regla ✦</button></form>
        </section>

        <section className="paperCard automationCard">
          <div className="sectionHead"><div><small>REGLAS DEL HOTEL</small><h3>Automatizaciones</h3></div></div>
          <div className="automationList">
            {automations.length ? automations.map((automation) => (
              <article key={automation.id}>
                <button className={`switch ${automation.enabled ? "on" : ""}`} onClick={() => toggleAutomation(automation)}><i /></button>
                <div><b>{automation.name}</b><p>{automation.trigger_text}</p><span>{automation.action_text}</span></div>
                <small>{automation.runs} ejecuciones</small>
              </article>
            )) : <div className="empty">Todavía no hay reglas. Escribí la primera arriba.</div>}
          </div>
        </section>
      </div>
    )
  ...
  function Twin() {
    ...
    rooms.forEach((room) => {
      const number = String(room.nombre || "")
      const floor = number.match(/^([1-9])/i)?.[1] || "PB"
    ...
    return (
      <div className="content">
        <section className="twinHero"><small>HOTEL DIGITAL TWIN</small><h2>El hotel físico, vivo dentro del sistema.</h2><p>Una representación espacial para entender ocupación, limpieza y próximas llegadas sin leer una lista.</p></section>
        <section className="hotelBuilding">
          {Array.from(floors.entries()).sort((a, b) => String(b[0]).localeCompare(String(a[0]))).map(([floor, list]) => (
            <div className="floor" key={floor}>
              <div className="floorLabel"><small>NIVEL</small><b>{floor}</b></div>
              <div className="doors">
                {list.map((room) => {
                  const occupant = todayOccupied.find((reservation) => String(reservation.habitacion_id) === String(room.id))
                  return (
                    <button className={`door ${roomMeta(room.estado)[1]} ${occupant ? "occupied" : ""}`} key={room.id} onClick={() => setSelectedRoomId(room.id)}>
                      <span className="doorHandle" /><small>{room.tipo || "Habitación"}</small><strong>{room.nombre}</strong><em>{occupant ? occupant.nombre_huesped : roomMeta(room.estado)[0]}</em>
                    </button>
                  )
                })}
              </div>
  ...
  function Identity({ compact = false } = {}) {
    return (
      <div className={compact ? "identity compact" : "content identity"}>
        <section className="identityShowcase">
          <div className="identityWallpaper">
            <div className="identityLogo">
              {settings.logo_data_url
                ? <img src={settings.logo_data_url} alt="Logo" />
                : <span>{(settings.hotel_name || "HL").split(" ").slice(0, 2).map((part) => part[0]).join("")}</span>}
            </div>
            <small>YOUR HOTEL · YOUR DESK</small><h2>{settings.hotel_name}</h2><p>{settings.motto}</p>
          </div>
        </section>

        <form className="paperCard identityForm" onSubmit={saveIdentity}>
          <div className="sectionHead"><div><small>IDENTIDAD</small><h3>Que el sistema también sea parte del hotel.</h3></div></div>
          <label><span>Nombre visible</span><input value={settings.hotel_name || ""} onChange={(event) => setSettings((current) => ({ ...current, hotel_name: event.target.value }))} /></label>
          <label><span>Lema</span><input value={settings.motto || ""} onChange={(event) => setSettings((current) => ({ ...current, motto: event.target.value }))} /></label>
          <label><span>Mensaje de recepción</span><input value={settings.welcome_message || ""} onChange={(event) => setSettings((current) => ({ ...current, welcome_message: event.target.value }))} /></label>
          <label><span>Ciudad</span><input value={settings.city || ""} onChange={(event) => setSettings((current) => ({ ...current, city: event.target.value }))} /></label>
          <label><span>Logo del hotel</span><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(event) => readLogo(event.target.files?.[0])} /></label>
          <label><span>Atmósfera</span><select value={settings.theme || "forest"} onChange={(event) => setSettings((current) => ({ ...current, theme: event.target.value }))}><option value="forest">Oliva & latón</option><option value="midnight">Medianoche & champagne</option><option value="sand">Travertino & nogal</option></select></label>
  ...
  function ReservationDrawer({ reservation }) {
    const room = rooms.find((item) => String(item.id) === String(reservation.habitacion_id))
    const balance = balanceFor(reservation)

    return (
      <div className="drawerShade" onClick={() => setSelectedReservationId(null)}>
        <aside className="drawer" onClick={(event) => event.stopPropagation()}>
          <button className="drawerClose" onClick={() => setSelectedReservationId(null)}>×</button>
          <div className="drawerHero">
            <small>{reservation.numero_reserva || `RESERVA #${reservation.id}`}</small>
            <h2>{reservation.nombre_huesped}</h2>
            <p>{room?.nombre} · {room?.tipo} · {reservation.canal_reserva || "Directa"}</p>
            <div className={`statusPill ${statusMeta(reservation.estado)[1]}`}>{statusMeta(reservation.estado)[0]}</div>
          </div>

          <div className="stayCard">
            <div><small>ENTRADA</small><b>{shortDate(reservation.fecha_entrada)}</b></div><i>→</i>
            <div><small>SALIDA</small><b>{shortDate(reservation.fecha_salida)}</b></div>
            <div><small>NOCHES</small><b>{reservation.noches || diffDays(reservation.fecha_entrada, reservation.fecha_salida)}</b></div>
          </div>

          <div className="drawerMoney">
            <div><small>Total</small><b>{money(reservation.precio_total)}</b></div>
            <div><small>Pagado</small><b>{money(paidFor(reservation))}</b></div>
            <div className={balance > 0 ? "due" : ""}><small>Saldo</small><b>{money(balance)}</b></div>
          </div>

          <div className="drawerActions">
            {normalize(reservation.estado) !== "alojado" && normalize(reservation.estado) !== "finalizada" && <button onClick={() => checkIn(reservation)}>✓ Check-in</button>}
            {normalize(reservation.estado) === "alojado" && <button onClick={() => checkout(reservation)} disabled={balance > 0}>✓ Check-out{balance > 0 ? " · saldo pendiente" : ""}</button>}
            <button onClick={() => resizeReservation(reservation, 1)}>＋ 1 noche</button>
            <button onClick={() => resizeReservation(reservation, -1)}>− 1 noche</button>
          </div>

          <section><small>NOTAS DE HOSPITALIDAD</small><p>{reservation.notas || "Sin observaciones cargadas."}</p></section>

          <form className="paymentBox" onSubmit={registerPayment}>
            <small>REGISTRAR PAGO</small>
            <div>
              <input type="number" min="0" step="0.01" value={paymentForm.amount} onChange={(event) => setPaymentForm((form) => ({ ...form, amount: event.target.value }))} placeholder={balance ? String(Math.round(balance)) : "Monto"} />
              <select value={paymentForm.method} onChange={(event) => setPaymentForm((form) => ({ ...form, method: event.target.value }))}><option>Efectivo</option><option>Tarjeta</option><option>Transferencia</option><option>Mercado Pago</option></select>
              <button>Registrar</button>
            </div>
          </form>

          <section className="splitBox">
            <small>SPLIT STAY</small><p>Dividí la estadía y continuá en otra habitación sin perder la trazabilidad.</p>
            <div>
              <input type="date" min={addDays(reservation.fecha_entrada, 1)} max={addDays(reservation.fecha_salida, -1)} value={splitForm.date} onChange={(event) => setSplitForm((form) => ({ ...form, date: event.target.value }))} />
              <select value={splitForm.roomId} onChange={(event) => setSplitForm((form) => ({ ...form, roomId: event.target.value }))}>
                <option value="">Habitación destino</option>
                {rooms.filter((item) => String(item.id) !== String(reservation.habitacion_id) && item.activa !== false).map((item) => <option value={item.id} key={item.id}>{item.nombre} · {item.tipo}</option>)}
              </select>
              <button type="button" onClick={splitReservation}>Dividir</button>
            </div>
          </section>
        </aside>
      </div>
  ...
  function RoomDrawer({ room }) {
    const current = todayOccupied.find((reservation) => String(reservation.habitacion_id) === String(room.id))
    const next = activeReservations
      .filter((reservation) => String(reservation.habitacion_id) === String(room.id) && reservation.fecha_entrada > today)
      .sort((a, b) => a.fecha_entrada.localeCompare(b.fecha_entrada))[0]

    return (
      <div className="drawerShade" onClick={() => setSelectedRoomId(null)}>
        <aside className="roomDrawer drawer" onClick={(event) => event.stopPropagation()}>
          <button className="drawerClose" onClick={() => setSelectedRoomId(null)}>×</button>
          <small>HABITACIÓN</small><h2>{room.nombre}</h2><p>{room.tipo || "Sin tipo"} · {money(room.precio)} / noche</p>
          <div className={`roomStateHero ${roomMeta(room.estado)[1]}`}><span>{roomMeta(room.estado)[0]}</span><small>estado operativo</small></div>
          {current && <section><small>AHORA</small><button className="guestLink" onClick={() => { setSelectedRoomId(null); setSelectedReservationId(current.id) }}>{current.nombre_huesped} →</button></section>}
          {next && <section><small>PRÓXIMA LLEGADA</small><p><b>{next.nombre_huesped}</b><br />{shortDate(next.fecha_entrada)}</p></section>}
          <section><small>HOUSEKEEPING</small><div className="roomStateButtons">{[["limpia", "Lista"], ["sucia", "Sucia"], ["inspeccion", "Inspección"], ["mantenimiento", "Mantenimiento"]].map(([value, label]) => <button key={value} onClick={() => setRoomStatus(room.id, value)}>{label}</button>)}</div></section>
        </aside>
      </div>
    )
  }
  ...
  function NewReservation() {
    return (
      <div className="modalShade" onClick={() => setNewOpen(false)}>
        <form className="newModal" onSubmit={createReservation} onClick={(event) => event.stopPropagation()}>
          <button type="button" className="drawerClose" onClick={() => setNewOpen(false)}>×</button>
          <small>NUEVA RESERVA</small><h2>Una reserva debería tomar segundos.</h2>
          <div className="newGrid">
            <label><span>Huésped</span><input autoFocus value={newForm.guest} onChange={(event) => setNewForm((form) => ({ ...form, guest: event.target.value }))} /></label>
            <label><span>Habitación</span><select value={newForm.roomId} onChange={(event) => setNewForm((form) => ({ ...form, roomId: event.target.value }))}><option value="">Elegir habitación</option>{rooms.filter((room) => room.activa !== false && normalize(room.estado) !== "mantenimiento").map((room) => <option key={room.id} value={room.id}>{room.nombre} · {room.tipo} · {money(room.precio)}</option>)}</select></label>
            <label><span>Entrada</span><input type="date" value={newForm.start} onChange={(event) => setNewForm((form) => ({ ...form, start: event.target.value, end: form.end <= event.target.value ? addDays(event.target.value, 2) : form.end }))} /></label>
            <label><span>Salida</span><input type="date" min={addDays(newForm.start || today, 1)} value={newForm.end} onChange={(event) => setNewForm((form) => ({ ...form, end: event.target.value }))} /></label>
            <label><span>Email</span><input type="email" value={newForm.email} onChange={(event) => setNewForm((form) => ({ ...form, email: event.target.value }))} /></label>
            <label><span>Teléfono</span><input value={newForm.phone} onChange={(event) => setNewForm((form) => ({ ...form, phone: event.target.value }))} /></label>
            <label><span>Huéspedes</span><input type="number" min="1" value={newForm.pax} onChange={(event) => setNewForm((form) => ({ ...form, pax: event.target.value }))} /></label>
            <label><span>Canal</span><select value={newForm.channel} onChange={(event) => setNewForm((form) => ({ ...form, channel: event.target.value }))}><option>Directa</option><option>Booking.com</option><option>Expedia</option><option>Airbnb</option><option>Agoda</option><option>Teléfono</option><option>Walk-in</option></select></label>
            <label className="wide"><span>Notas</span><textarea value={newForm.notes} onChange={(event) => setNewForm((form) => ({ ...form, notes: event.target.value }))} placeholder="Preferencias, llegada, cuna, cochera…" /></label>
          </div>
          {newForm.roomId && newForm.start && newForm.end > newForm.start && <div className="rateSummary"><span>{diffDays(newForm.start, newForm.end)} noches</span><b>{money(Number(rooms.find((room) => String(room.id) === String(newForm.roomId))?.precio || 0) * diffDays(newForm.start, newForm.end))}</b></div>}
          <button className="primary submit">Crear reserva</button>
        </form>
      </div>
    )
  }
