...
import StaticCaretGuard from"../StaticCaretGuard"

const HOTEL_IMAGE="https://images.unsplash.com/photo-1564501049412-61c2a3083791?auto=format&fit=crop&w=1800&q=88"

export default function HospitalityShell({eyebrow="HABITACIÓN LLENA",title,copy,children,hotelName="Habitación Llena",productLabel="Hotel OS",sceneEyebrow="HOSPITALITY OPERATING SYSTEM",compact=false,backHref=null,backOnScene=false,hideBrand=false,topAligned=false}){
  return <><StaticCaretGuard/><main className={`${ui.page} ${compact?ui.compact:""} ${topAligned?ui.topAligned:""}`}>
    ...
    <section className={ui.frame}>
      ...
      <aside className={ui.scene} style={{backgroundImage:`linear-gradient(180deg,rgba(11,28,22,.08),rgba(8,22,17,.78)),url(${HOTEL_IMAGE})`}}>
        ...
        <div className={ui.sceneCopy}><small>{sceneEyebrow}</small><h2>La tecnología también puede sentirse <em>hospitalaria.</em></h2><p>Un escritorio sereno, cálido y preciso para que el equipo vuelva a mirar al huésped.</p></div>
      </aside>
      <section className={ui.surface}>
        ...
        <div className={`${ui.content} ${hideBrand?ui.brandless:""}`}>
          ...
          {!hideBrand&&<a href="/" className={ui.brand}><span>HL</span><div><b>{hotelName}</b><small>{productLabel}</small></div></a>}
