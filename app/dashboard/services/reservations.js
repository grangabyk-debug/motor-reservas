...
export async function checkReservationAvailability({propertyId,roomId,draft,excludeReservationId=null}){
  ...
  const[{data:reservations,error:reservationError},{data:blocks,error:blockError}]=await Promise.all([
    ...
    supabase.from("bloqueos").select("id,habitacion_id,fecha_desde,fecha_hasta,motivo,detalle").eq("property_id",pid).eq("habitacion_id",rid).lt("fecha_desde",blockQueryEnd).gt("fecha_hasta",draft.start),
  ])
  if(reservationError)throw reservationError
  if(blockError)throw blockError
  const conflict=(reservations||[]).find(r=>String(r.id)!==String(excludeReservationId||"")&&roomInReservation(r,rid))
  if(conflict){const who=text(conflict.nombre_huesped)||"otro huésped",code=conflict.numero_reserva?` · ${conflict.numero_reserva}`:"",from=humanMoment(conflict.ocupacion_desde_local||`${conflict.fecha_entrada}T${conflict.hora_llegada_estimada||"14:00"}`),to=humanMoment(conflict.ocupacion_hasta_local||`${conflict.fecha_salida}T${conflict.hora_salida_estimada||"10:00"}`);return{available:false,type:"reservation",reservation:conflict,message:`Esa habitación ya está ocupada por ${who}${code} desde ${from} hasta ${to}. Cambiá la habitación o el horario antes de guardar.`}}
  const block=(blocks||[]).find(item=>occupancy.startAt<`${item.fecha_hasta}T00:00:00`&&occupancy.endAt>`${item.fecha_desde}T00:00:00`)
  if(block){const reason=text(block.motivo||block.detalle)||"bloqueo operativo";return{available:false,type:"block",block,message:`Esa habitación está bloqueada del ${block.fecha_desde} al ${block.fecha_hasta} (${reason}). Cambiá la habitación o el horario antes de guardar.`}}
...
export async function saveReservation({draft,room,propertyId,userId,original}){
  ...
  const availability=await checkReservationAvailability({propertyId:pid,roomId:room.id,draft,excludeReservationId:draft.id||null})
