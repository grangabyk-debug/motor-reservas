...
export async function saveReservation({draft,room,propertyId,userId,original}){
  const data=payload(draft,room,propertyId,userId),pending=draft.pendingDocuments||[],stagedPayments=draft.id?[]:(draft.initialPayments||[]).filter(item=>Number(item.amount)>0&&text(item.method)),pid=requirePropertyId(propertyId);let saved
  if(draft.id){
