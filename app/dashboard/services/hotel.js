...
function localIntelligence(question,context={}){const q=String(question||"").toLowerCase(),reservations=Array.isArray(context.reservas)?context.reservas:[],rooms=Array.isArray(context.habitaciones)?context.habitaciones:[],today=context.hoy,active=rooms.filter(r=>r.activa!==false),occupied=reservations.filter(r=>r.estado!=="cancelada"&&r.entrada<=today&&r.salida>today),arrivals=reservations.filter(r=>r.estado!=="cancelada"&&r.entrada===today),departures=reservations.filter(r=>r.estado!=="cancelada"&&r.salida===today);if(q.includes("ocup"))return`Hoy veo ${occupied.length} habitación(es) ocupada(s) sobre ${active.length} activa(s).`;if(q.includes("lleg")||q.includes("atención")||q.includes("atencion"))return`Hoy hay ${arrivals.length} llegada(s) y ${departures.length} salida(s). Revisá primero habitaciones con rotación el mismo día o estados de limpieza pendientes.`;if(q.includes("salid")||q.includes("out"))return`Hoy hay ${departures.length} salida(s) registradas.`;if(q.includes("habit"))return`Hay ${active.length} habitación(es) activa(s) en esta propiedad.`;if(q.includes("reserv"))return`El contexto actual contiene ${reservations.length} reserva(s) disponibles para analizar.`;return"Estoy disponible. Puedo ayudarte con ocupación, llegadas, salidas, reservas, habitaciones y prioridades operativas usando el contexto actual del hotel."}
