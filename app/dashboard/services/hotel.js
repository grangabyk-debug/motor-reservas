...
export async function sendReservationEmail({reservationId}){
  const{data:{session}}=await supabase.auth.getSession();if(!session?.access_token)throw new Error("La sesión expiró.")
...
export async function askIntelligence({question,context}){const response=await fetch("/api/assistant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question,context})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data?.error||"No se pudo consultar Llena Intelligence.");return data}
