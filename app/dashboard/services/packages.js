...
export async function applyPackageToReservation({propertyId,reservationId,packageId}){
  ...
  if(pack.room_type){const{data:room,error:roomError}=await supabase.from("habitaciones").select("id,nombre,tipo").eq("id",reservation.habitacion_id).eq("property_id",property).single();if(roomError)throw roomError;if(clean(room.tipo).toLowerCase()!==clean(pack.room_type).toLowerCase())throw new Error(`El pack está configurado para ${pack.room_type}. La reserva está en ${room.tipo||room.nombre}. Cambiá la habitación y volvé a aplicarlo.`)}

  const currentRate=Number(reservation.tarifa_noche||0),currentStay=currentRate*nights,price=Number(pack.price||0)
