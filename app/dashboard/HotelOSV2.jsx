...
import{canViewModule}from"./core/modules"
...
export default function HotelOSV2(){
  ...
  const notify=message=>{setToast(String(message));setTimeout(()=>setToast(""),3400)}
  const changeView=id=>{if(!canView(id)){notify("Este módulo no está incluido en la suscripción de este hotel.");return false}setView(id);setMobile(false);setNotificationsOpen(false);return true}
  ...
  useEffect(()=>{if(!canView(view))setView("lobby")},[view,moduleAccess])
  ...
  const hotelName=settings.hotel_name||currentProperty?.hotel_name||currentProperty?.name||"Hotel",hotelLogo=settings.logo_data_url||currentProperty?.logo_data_url||"",newReservationAction=allowed("frontdesk.reservations.edit")&&canView("reservations")?()=>newReservation():null
  return <div className={ui.shell}>
    <HotelSidebar view={view} onView={changeView} hotelName={hotelName} hotelLogo={hotelLogo} role={role} properties={session.properties} propertyId={session.propertyId} onPropertyChange={session.setPropertyId} onLogout={session.logout} onNewReservation={newReservationAction} mobileOpen={mobile} canView={canView}/>
    ...
    <CommandPalette open={commandOpen} onClose={()=>setCommandOpen(false)} reservations={data.reservations} rooms={activeRooms} guests={data.guests} onNavigate={changeView} onOpenReservation={openReservation} onNewReservation={newReservationAction} canView={canView}/>
