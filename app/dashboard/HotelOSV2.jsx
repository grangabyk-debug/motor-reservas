...
const TITLES={lobby:"Dashboard",calendar:"Planning",reservations:"Reservas",guests:"Huéspedes",keys:"Puertas",rooms:"Habitaciones",housekeeping:"Rack de limpiezas",maintenance:"Última actividad",resources:"Recursos",twin:"Digital Twin",rates:"Tarifas",packages:"Packs & Promos",partners:"Empresas & Agencias",groups:"Grupos",upselling:"Upselling",distribution:"Distribución",cash:"Caja & Folios",billing:"Facturación",reports:"Informes",team:"Equipo & Roles",automations:"Automatizaciones",intelligence:"Llena Intelligence",integrations:"Integraciones",settings:"Configuración",support:"Ayuda"}
...
export default function HotelOSV2(){
  ...
  const notify=message=>{setToast(String(message));setTimeout(()=>setToast(""),3400)},changeView=id=>{setView(id);setMobile(false)}
  ...
  return <div className={ui.shell}>
    ...
    <main className={ui.work}>
      {view!=="lobby"&&<HotelTopbar view={view} title={TITLES[view]||VIEW_META[view]?.label||"Hotel"} onNewReservation={view==="support"?null:newReservationAction} onMenu={()=>setMobile(v=>!v)} onCommand={()=>setCommandOpen(true)} onSupport={()=>changeView("support")} onSettings={()=>changeView("settings")}/>} 
