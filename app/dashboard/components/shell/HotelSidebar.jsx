...
export default function HotelSidebar({view,onView,hotelName="Habitación Llena",role="reception",properties=[],propertyId,onPropertyChange,onLogout,mobileOpen=false}){
  ...
  return <aside className={`${ui.sidebar} ${mobileOpen?ui.open:""}`}>
    <div className={ui.leather}/><button className={ui.brand} onClick={()=>onView("lobby")}><span>HL</span><div><b>{hotelName}</b><small>Hospitality Operating System</small></div></button>
    ...
    <nav className={ui.categories}>{HOTEL_NAVIGATION.map(group=><section key={group.id}><button className={`${ui.category} ${activeGroup===group.id?ui.categoryActive:""}`} onClick={()=>setExpanded(open===group.id?"":group.id)}><i>{group.icon}</i><span><b>{group.label}</b><small>{group.description}</small></span><em>{open===group.id?"−":"+"}</em></button>{open===group.id&&<div className={ui.children}>{group.items.map(([id,label,icon])=><button key={id} className={view===id?ui.active:""} onClick={()=>onView(id)}><i>{icon}</i><span>{label}</span></button>)}</div>}</section>)}</nav>
