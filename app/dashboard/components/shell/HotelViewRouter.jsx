...
export default function HotelViewRouter({view,data,session,settings,permissions,role,live,committed,activeRooms,packages,search,setSearch,allowed,action,changeView,openReservation,openReservationTab,newReservation,newReservationAction,move,resize,quickCheckin,quickCheckout,blockRange,setBlockDraft,onMenu,onCommand,onNotifications,notificationCount=0,hotelName}){
  const today=isoDate()
  if(view==="lobby")return <><LobbyTentativeStrip reservations={live} rooms={data.rooms} onOpen={openReservation}/><DashboardExperience settings={settings} rooms={data.rooms} reservations={committed} payments={data.payments} housekeeping={data.housekeepingTasks||data.operations.housekeeping||[]} maintenance={data.maintenanceTickets||data.operations.maintenance||[]} automationEvents={data.automationEvents||[]} inboxConversations={data.inboxConversations||[]} onView={changeView} onOpen={openReservation} onCheckin={quickCheckin} onCheckout={quickCheckout} search={search} onSearch={setSearch} onNewReservation={newReservationAction} onMenu={onMenu} onCommand={onCommand} onNotifications={onNotifications} notificationCount={notificationCount} userId={session.user?.id}/></>
  ...
  if(view==="calendar")return <CommandCenter rooms={activeRooms} reservations={live} payments={data.payments} blocks={data.blocks} floors={data.floors} onMove={move} onResize={resize} onOpen={openReservation} onOpenExternal={openReservationTab} onNew={(room,day)=>newReservation({roomId:room.id,start:day,end:addDays(day,1)})} onNewRange={({roomIds,start,end,tentative})=>newReservation({roomIds,start,end,tentative})} onBlockRange={blockRange} onBlock={(room,day)=>setBlockDraft({roomId:String(room.id),start:day,end:addDays(day,1),reason:"Bloqueo operativo",detail:""})}/>
  ...
  if(view==="messages")return <GuestInbox propertyId={session.propertyId} conversations={data.inboxConversations||[]}/>
  ...
  if(view==="analytics")return <AnalyticsOverview rooms={data.rooms} reservations={data.reservations} payments={data.payments} blocks={data.blocks} settings={settings} snapshots={data.analytics?.snapshots||[]} channelCosts={data.analytics?.channelCosts||[]} canManageChannelCosts={role==="owner"||role==="manager"} onSaveChannelCost={draft=>action(()=>saveChannelCost({propertyId:session.propertyId,draft}),"Costo de canal guardado.")}/>
