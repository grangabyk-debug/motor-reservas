...
export default function NotificationCenter({open,onClose,data,onOpenReservation,onView,onOpenMessages}){
  ...
  return <>{alert}<div className={s.shade} onMouseDown={e=>e.target===e.currentTarget&&onClose?.()}>
    <aside className={s.panel}>
      ...
      <footer><span>{realtimeLabel}</span><button type="button" className={s.soundToggle} onClick={toggleSound}>{otaSoundEnabled?"🔊 Sonido OTA activado":"🔇 Sonido OTA desactivado"}</button><button type="button" className={s.soundToggle} onClick={testSound}>{soundTestStatus==="ok"?"✓ Sonido reproducido":soundTestStatus==="blocked"?"⚠ Audio bloqueado":"▶ Probar sonido"}</button><button type="button" onClick={()=>onView?.("automations")}>Ver automatizaciones</button></footer>
    </aside>
  </div></>
}
