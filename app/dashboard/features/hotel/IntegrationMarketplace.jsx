...
export default function IntegrationMarketplace({settings,channels=[]}){
  const ops=operational(settings),region=commerceRegion(ops),propertyId=settings?.property_id||"hotel",storageKey=`hl-onboarding-${propertyId}`,[group,setGroup]=useState("Todos"),[done,setDone]=useState([])
  ...
  return <div className={ui.content}>
    ...
    <section className={s.grid}>{cards.map(card=><article key={card.id} className={s.card}><header><span>{initials(card.name)}</span><div><small>{card.group}</small><h3>{card.name}</h3></div><em className={s[card.state]}>{stateText(card.state)}</em></header><p>{card.desc}</p><footer><span>{card.region}</span><b>{card.kind==="native"?"Nativo":card.kind==="local"?"Regional":card.kind==="hardware"?"Hardware":"Integración"}</b></footer></article>)}</section>
    ...
    <section className={s.runtime}><div><small>CONFIGURACIÓN ACTUAL · {region.name.toUpperCase()}</small><h3>{region.currency} · {region.locale} · {region.timezone}</h3></div><span className={email.mode==="api"?s.ok:s.pending}>Email {email.mode==="api"?"servidor":"por configurar"}</span><span className={encoder.enabled&&encoder.bridge_url?s.ok:s.pending}>Llaves {encoder.enabled&&encoder.bridge_url?"con bridge":"por configurar"}</span><span className={motorConnected?s.ok:s.pending}>Motor {motorConnected?"conectado":"disponible"}</span></section>
  </div>
}
