...
export default function DashboardTodayRail({arrivals=[],departures=[],inhouse=[],rooms=[],paid=new Map(),occupancy=0,onView,onOpen,onNewReservation}){
  ...
  return <section className={rail.shell} aria-label="Operación de hoy">
    ...
    <div className={rail.actions}><button type="button" onClick={()=>onView?.("calendar")}>Planning</button><button type="button" onClick={()=>onView?.("housekeeping")}>HK</button>{onNewReservation&&<button type="button" className={rail.primary} onClick={onNewReservation}>＋ Reserva</button>}</div>
  </section>
}
