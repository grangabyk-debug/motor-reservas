...
function Trend({values=[]}){
  const width=760,height=210,pad=10,max=Math.max(100,...values),step=(width-pad*2)/Math.max(1,values.length-1),y=v=>height-pad-(v/max)*(height-pad*2),points=values.map((v,i)=>`${pad+i*step},${y(v)}`).join(" "),area=values.length?`M ${pad} ${height-pad} L ${values.map((v,i)=>`${pad+i*step} ${y(v)}`).join(" L ")} L ${pad+(values.length-1)*step} ${height-pad} Z`:""
  return <svg className={s.trend} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="hlSimpleFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#2b67b1" stopOpacity=".16"/><stop offset="1" stopColor="#2b67b1" stopOpacity="0"/></linearGradient></defs><path d={area} fill="url(#hlSimpleFill)"/><polyline points={points} fill="none" stroke="#2b67b1" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
}
...
export default function DashboardExperience(props){
  ...
  return <main className={s.simple}>
    <header className={s.header}>
      ...
      <div className={s.headerActions}><button className={s.search} onClick={onCommand}>⌕ <span>Buscar</span></button><div className={s.modeSwitch}><button className={s.modeActive}>Simple</button><button onClick={()=>setMode("advanced")}>Avanzada</button></div>{onNewReservation&&<button className={s.newReservation} onClick={onNewReservation}>＋ Nueva reserva</button>}</div>
    </header>
