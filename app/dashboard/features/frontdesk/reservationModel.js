...
export function reservationToDraft(r){
  const primary=String(r.habitacion_id||""),details=safeJson(r.habitaciones_detalle,[]),ids=[...new Set([...(Array.isArray(r.habitaciones_ids)?r.habitaciones_ids:[]),r.habitacion_id].map(String).filter(Boolean))],detailMap=new Map((Array.isArray(details)?details:[]).map(item=>[String(item?.habitacion_id||item?.roomId||""),item]))
  const additionalRooms=ids.filter(id=>id!==primary).map(id=>{const item=detailMap.get(id)||{};return{roomId:id,rate:item.tarifa_noche==null?"":Number(item.tarifa_noche),name:item.nombre||"",type:item.tipo||""}})
