...
const span=r=>Math.max(0,Math.round((new Date(`${r.fecha_salida}T12:00:00Z`)-new Date(`${r.fecha_entrada}T12:00:00Z`))/86400000))
...
const channelColor=value=>{const v=String(value||"").toLowerCase();if(v.includes("booking"))return"#2f58bf";if(v.includes("airbnb"))return"#d85a68";if(v.includes("expedia"))return"#cf991d";if(v.includes("agencia"))return"#6d59b2";return"#42a965"}
...
function ReservationBlock({r,days,onSelect,onDragStart,onResizeStart,onDragEnd,selected}){
  ...
  return <button type="button" draggable onDragStart={e=>onDragStart(e,r)} onDragEnd={onDragEnd} onClick={e=>{e.stopPropagation();onSelect(r)}} className={`${s.stay} ${selected?s.staySelected:""}`} style={{gridColumn:`${startLine}/${Math.max(startLine+1,endLine)}`,"--channel":channelColor(r.canal_reserva)}} title={`${r.nombre_huesped||"Reserva"} · ${shortDate(r.fecha_entrada)} → ${shortDate(r.fecha_salida)}`}>
    <span><b>{r.nombre_huesped||"Reserva"}</b><small>{r.canal_reserva||"Directa"}</small></span><i draggable onMouseDown={e=>e.stopPropagation()} onDragStart={e=>onResizeStart(e,r)} onDragEnd={onDragEnd} title="Arrastrar para cambiar la salida" aria-label="Cambiar fecha de salida"/>
  </button>
}
...
export default function HotelGestPlanning({rooms=[],reservations=[],payments=[],blocks=[],floors=[],onMove,onResize,onOpen,onOpenExternal,onNew,onNewRange,onBlockRange,onBlock}){
  ...
  function beginMove(e,r){e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(r.id));setRange(null);setDrag({kind:"move",reservation:r});setHover(null)}
  function beginResize(e,r){e.stopPropagation();e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(r.id));setRange(null);setDrag({kind:"resize",reservation:r});setHover(null)}
  ...
  function extendSelection(room,day){if(!pointerDown.current||!selecting||String(room.id)!==selecting.anchorRoomId)return;const anchor=selecting.lastDay?selecting.start:day,from=day<selecting.start?day:selecting.start,to=day<selecting.start?addDays(selecting.start,1):addDays(day,1);setSelecting({...selecting,start:from,end:to,lastDay:day})}
