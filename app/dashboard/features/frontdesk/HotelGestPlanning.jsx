...
function validateTarget({kind,reservation,room,day,reservations,blocks}){
  ...
  if(kind==="move"&&String(room.id)===String(reservation.habitacion_id)&&targetStart===reservation.fecha_entrada)return{ok:false,reason:"La reserva ya está en esa posición."}
  if(kind==="resize"&&targetEnd===reservation.fecha_salida)return{ok:false,reason:"La salida ya está en esa fecha."}
...
export default function HotelGestPlanning({rooms=[],reservations=[],payments=[],blocks=[],floors=[],onMove,onResize,onOpen,onOpenExternal,onNew,onBlock}){
  ...
  function beginMove(e,r){const rect=e.currentTarget.getBoundingClientRect(),duration=Math.max(1,span(r)),ratio=rect.width?Math.max(0,Math.min(.999,(e.clientX-rect.left)/rect.width)):0,grabOffset=Math.min(duration-1,Math.floor(ratio*duration));e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(r.id));setDrag({kind:"move",reservation:r,grabOffset});setHover(null)}
  function beginResize(e,r){e.stopPropagation();e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(r.id));setDrag({kind:"resize",reservation:r,grabOffset:0});setHover(null)}
  function endDrag(){if(hover&&!hover.ok&&hover.reason!=="La reserva ya está en esa posición."&&hover.reason!=="La salida ya está en esa fecha.")flash("error",hover.reason);setDrag(null);setHover(null)}
  function targetFor(room,cellDay){const effectiveDay=drag?.kind==="move"?addDays(cellDay,-Number(drag?.grabOffset||0)):cellDay;return validateTarget({kind:drag?.kind,reservation:drag?.reservation,room,day:effectiveDay,reservations,blocks})}
  ...
  async function dropTarget(e,room,day){if(!drag)return;const current=drag,result=targetFor(room,day);setHover(null);if(!result.ok){if(!result.reason.includes("ya está"))flash("error",result.reason);setDrag(null);return}e.preventDefault();try{const saved=current.kind==="resize"?await onResize?.(current.reservation.id,result.targetEnd):await onMove?.(current.reservation.id,room.id,result.targetStart);if(saved)flash("success",current.kind==="resize"?`Salida actualizada al ${compactDate(result.targetEnd)}.`:`${current.reservation.nombre_huesped||"Reserva"} movida a ${room.nombre}.`)}catch(error){flash("error",error?.message||"No se pudo completar el movimiento.")}finally{setDrag(null)}}
  async function tapTarget(room,day){if(!moveMode)return;const r=reservations.find(x=>String(x.id)===String(moveMode));if(!r)return;const result=validateTarget({kind:"move",reservation:r,room,day,reservations,blocks});if(!result.ok){if(!result.reason.includes("ya está"))flash("error",result.reason);return}try{const saved=await onMove?.(r.id,room.id,result.targetStart);if(saved){setMoveMode("");flash("success",`${r.nombre_huesped||"Reserva"} movida a ${room.nombre}.`)}}catch(error){flash("error",error?.message||"No se pudo mover la reserva.")}}
