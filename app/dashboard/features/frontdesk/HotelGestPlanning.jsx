...
function validateTarget({kind,reservation,room,day,reservations,blocks}){
  ...
  const targetEnd=kind==="resize"?addDays(day,1):addDays(day,span(reservation))
  if(targetEnd<targetStart)return{ok:false,reason:"La salida no puede quedar antes de la entrada."}
  if(String(reservation.tipo_estadia||"")==="day_use"&&targetEnd!==targetStart)return{ok:false,reason:"Un Day Use debe comenzar y terminar el mismo día."}
  const blocked=(blocks||[]).find(b=>String(b.habitacion_id)===String(room.id)&&overlap(targetStart,targetEnd,b.fecha_desde,b.fecha_hasta))
  if(blocked)return{ok:false,reason:`Bloqueada: ${blocked.motivo||blocked.detalle||"bloqueo operativo"}`}
  const conflict=(reservations||[]).find(r=>active(r)&&String(r.id)!==String(reservation.id)&&uses(r,room.id)&&overlap(targetStart,targetEnd,r.fecha_entrada,r.fecha_salida))
...
function ReservationBlock({r,room,days,onSelect,onDragStart,onResizeStart,selected}){
  ...
  return <button type="button" draggable onDragStart={e=>onDragStart(e,r)} onClick={()=>onSelect(r)} className={`${s.stay} ${selected?s.staySelected:""}`} style={{gridColumn:`${startLine}/${Math.max(startLine+1,endLine)}`,"--channel":channelColor(r.canal_reserva)}} title={`${r.nombre_huesped||"Reserva"} · ${shortDate(r.fecha_entrada)} → ${shortDate(r.fecha_salida)}`}>
    <span><b>{r.nombre_huesped||"Reserva"}</b><small>{r.canal_reserva||"Directa"}</small></span><i draggable onDragStart={e=>onResizeStart(e,r)} title="Arrastrar para cambiar la salida" aria-label="Cambiar fecha de salida"/>
  </button>
}
...
export default function HotelGestPlanning({rooms=[],reservations=[],payments=[],blocks=[],floors=[],onMove,onResize,onOpen,onOpenExternal,onNew,onBlock}){
  ...
  async function dropTarget(e,room,day){if(!drag)return;const result=targetFor(room,day);setHover(null);if(!result.ok){flash("error",result.reason);setDrag(null);return}e.preventDefault();try{const ok=drag.kind==="resize"?await onResize?.(drag.reservation.id,result.targetEnd):await onMove?.(drag.reservation.id,room.id,result.targetStart);if(ok!==false)flash("success",drag.kind==="resize"?`Salida actualizada al ${compactDate(result.targetEnd)}.`:`${drag.reservation.nombre_huesped||"Reserva"} movida a ${room.nombre}.`)}catch(error){flash("error",error?.message||"No se pudo completar el movimiento.")}finally{setDrag(null)}}
  async function tapTarget(room,day){if(!moveMode)return;const r=reservations.find(x=>String(x.id)===String(moveMode));if(!r)return;const result=validateTarget({kind:"move",reservation:r,room,day,reservations,blocks});if(!result.ok){flash("error",result.reason);return}try{const ok=await onMove?.(r.id,room.id,result.targetStart);if(ok!==false){setMoveMode("");flash("success",`${r.nombre_huesped||"Reserva"} movida a ${room.nombre}.`)}}catch(error){flash("error",error?.message||"No se pudo mover la reserva.")}}
