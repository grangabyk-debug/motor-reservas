...
const quickChannels=["Directa","WhatsApp","Teléfono","Walk-in"]
...
function shortStay(start,end){if(!start&&!end)return"Fechas pendientes";if(start&&!end)return`Desde ${start}`;return`${start} → ${end}`}
...
export default function ReservationCreateWizard({initial,rooms=[],busy,onClose,onSave}){
  ...
  const canSubmit=step===3&&!busy&&!availability.loading
  ...
  useEffect(()=>{
    const handle=e=>{
      ...
      if((e.metaKey||e.ctrlKey)&&e.key==="Enter"&&canSubmit){e.preventDefault();formRef.current?.requestSubmit();return}
      if(e.key!=="Enter"||e.shiftKey||e.metaKey||e.ctrlKey||step>=3)return
  ...
  },[step,busy,canSubmit,draft,availability.signature])
  const availableCount=availability.ready?availability.rooms.length:0
  const pulseTitle=draft.guest?.trim()||"Reserva nueva"
  return <div className={s.shade} onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <form ref={formRef} className={s.panel} onSubmit={submit}>
      ...
      <main className={s.body}>
        <aside className={s.liveSummary} aria-label="Resumen en vivo de la reserva">
          <div className={s.liveOrb}><span/></div>
          <div className={s.liveCopy}><small>RESERVA EN VIVO</small><b>{pulseTitle}</b><span>{shortStay(draft.start,draft.end)} · {selectedIds.length||0} {selectedIds.length===1?"habitación":"habitaciones"}</span></div>
          <div className={s.liveTotal}><small>TOTAL</small><strong>{money(totals.total||0,draft.currency||"ARS")}</strong></div>
          <span className={`${s.intentBadge} ${draft._tentative?s.intentTentative:s.intentConfirmed}`}>{draft._tentative?"Tentativa":"Confirmada"}</span>
        </aside>
        ...
        {step===3&&<section className={s.section}><div className={s.title}><small>PASO 4</small><h3>Confirmá la reserva</h3><p>Origen, estado, tarifa y observaciones quedan vinculados a la ficha y al Planning.</p></div><div className={s.intentPicker}><button type="button" className={!draft._tentative?s.intentActive:""} onClick={()=>change("_tentative",false)}><span>✓</span><div><b>Confirmada</b><small>Bloquea inventario y queda operativa.</small></div></button><button type="button" className={draft._tentative?s.intentActive:""} onClick={()=>change("_tentative",true)}><span>◌</span><div><b>Tentativa</b><small>Seguimiento comercial antes de confirmar.</small></div></button></div><div className={s.quickChannels}>{quickChannels.map(item=><button type="button" key={item} className={(draft.channel||"Directa")===item?s.quickChannelActive:""} onClick={()=>change("channel",item)}>{item}</button>)}</div><div className={s.fields}><label><span>Canal</span><select value={draft.channel||"Directa"} onChange={e=>change("channel",e.target.value)}>{channels.map(x=><option key={x}>{x}</option>)}</select></label><label><span>Tarifa por noche</span><input type="number" min="0" value={draft.rate===""?(room?.precio||0):draft.rate} onChange={e=>change("rate",e.target.value)}/></label><label><span>Moneda</span><select value={draft.currency||"ARS"} onChange={e=>change("currency",e.target.value)}><option>ARS</option><option>USD</option></select></label><label><span>Pago preferido</span><select value={draft.preferredPayment||""} onChange={e=>change("preferredPayment",e.target.value)}><option value="">Sin definir</option><option>Efectivo</option><option>Transferencia</option><option>Mercado Pago</option><option>Tarjeta</option></select></label><label className={s.wide}><span>Notas</span><textarea rows="4" value={draft.notes||""} onChange={e=>change("notes",e.target.value)} placeholder="Pedido especial, horario, observaciones..."/></label></div><div className={s.totalBox}><div><small>ESTADÍA</small><b>{selectedIds.length} hab. · {nights||1} {draft.stayType==="day_use"?"uso":"noches"}</b></div><div><small>TOTAL ESTIMADO</small><strong>{money(totals.total||0,draft.currency||"ARS")}</strong></div>{draft._tentative&&<span>Reserva tentativa</span>}</div><div className={s.confirmStrip}><span>✓</span><div><b>Impacto inmediato</b><small>Al crearla se guarda en el PMS y aparece en el Planning después de la sincronización.</small></div></div></section>}
        {error&&<div className={s.error}>{error}</div>}
      </main>
      <footer className={s.footer}><span className={s.keyHint}>{step<3?"Enter · continuar":"⌘/Ctrl + Enter · crear"} · Esc · cerrar</span><button type="button" onClick={step===0?onClose:back}>{step===0?"Cancelar":"Atrás"}</button>{step<3?<button type="button" className={s.primary} onClick={next}>Continuar</button>:<button className={s.primary} disabled={busy||availability.loading}>{busy?"Creando…":draft._tentative?"Crear tentativa":"Crear reserva"}</button>}</footer>
    </form>
  </div>
}
