...
const reservationRoomIds=r=>[...new Set([r?.habitacion_id,...(Array.isArray(r?.habitaciones_ids)?r.habitaciones_ids:[])].map(String).filter(Boolean))]
const reservationUsesRoom=(r,roomId)=>reservationRoomIds(r).includes(String(roomId))
...
export default function AssignmentRack({rooms=[],reservations=[],blocks=[],floors=[],onMove,onOpen,onClose}){
  ...
  const floorRows=useMemo(()=>{
    const activeFloors=floors.filter(f=>f.active!==false).map(f=>({id:String(f.id),name:f.name||`Piso ${f.sort_order||""}`})),known=new Set(activeFloors.map(f=>f.id)),result=activeFloors.map(f=>({...f,rooms:rooms.filter(r=>r.activa!==false&&String(r.floor_id||"")===f.id).sort((a,b)=>String(a.nombre||"").localeCompare(String(b.nombre||""),"es",{numeric:true}))})).filter(f=>f.rooms.length)
    const orphan=rooms.filter(r=>r.activa!==false&&!known.has(String(r.floor_id||""))).sort((a,b)=>String(a.nombre||"").localeCompare(String(b.nombre||""),"es",{numeric:true}));if(orphan.length)result.push({id:"none",name:"Sin piso",rooms:orphan});return result
  },[rooms,floors])

  function roomState(room){
    if(!selected)return{disabled:true,tone:"neutral",label:"—"}
    const roomId=String(room.id),thirdParty=reservations.find(r=>active(r)&&String(r.id)!==String(selected.id)&&reservationUsesRoom(r,roomId)&&overlaps(selected.fecha_entrada,selected.fecha_salida,r.fecha_entrada,r.fecha_salida)),block=blocks.find(b=>String(b.habitacion_id)===roomId&&String(b.fecha_desde||"")<String(selected.fecha_salida||"")&&String(b.fecha_hasta||"")>String(selected.fecha_entrada||"")),state=cleanState(room.estado)
    ...
    if(reservationUsesRoom(selected,roomId))return{disabled:true,tone:"current",label:"Actual",detail:"Ya asignada a esta reserva"}
  ...
  return <section className={styles.wrap} data-assignment-rack="true">
    <header><div><small>RACK DE ASIGNACIONES</small><h3>Colocá las llegadas en su habitación</h3><p>Elegí una llegada y después una habitación. Respeta ocupación, bloqueos y housekeeping.</p></div><div className={styles.headerActions}><label aria-label="Fecha del rack"><input type="date" value={date} onChange={e=>{setDate(e.target.value);setSelectedId("")}}/></label><button type="button" className={allowUpgrade?styles.toggleOn:""} onClick={()=>setAllowUpgrade(v=>!v)}><i/>{allowUpgrade?"Upgrade permitido":"Misma tipología"}</button><button type="button" className={styles.close} onClick={onClose}>×</button></div></header>
    <div className={styles.body}>
      <aside className={styles.arrivals}><div className={styles.sectionTitle}><b>Llegadas</b><span>{arrivals.length}</span></div>{arrivals.length?arrivals.map(r=>{const linked=reservationRoomIds(r).map(id=>rooms.find(x=>String(x.id)===id)).filter(Boolean),roomLabel=linked.length?linked.map(x=>x.nombre).join(", "):"Sin habitación";return <button type="button" key={r.id} className={String(selected?.id)===String(r.id)?styles.arrivalSelected:""} onClick={()=>setSelectedId(String(r.id))} onDoubleClick={()=>onOpen?.(r)}><i>{initials(r.nombre_huesped)}</i><span><b>{r.nombre_huesped||"Sin nombre"}</b><small>{r.cantidad_huespedes||1} pax · {r.hora_llegada_estimada||"Horario pendiente"}</small><em>{roomLabel}{linked.length===1?` · ${linked[0].tipo||""}`:linked.length>1?` · ${linked.length} habitaciones`:""}</em></span></button>}):<div className={styles.empty}>No hay llegadas para {shortDate(date)}.</div>}</aside>
