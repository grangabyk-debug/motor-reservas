...
export default function useReservationDraftSafety({draft,setDraft,tab,setTab,initial,propertyId}){
  ...
  const submitted=useRef(false),mounted=useRef(true)

  useEffect(()=>()=>{mounted.current=false;if(submitted.current&&!initial?.id&&propertyId)clearReservationDraft(propertyId)},[initial?.id,propertyId])
