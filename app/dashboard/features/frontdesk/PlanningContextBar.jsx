...
import GapFillStudio from"./GapFillStudio"
...
export default function PlanningContextBar({rooms=[],reservations=[],blocks=[],onMove,onOpen}){
  ...
  const[groupBlocks,setGroupBlocks]=useState([]),[groups,setGroups]=useState([]),[open,setOpen]=useState(true),[assigning,setAssigning]=useState(false),[assignMessage,setAssignMessage]=useState(""),[insightOpen,setInsightOpen]=useState(""),[copied,setCopied]=useState(""),[gapStudio,setGapStudio]=useState(null)
  ...
  const pulse=useMemo(()=>{
    ...
    const daily=days.map(day=>{const next=addDate(day,1),occupied=new Set(active.filter(r=>occupancyStart(r)<`${next}T00:00:00`&&occupancyEnd(r)>`${day}T00:00:00`).map(r=>String(r.habitacion_id)).filter(Boolean)).size,blocked=new Set((blocks||[]).filter(b=>b.fecha_desde<next&&b.fecha_hasta>day).map(b=>String(b.habitacion_id))).size,total=Math.max(rooms.length,1),pressure=Math.min(100,Math.round((occupied+blocked)/total*100)),arrivals=active.filter(r=>r.fecha_entrada===day).length,departures=active.filter(r=>r.fecha_salida===day).length,free=Math.max(0,total-occupied-blocked);return{day,occupied,blocked,pressure,total,arrivals,departures,free}})
  ...
  return <section className={pc.context}>
    {hasPulse&&<div className={pc.pulse}>
      ...
      <div className={pc.visionRail}>
        <div className={pc.visionLead}><small>HL VISION · 7 DÍAS</small><b>La semana completa en una mirada.</b><span>Ocupación, libres, ingresos y salidas sin recorrer el calendario.</span></div>
        <div className={pc.visionDays}>{pulse.daily.map(item=><button key={item.day} onClick={()=>setInsightOpen("pressure")} style={{"--pressure":`${Math.max(6,item.pressure)}%`}} data-level={item.pressure>=85?"high":item.pressure>=65?"medium":"low"}><div className={pc.visionBar}><i/></div><span><small>{dayLabel(item.day)}</small><b>{item.pressure}%</b><em>{item.free} libres</em></span><footer><strong>IN {item.arrivals}</strong><strong>OUT {item.departures}</strong></footer></button>)}</div>
      </div>
    </div>}
    ...
    {gapStudio&&<GapFillStudio item={gapStudio} onClose={()=>setGapStudio(null)} onOpenReservation={reservation=>{setGapStudio(null);onOpen?.(reservation)}}/>}
  </section>
}
