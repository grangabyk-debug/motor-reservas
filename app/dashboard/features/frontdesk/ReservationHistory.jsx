...
export default function ReservationHistory({propertyId,reservationId,guestProfileId,guestIdentity,rooms=[],compact=false}){
  ...
  useEffect(()=>{let active=true;if(!propertyId||!reservationId)return;setLoading(true);setMessage("");loadReservationHistory({propertyId,reservationId,guestProfileId,guestIdentity}).then(next=>active&&setData(next)).catch(error=>active&&setMessage(error.message||"No se pudo cargar el historial.")).finally(()=>active&&setLoading(false));return()=>{active=false}},[propertyId,reservationId,guestProfileId,guestIdentity?.document,guestIdentity?.email,guestIdentity?.phone])
