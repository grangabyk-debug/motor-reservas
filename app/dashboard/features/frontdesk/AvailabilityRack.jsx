...
export default function AvailabilityRack({rooms=[],reservations=[],blocks=[]}){
  ...
  const types=useMemo(()=>[...new Set(activeRooms.map(roomType))].sort((a,b)=>a.localeCompare(b,"es")),[activeRooms])
  ...
  function statFor(type,day){
    const typeRooms=activeRooms.filter(r=>roomType(r)===type),ids=new Set(typeRooms.map(r=>String(r.id))),unavailable=new Set()
    reservations.filter(r=>reservationOverlapsDay(r,day)).forEach(r=>reservationRoomIds(r).forEach(id=>{if(ids.has(id))unavailable.add(id)}))
    ...
    let groupHold=0
    grouped.forEach((qty,groupId)=>{const assigned=new Set();reservations.filter(r=>reservationOverlapsDay(r,day)&&String(r.group_id||"")===groupId).forEach(r=>reservationRoomIds(r).forEach(id=>{if(ids.has(id))assigned.add(id)}));groupHold+=Math.max(0,qty-assigned.size)})
