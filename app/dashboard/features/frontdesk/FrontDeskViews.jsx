...
export function Lobby({settings,rooms,reservations,payments,onView,onOpen,search,onSearch,onNewReservation,onMenu,onCommand,userId}){
  ...
  useEffect(()=>{let cancelled=false;setPrefsReady(false);const apply=saved=>{if(saved?.order&&Array.isArray(saved.order)){const known=saved.order.filter(id=>DEFAULT_WIDGET_ORDER.includes(id)),missing=DEFAULT_WIDGET_ORDER.filter(id=>!known.includes(id));setWidgetOrder([...known,...missing])}if(Array.isArray(saved?.hidden))setHiddenWidgets(saved.hidden.filter(id=>DEFAULT_WIDGET_ORDER.includes(id)))};try{apply(JSON.parse(localStorage.getItem(storageKey)||"null"))}catch{};(async()=>{if(!propertyId||!userId){if(!cancelled)setPrefsReady(true);return}const{data}=await supabase.from("hotel_user_preferences").select("value").eq("property_id",propertyId).eq("user_id",userId).eq("preference_key","dashboard_layout").maybeSingle();if(!cancelled){if(data?.value)apply(data.value);setPrefsReady(true)}})();return()=>{cancelled=true}},[storageKey,propertyId,userId])
  useEffect(()=>{const payload={order:widgetOrder,hidden:hiddenWidgets};try{localStorage.setItem(storageKey,JSON.stringify(payload))}catch{}if(!prefsReady||!propertyId||!userId)return;const timer=setTimeout(()=>{supabase.from("hotel_user_preferences").upsert({property_id:propertyId,user_id:userId,preference_key:"dashboard_layout",value:payload,updated_at:new Date().toISOString()},{onConflict:"property_id,user_id,preference_key"}).then(()=>{})},450);return()=>clearTimeout(timer)},[storageKey,widgetOrder,hiddenWidgets,prefsReady,propertyId,userId])
  ...
  return <div className={`${ui.content} ${polish.dashboard}`}>
    <header className={polish.dashboardTop}><div><button className={polish.mobileMenu} onClick={onMenu}>☰</button><small>HABITACIÓN LLENA · PMS</small><h1>Panel operativo</h1><p>{clockDate}{clockTime?` · ${clockTime}`:""} · {settings?.hotel_name||"Hotel"}</p></div><div className={polish.topActions}><button type="button" className={polish.searchBox} onClick={onCommand}><span>⌕</span><input readOnly value="" placeholder="Buscar en todo el PMS…" tabIndex={-1}/><button type="button" tabIndex={-1}>⌘K</button></button><button className={`${polish.customizeButton} ${editing?polish.customizeButtonActive:""}`} onClick={()=>setEditing(v=>!v)}>⌘ Personalizar</button>{onNewReservation&&<button className={polish.primaryAction} onClick={onNewReservation}>＋ Nueva reserva</button>}</div></header>
