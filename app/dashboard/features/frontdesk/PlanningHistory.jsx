...
import{loadPlanningHistory,undoPlanningHistory}from"../../services/planningWorkspace"
...
export default function PlanningHistory({propertyId,rooms=[]}){
  ...
  async function load(){if(!propertyId)return;setLoading(true);try{setRows(await loadPlanningHistory(propertyId))}catch(error){setMessage(error.message||"No se pudo cargar el historial.")}finally{setLoading(false)}}
  ...
  async function undo(op){if(busy)return;setBusy(op.id);setMessage("");try{await undoPlanningHistory(op.id);setMessage("Operación deshecha. El Planning volvió al estado anterior.");await load()}catch(error){setMessage(error.message||"No se pudo deshacer la operación.")}finally{setBusy("")}}
