...
function pickup(current, past) { if (!current || !past) return null; return { rooms:current.rooms-past.rooms, revenue:current.revenue-past.revenue, occupancy:current.occupancy-past.occupancy } }
const pct = value => `${Math.round(value)}%`
const signed = value => `${value>=0?"+":""}${Math.round(value)}`
const metricDelta = (current, previous, inverse = false) => { if (!previous) return { text: current ? "Sin base comparable" : "Sin movimiento", tone: "neutral" }; const delta = (current - previous) / Math.abs(previous) * 100; const good = inverse ? delta <= 0 : delta >= 0; return { text: `${delta >= 0 ? "+" : ""}${delta.toFixed(1)}% vs período anterior`, tone: good ? "good" : "bad" } }
const short = value => new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "short" }).format(asDate(value))
function Kpi({ label, value, detail, delta }) { return <article className={s.kpi}><span>{label}</span><strong>{value}</strong><small>{detail}</small><em data-tone={delta.tone}>{delta.text}</em></article> }
function PickupCard({label,value,currency}){const available=Boolean(value);return <article className={s.pickupCard}><span>{label}</span><strong>{available?signed(value.rooms):"—"}<small> noches</small></strong><b>{available?`${value.revenue>=0?"+":""}${money(value.revenue,currency)}`:"Historial no disponible"}</b><em>{available?`${value.occupancy>=0?"+":""}${value.occupancy.toFixed(1)} pp de ocupación`:"Se completa automáticamente"}</em></article>}

export default function AnalyticsOverview({ rooms = [], reservations = [], payments = [], settings = {}, snapshots = [], channelCosts = [], canManageChannelCosts = false, onSaveChannelCost }) {
  ...
  const pace7=useMemo(()=>{const now=new Map(snapshots.filter(r=>r.captured_on===today&&r.stay_date>=today&&r.stay_date<futureEnd&&String(r.currency||"ARS").toUpperCase()===currency).map(r=>[r.stay_date,r]));const past=new Map(snapshots.filter(r=>r.captured_on===shift(today,-7)&&r.stay_date>=today&&r.stay_date<futureEnd&&String(r.currency||"ARS").toUpperCase()===currency).map(r=>[r.stay_date,r]));return [...now.entries()].map(([day,row])=>({day,now:Number(row.occupancy_on_books||0),past:Number(past.get(day)?.occupancy_on_books||0)})).sort((a,b)=>a.day.localeCompare(b.day))},[snapshots,today,futureEnd,currency])
  const cards = [
  ...
  return <main className={s.page}>
    <header className={s.hero}><div><small>INTELIGENCIA HOTELERA</small><h1>El negocio, en una sola lectura.</h1><p>Indicadores calculados con reservas, habitaciones y snapshots históricos reales del PMS.</p></div><div className={s.heroStatus}><span>PERÍODO</span><b>{range.label}</b><small>{short(range.start)} → {short(shift(range.end, -1))}</small></div></header>
    <section className={s.filters} aria-label="Filtros de inteligencia"><div className={s.periods}>{[["today","Hoy"],["7d","7 días"],["30d","30 días"],["month","Mes"],["year","Año"]].map(([id,label])=><button key={id} type="button" data-active={period===id} onClick={()=>setPeriod(id)}>{label}</button>)}</div>{currencies.length>1&&<label>Moneda<select value={currency} onChange={e=>setChosenCurrency(e.target.value)}>{currencies.map(item=><option key={item} value={item}>{item}</option>)}</select></label>}</section>
    ...
    <section className={s.pickupSection}><header><div><small>PICKUP · PRÓXIMOS 30 DÍAS</small><h2>Qué se vendió desde la última foto</h2></div><span>{booksNow?`${booksNow.rooms} noches OTB · ${money(booksNow.revenue,currency)}`:"Preparando historial"}</span></header><div className={s.pickupGrid}><PickupCard label="Últimas 24 horas" value={pickup1} currency={currency}/><PickupCard label="Últimos 7 días" value={pickup7} currency={currency}/><PickupCard label="Últimos 30 días" value={pickup30} currency={currency}/></div></section>
    <IntelligenceSignals reservations={reservations} payments={payments} snapshots={snapshots} currency={currency}/>
    <section className={s.grid}><article className={s.chartCard}><header><div><small>DESEMPEÑO DEL PERÍODO</small><h2>Ingresos de habitación y ocupación</h2></div><span>{money(current.roomRevenue,currency)} en alojamiento</span></header><div className={s.chart}>{series.map((row,index)=><div className={s.barColumn} key={`${row.day}-${index}`} title={`${short(row.day)} · ${pct(row.occupancy)} · ${money(row.roomRevenue,currency)}`}><div className={s.barTrack}><i style={{height:`${Math.max(3,row.roomRevenue/maxRevenue*100)}%`}}/><b style={{bottom:`${Math.min(94,row.occupancy)}%`}}/></div><small>{index===0||index===series.length-1||series.length<=12?short(row.day):""}</small></div>)}</div><footer><span><i/> Ingreso habitación</span><span><b/> Ocupación</span></footer></article><aside className={s.reading}><small>LECTURA RÁPIDA</small><h2>Qué está pasando</h2><div><span>01</span><p><b>{pct(current.occupancy)} de ocupación</b> sobre {current.capacity.toLocaleString("es-AR")} habitaciones-noche disponibles.</p></div><div><span>02</span><p><b>ADR {money(current.adr,currency)}</b> y RevPAR {money(current.revpar,currency)}.</p></div><div><span>03</span><p><b>{current.reservations} reservas activas</b> y {current.cancellations} cancelaciones/no-show dentro del período.</p></div><div><span>04</span><p><b>{money(current.roomRevenue,currency)} de alojamiento</b> dentro de {money(current.production,currency)} de producción asociada.</p></div></aside></section>
    <section className={s.paceCard}><header><div><small>PACE · PRÓXIMOS 30 DÍAS</small><h2>Ocupación hoy vs. hace 7 días</h2></div><span>Detecta fechas que aceleran o se frenan</span></header><div className={s.paceRows}>{pace7.length?pace7.map(row=>{const delta=row.now-row.past;return <div key={row.day} className={s.paceRow}><small>{short(row.day)}</small><div><i style={{width:`${Math.min(100,row.past)}%`}}/><b style={{width:`${Math.min(100,row.now)}%`}}/></div><strong data-tone={delta>2?"good":delta<-2?"bad":"neutral"}>{Math.round(row.now)}% <em>{delta>=0?"+":""}{delta.toFixed(0)} pp</em></strong></div>}):<p className={s.noHistory}>Todavía no hay suficiente historial para dibujar Pace. Los snapshots se generan automáticamente todos los días.</p>}</div></section>
    <ChannelPerformance reservations={reservations} start={range.start} end={range.end} currency={currency} costs={channelCosts} canManage={canManageChannelCosts} onSaveCost={onSaveChannelCost}/>
