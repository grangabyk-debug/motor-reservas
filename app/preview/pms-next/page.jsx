...
export default function PMSNextPreview() {
  ...
  const filteredReservations = reservations.filter(r => {
    ...
    return !q || [r.id, r.guest, r.roomId, r.channel].some(v => String(v).toLowerCase().includes(q))
  })
...
function Overview({ role, occupancy, arrivalsToday, departuresToday, availableNow, rooms, adr, revpar, directShare, reservations, channelMismatch, setView, setSelectedReservationId }) {
  ...
  return <div className="content overviewGrid">
    <section className="heroPanel">
      <div><span className="eyebrow">{role === "owner" ? "PULSO DEL NEGOCIO" : "PULSO DE RECEPCIÓN"}</span><h2>{role === "owner" ? "Un hotel legible en segundos." : "Todo lo importante, antes de que se vuelva urgente."}</h2><p>{channelMismatch ? "La operación está estable, pero Distribution detectó una diferencia de inventario que conviene reconciliar." : "Reservas, limpieza, precios y distribución están alineados. No hay alertas críticas de inventario."}</p><div className="heroButtons"><button onClick={() => setView("calendar")}>Abrir Command Center</button><button onClick={() => setView(channelMismatch ? "distribution" : "revenue")}>{channelMismatch ? "Resolver distribución" : "Ver revenue"}</button></div></div>
      <div className="orb"><b>{occupancy}%</b><span>ocupación hoy</span><small>{availableNow} disponibles ahora</small></div>
    </section>
    <section className="metricRow">
      {(role === "owner" ? [["ADR", fmtMoney(adr), "+4,2%"], ["RevPAR", fmtMoney(revpar), "+6,8%"], ["Venta directa", `${directShare}%`, "mejor margen"], ["Distribución", channelMismatch ? "Revisar" : "Saludable", channelMismatch ? "1 diferencia" : "sin diferencias"]] : [["Llegadas", arrivalsToday.length, "hoy"], ["Salidas", departuresToday.length, "hoy"], ["Listas", rooms.filter(r => r.status === "clean").length, "habitaciones"], ["Alertas", channelMismatch ? 1 : 0, channelMismatch ? "distribución" : "sin críticas"]]).map(([label, value, detail]) => <article key={label} className="metric"><span>{label}</span><b>{value}</b><small>{detail}</small></article>)}
    </section>
    <section className="card operationCard"><Header eyebrow="OPERACIÓN" title="El hotel de hoy" action="Ver calendario" onAction={() => setView("calendar")} /><div className="operationColumns"><OperationColumn title="Llegan" items={arrivalsToday} onSelect={setSelectedReservationId} /><OperationColumn title="Alojados" items={active} onSelect={setSelectedReservationId} /><OperationColumn title="Próximos" items={reservations.filter(r => r.start > TODAY).slice(0, 4)} onSelect={setSelectedReservationId} /></div></section>
    <section className="card intelligenceCard"><Header eyebrow="LLENA INTELLIGENCE" title="Decisiones, no ruido" action="Abrir copiloto" onAction={() => document.querySelector(".aiFloat")?.click()} /><div className="insightList"><Insight number="01" title="Demanda en ascenso" text="El próximo fin de semana admite un aumento controlado en suites." action="Aplicar en Revenue" onClick={() => setView("revenue")} /><Insight number="02" title="Perfil duplicado detectado" text="Sofía Martínez aparece en dos fuentes con el mismo teléfono." action="Resolver perfil" onClick={() => setView("guests")} /><Insight number="03" title="Distribution Health" text={channelMismatch ? "Hay un canal con inventario distinto al PMS." : "Todos los canales activos coinciden con el PMS."} action="Abrir Distribution" onClick={() => setView("distribution")} /></div></section>
  </div>
}
