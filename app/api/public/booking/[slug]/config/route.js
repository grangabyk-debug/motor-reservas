...
export async function GET(request,{params}){try{const{slug}=await params,client=publicClient(),{data,error}=await client.rpc("hl_public_booking_config_with_plans",{p_slug:slug});if(error)throw error;const ownOrigin=new URL(request.url).origin,allowed=Array.isArray(data?.allowed_origins)?data.allowed_origins.filter(Boolean):[],engine={...data,allowed_origins:[...new Set([ownOrigin,...allowed])]};return Response.json({engine},{headers:{"Cache-Control":"public, max-age=60, stale-while-revalidate=300"}})}catch(error){console.error("booking config",error);return err("Este motor de reservas no está disponible.",404)}}
