...
export async function POST(request,{params}){
  try{
    ...
    const payload={check_in:text(raw.check_in,10),check_out:text(raw.check_out,10),guests:Math.min(20,Math.max(1,Number(raw.guests)||1)),name:text(raw.name,160),email:text(raw.email,180),phone:text(raw.phone,80),room_type:text(raw.room_type,120),request_id:text(raw.request_id,120)}
