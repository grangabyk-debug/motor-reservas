...
import{listingIdFor,priceLabsCallbacks,requirePriceLabsRole}from"../../../../../lib/pricelabsServer"
import{priceLabsGoLiveState}from"../../../../../lib/pricelabsSync"
...
async function snapshot(db,propertyId,origin){
  ...
  const listingByType=new Map((listings||[]).map(row=>[String(row.room_type_id),row])),platform=priceLabsGoLiveState(),callbacks=priceLabsCallbacks(origin)
...
export async function POST(request){
  try{
    ...
    if(action==="save"){
      ...
      const payload={property_id:propertyId,user_email:email,status:existing.data?.status&&existing.data.status!=="setup"?existing.data.status:"setup",sync_enabled:existing.data?.sync_enabled===true,metadata:{...(existing.data?.metadata||{}),configured_from:"revenue",property_name:auth.property.name,email_saved_at:now},created_by:auth.user.id,updated_at:now}
      ...
      const platform=priceLabsGoLiveState(),nextStatus=platform.ready?"prepared":"pending_partner",updated=await db.from("hotel_pricelabs_connections").update({status:nextStatus,sync_enabled:false,last_error:null,metadata:{...(connection.data.metadata||{}),prepared_types:prepared,prepared_at:now,partner_credentials_ready:platform.ready,certified:platform.certified},updated_at:now}).eq("property_id",propertyId);if(updated.error)throw updated.error
