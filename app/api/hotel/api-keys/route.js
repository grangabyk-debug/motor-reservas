...
export async function GET(request){
  try{const propertyId=new URL(request.url).searchParams.get("property_id"),{client}=await context(request);if(!propertyId)return Response.json({error:"Falta la propiedad."},{status:400});const{data,error}=await client.rpc("hl_api_keys_snapshot",{p_property_id:propertyId});if(error)throw error;return Response.json(data||{keys:[],logs:[],metrics:{requests_24h:0,errors_24h:0},available_scopes:[]})}
  catch(error){return failure(error)}
}

export async function POST(request){
  try{const body=await request.json().catch(()=>({})),propertyId=String(body.property_id||""),name=String(body.name||"").trim().slice(0,80),{client}=await context(request);if(!propertyId)return Response.json({error:"Falta la propiedad."},{status:400});if(!name)return Response.json({error:"Ingresá un nombre para identificar la clave."},{status:400})
...
export async function PATCH(request){
  try{const body=await request.json().catch(()=>({})),propertyId=String(body.property_id||""),keyId=String(body.key_id||""),action=String(body.action||""),{client}=await context(request);if(!propertyId||!keyId||action!=="revoke")return Response.json({error:"Acción de clave inválida."},{status:400});const{data,error}=await client.rpc("hl_api_key_revoke",{p_property_id:propertyId,p_key_id:keyId});if(error)throw error;return Response.json({key:data})}
  catch(error){return failure(error)}
}
