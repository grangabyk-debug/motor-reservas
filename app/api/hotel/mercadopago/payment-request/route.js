...
async function requireAction(client,propertyId,action){const{data,error}=await client.rpc("hl_can_property_action",{p_property_id:propertyId,p_action:action});if(error)throw error;if(!data)throw Object.assign(new Error("Tu rol no tiene permiso para esta acción."),{status:403})}
...
export async function POST(request){
  try{
    ...
    const{accessToken}=await accessTokenFor(client,propertyId),externalReference=`hlpr_${String(row.id).replaceAll("-","")}`,notificationUrl=new URL("/api/hotel/mercadopago/webhook",request.url);notificationUrl.searchParams.set("request_id",row.id)
    const preference={items:[{id:`reservation_${reservation.id}`,title:`Reserva ${reservation.numero_reserva||reservation.id}`,description:"Saldo de alojamiento y servicios",quantity:1,currency_id:"ARS",unit_price:amount}],external_reference:externalReference,notification_url:notificationUrl.toString(),expires:true,expiration_date_to:expiresAt.toISOString(),metadata:{payment_request_id:row.id,reservation_id:String(reservation.id),property_id:String(propertyId)}};const payer=cleanEmail(reservation.email_huesped);if(payer)preference.payer={email:payer}
    const mp=await mpFetch("/checkout/preferences",{accessToken,method:"POST",body:preference,idempotencyKey:row.id}),{data:updated,error:updateError}=await client.from("hotel_payment_requests").update({status:"requested",provider_preference_id:String(mp.id||""),init_point:mp.init_point||null,sandbox_init_point:mp.sandbox_init_point||null,metadata:{external_reference:externalReference,notification_url:notificationUrl.toString()},updated_at:new Date().toISOString()}).eq("id",row.id).eq("property_id",propertyId).select("*").single();if(updateError)throw updateError
...
export async function PATCH(request){
  try{
    const{client,user}=await authContext(request),body=await request.json().catch(()=>({})),propertyId=body?.property_id,requestId=body?.request_id,action=body?.action;if(!propertyId||!requestId)return Response.json({error:"Falta identificar la solicitud."},{status:400});await requireRole(client,user.id,propertyId,ROLES);await requireAction(client,propertyId,"payments.verify_request")
