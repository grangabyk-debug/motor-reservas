...
export async function POST(request){
  try{
    ...
    outbound.append("file",file,file.name||"mensaje.webm");
    ...
    if(!text)return NextResponse.json({error:"No pude entender el audio."},{status:422});
    return NextResponse.json({text});
  }catch(error){
    console.error("COMANDA VOICE ERROR",error);
