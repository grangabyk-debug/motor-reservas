...
function extractText(data){
  ...
  const parts=[];for(const item of data?.output||[])for(const content of item?.content||[])if(content?.type==="output_text"&&content?.text)parts.push(content.text);return parts.join("\n").trim();
}
function compactHistory(rows){if(!Array.isArray(rows))return [];return rows.slice(-10).map(row=>({role:row?.role==="assistant"?"assistant":"user",text:String(row?.text||"").slice(0,900)})).filter(row=>row.text)}
function compactManual(value){if(!value||typeof value!=="object")return "Manual detallado no recibido.";return JSON.stringify(value).slice(0,18000)}
...
export async function POST(request){
  try{
    ...
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${apiKey}`},body:JSON.stringify({model:"gpt-5-mini",instructions:rules,input:task,max_output_tokens:520,store:false})});
    ...
    const answer=extractText(data);return NextResponse.json({answer:answer||"No pude generar una respuesta con suficiente evidencia."});
  }catch(error){console.error("COMANDA INSPECTOR ERROR",error);return NextResponse.json({error:"No pude procesar la consulta del Inspector."},{status:500})}
}
