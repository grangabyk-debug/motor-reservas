...
function safeNext(){if(typeof window==="undefined")return PMS_HOME;const value=new URL(window.location.href).searchParams.get("next");return value&&value.startsWith("/")&&!value.startsWith("//")?value:PMS_HOME}
...
export default function LoginPage(){
  ...
  useEffect(()=>{let active=true;const blockContextMenu=(event)=>event.preventDefault();document.addEventListener("contextmenu",blockContextMenu);(async()=>{const{data}=await supabase.auth.getUser();if(active&&data?.user)location.replace(safeNext())})();return()=>{active=false;document.removeEventListener("contextmenu",blockContextMenu)}},[])
  async function login(e){e.preventDefault();setMessage("");setLoading(true);const{error}=await supabase.auth.signInWithPassword({email:email.trim(),password});if(error){setMessage(error.message==="Invalid login credentials"?"Email o contraseña incorrectos.":error.message);setKind("error");setLoading(false);return}location.replace(safeNext())}
  async function recover(){setMessage("");const clean=email.trim();if(!clean){setMessage("Primero ingresá tu email.");setKind("error");return}setRecovering(true);const resetUrl=new URL("/reset-password",location.origin),next=safeNext();if(next!==PMS_HOME)resetUrl.searchParams.set("next",next);const{error}=await supabase.auth.resetPasswordForEmail(clean,{redirectTo:resetUrl.toString()});setRecovering(false);if(error){setMessage(error.message);setKind("error");return}setMessage("Te enviamos un email para recuperar tu acceso. Revisá también spam.");setKind("success")}
