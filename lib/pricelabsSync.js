...
export async function buildPriceLabsReservations(db,propertyId){
  ...
  for(const reservation of data.reservations){const assigned=roomIds(reservation),share=Math.max(1,assigned.length),total=number(reservation.precio_total)??0,rental=number(reservation.subtotal)??number(reservation.tarifa_noche)*Math.max(1,Number(reservation.noches||1))??total
    for(const roomId of assigned){const room=roomById.get(roomId),listing=listingByType.get(String(room?.room_type_id||""));if(!listing)continue;rows.push({listing_id:listing.listing_id,reservation_id:`${reservation.id}:${roomId}`,start_date:reservation.fecha_entrada,end_date:reservation.fecha_salida,total_cost:Math.round(total/share*100)/100,rental_revenue:Math.round((Number.isFinite(rental)?rental:total)/share*100)/100,currency:reservation.moneda||data.currency,pl_status:reservation.estado==="cancelada"||reservation.no_show?"cancelled":"booked",booked_time:reservation.created_at||new Date().toISOString(),guest_count:Math.max(1,Math.ceil(Number(reservation.cantidad_huespedes||1)/share))})}
  }
...
export function extractPriceLabsUpdates(payload){const updates=[],visit=(node,context={})=>{if(!node)return;if(Array.isArray(node)){node.forEach(item=>visit(item,context));return}if(typeof node!=="object")return;const listingId=pick(node,["listing_id","listingId","id"])||context.listingId,date=pick(node,["date","stay_date","stayDate","night"]),price=pick(node,["price","rate","recommended_price","recommendedPrice","nightly_rate","nightlyRate"]),minStay=pick(node,["min_stay","minStay","minimum_stay","minimumStay","min_nights"]),cta=pick(node,["closed_to_arrival","closedToArrival","cta"]),ctd=pick(node,["closed_to_departure","closedToDeparture","ctd"]);if(listingId&&validDate(date)&&number(price)!==null)updates.push({listing_id:String(listingId),date:String(date),price:number(price),min_stay:minStay==null?null:Math.max(1,Number(minStay)||1),closed_to_arrival:cta==null?null:bool(cta),closed_to_departure:ctd==null?null:bool(ctd)});const next={listingId:listingId||context.listingId};for(const value of Object.values(node))if(value&&typeof value==="object")visit(value,next)};visit(payload);const deduped=new Map();for(const row of updates)deduped.set(`${row.listing_id}:${row.date}`,row);return[...deduped.values()]}
...
export async function applyPriceLabsUpdates(db,payload){
  ...
  const propertyId=properties[0],roomTypes=[...new Set(mapped.map(row=>row.room_type_id))],roomResult=await db.from("habitaciones").select("id,room_type_id").eq("property_id",propertyId).eq("activa",true).in("room_type_id",roomTypes);if(roomResult.error)throw roomResult.error;const roomsByType=new Map();for(const room of roomResult.data||[]){const key=String(room.room_type_id);if(!roomsByType.has(key))roomsByType.set(key,[]);roomsByType.get(key).push(Number(room.id))}
  const mappingByListing=new Map(mapped.map(row=>[String(row.listing_id),row])),dates=[...new Set(updates.map(row=>row.date))],roomIdsFlat=[...new Set((roomResult.data||[]).map(row=>Number(row.id)))],existingResult=roomIdsFlat.length&&dates.length?await db.from("hotel_rate_calendar").select("property_id,habitacion_id,stay_date,price,min_stay,stop_sell,closed_to_arrival,closed_to_departure,notes").eq("property_id",propertyId).in("habitacion_id",roomIdsFlat).in("stay_date",dates):{data:[],error:null};if(existingResult.error)throw existingResult.error;const existing=new Map((existingResult.data||[]).map(row=>[`${Number(row.habitacion_id)}:${row.stay_date}`,row])),rows=[]
  for(const update of updates){const map=mappingByListing.get(update.listing_id);if(!map)continue;for(const roomId of roomsByType.get(String(map.room_type_id))||[]){const current=existing.get(`${roomId}:${update.date}`)||{};rows.push({property_id:propertyId,habitacion_id:roomId,stay_date:update.date,price:update.price,min_stay:update.min_stay??current.min_stay??1,stop_sell:current.stop_sell===true,closed_to_arrival:update.closed_to_arrival??current.closed_to_arrival??false,closed_to_departure:update.closed_to_departure??current.closed_to_departure??false,notes:"Tarifa dinámica aplicada desde PriceLabs",updated_at:new Date().toISOString()})}}
