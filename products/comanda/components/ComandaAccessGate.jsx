...
export default function ComandaAccessGate({children}){
  ...
  async function fallbackStaffWorkstations(branchId,staffRows){
    const ids=(staffRows||[]).filter(s=>s.branch_id===branchId).map(s=>s.id);if(!ids.length)return [];
  ...
  async function allowedWorkstations(branchId,accountId,wsLinks=userWsLinks,staffRows=staff){
    const {data:all,error:we}=await supabase.from("comanda_workstations").select("*").eq("branch_id",branchId).eq("active",true).order("name");if(we)throw we;
    if(ownerAccountIds.has(accountId)||elevatedAccounts.has(accountId))return all||[];
    ...
    const ids=new Set([...directIds,...legacyIds]);return (all||[]).filter(w=>ids.has(w.id));
  }

  useEffect(()=>{let alive=true;(async()=>{try{
    const {data:{user:u}}=await supabase.auth.getUser();if(!u){location.href="/comanda/login";return}
    ...
    const {data:branchRows,error:be}=await supabase.from("comanda_branches").select("*").in("account_id",accountIds).eq("active",true).order("name");if(be)throw be;
    ...
    if(!alive)return;
    setUser(u);setOwned(accounts||[]);setElevatedAccounts(elevated);setBranches(allowed);setUserWsLinks(wsLinks||[]);setStaff(staffRows||[]);
    ...
    if(savedBranch&&savedWs&&allowed.some(b=>b.id===savedBranch)){
      const b=allowed.find(x=>x.id===savedBranch);const ws=await allowedWorkstations(savedBranch,b.account_id,wsLinks||[],staffRows||[]);
      if(ws.some(w=>w.id===savedWs)){setBranch(savedBranch);setWorkstations(ws);setReady(true);setLoading(false);return}
    }
    sessionStorage.removeItem("comanda_branch");sessionStorage.removeItem("comanda_workstation");setLoading(false);
  }catch(e){if(alive){setError(e?.message||"No se pudieron cargar tus accesos.");setLoading(false)}}})();return()=>{alive=false}},[]);

  async function chooseBranch(b){try{const ws=await allowedWorkstations(b.id,b.account_id);sessionStorage.setItem("comanda_branch",b.id);sessionStorage.removeItem("comanda_workstation");setBranch(b.id);setWorkstations(ws)}catch(e){setError(e?.message||"No se pudieron cargar los puestos.")}}
  ...
  return <div style={wrap}><main style={shell}><div style={{marginBottom:28}}><h1 style={{margin:"0 0 6px",fontSize:30}}>Comanda Llena</h1><p style={muted}>{selected?"Elegí el puesto de trabajo":"Elegí la sucursal con la que vas a trabajar"}</p></div>
