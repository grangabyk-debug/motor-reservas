...
export default function ComandaWaiterCash(){
  ...
  const canUseCash=station?.permissions?.all===true||station?.permissions?.cash_open_close===true||["waiter","cashier","principal","support"].includes(station?.kind);
  const fullCash=station?.permissions?.all===true||station?.permissions?.cash_full===true||["principal","support"].includes(station?.kind);
  ...
  async function refresh(){
    ...
    setRegisters(regs||[]);
    if(!registerId&&regs?.[0])setRegisterId(regs[0].id);
    const ids=(regs||[]).map(x=>x.id);
    if(ids.length){
      const {data:s}=await supabase.from("comanda_cash_sessions").select("*").in("register_id",ids).eq("status","open").order("opened_at",{ascending:false}).limit(1).maybeSingle();
      setSession(s||null);
  ...
  async function submit(){
    ...
    try{
      ...
      if(session){
        const {error}=await supabase.from("comanda_cash_sessions").update({status:"closed",closed_by:user.id,closed_at:new Date().toISOString(),closing_amount:Number(amount||0)}).eq("id",session.id);
        if(error)throw error;
      }else{
        const reg=registers.find(x=>x.id===registerId);
        if(!reg)throw new Error("Seleccioná una caja");
        const {error}=await supabase.from("comanda_cash_sessions").insert({account_id:reg.account_id,register_id:reg.id,opened_by:user.id,opened_at:new Date().toISOString(),opening_amount:Number(amount||0),status:"open",workstation_id:workstationId||null,cashier_name:userEmail||user.email||"Usuario"});
        if(error)throw error;
      }
  ...
  return <>
    ...
    {open&&<div className={c.overlay} onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}><section className={c.modal}>
      <header><div><span>CAJA OPERATIVA</span><h2>{session?"Cerrar caja":"Abrir caja"}</h2></div><button onClick={()=>setOpen(false)}>×</button></header>
      <div className={c.body}>{session?<><p>Este puesto sólo puede abrir o cerrar la caja. El usuario queda auditado automáticamente.</p><div className={c.audit}><span>Abierta por</span><strong>{session.cashier_name||"Usuario"}</strong><span>Apertura</span><strong>{new Date(session.opened_at).toLocaleString("es-AR")}</strong></div></>:<><p>Seleccioná una caja y cargá el monto inicial.</p><label>Caja<select value={registerId} onChange={e=>setRegisterId(e.target.value)}>{registers.map(r=><option key={r.id} value={r.id}>{r.name} #{r.register_number}</option>)}</select></label></>}<label>{session?"Monto de cierre":"Monto inicial"}<input type="number" min="0" value={amount} onChange={e=>setAmount(e.target.value)}/></label><small>Usuario auditado: {userEmail||"sesión actual"}</small></div>
