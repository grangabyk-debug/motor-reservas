...
function resolveCfg(role,permissions={}){
  ...
  return {
    ...
    salonEdit:all||permissions?.tables_layout===true,
    cashMode:all||permissions?.cash_full===true?"full":permissions?.cash_open_close===true?"open_close":base.cashMode,
    sale:all||permissions?.sale===true||base.sale,
    kds:permissions?.kds===true||base.kds,
    print:all||permissions?.print_queue===true||base.print,
    menuEdit:all||permissions?.menu_edit===true,
    reports:all||permissions?.reports===true,
    customers:all||permissions?.customers===true
  };
}
...
export default function ComandaRoleGuard(){
  ...
  useEffect(()=>{
    if(!station)return;
    const {role,permissions}=station,cfg=resolveCfg(role,permissions);
    ...
    const forbiddenLabel=(label)=>
      (!cfg.admin&&ADMIN_LABELS.has(label))||
      (!cfg.salonEdit&&SALON_EDIT_LABELS.has(label))||
      (cfg.cashMode!=="full"&&FULL_CASH_LABELS.has(label))||
      (!cfg.menuEdit&&MENU_EDIT_LABELS.has(label))||
      (!cfg.reports&&REPORT_LABELS.has(label))||
      (!cfg.customers&&CUSTOMER_ADMIN_LABELS.has(label));

    const apply=()=>{
      for(const el of document.querySelectorAll("button,a,[role=button]")){
        const label=txt(el);
        if(forbiddenLabel(label)&&!el.closest('[data-session-ui="1"]'))el.style.display="none";
      }
      // Operational roles never see the salon-layout editor, even if a stale admin screen remains mounted.
      if(!cfg.salonEdit){for(const el of document.querySelectorAll('[data-sector-planner="1"]'))el.style.display="none";}
    };

    const deny=(e)=>{const el=e.target?.closest?.("button,a,[role=button]");if(!el)return;if(!forbiddenLabel(txt(el)))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();};
    ...
    const t=setTimeout(()=>{if(role==="waiter"||role==="custom")clickLegacy(["Venta"]);else if(role==="kitchen")clickLegacy(["Cocina"]);else if(role==="bar")clickLegacy(["Bar"]);},180);
