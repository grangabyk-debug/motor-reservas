...
begin
  ...
  foreach rid in array ids loop
    ...
    if coalesce(checkout->>rid::text,'') ~ '^\d{4}-\d{2}-\d{2}$' then
    ...
    if d is null then raise exception using errcode='22023',message=format('La habitación %s ya no está activa dentro del grupo.',rid); end if;
    ...
    where coalesce(svc->>'room_id',svc->>'habitacion_id','') ~ '^\d+$'
  ...
  where coalesce(e->>'fecha_salida','') ~ '^\d{4}-\d{2}-\d{2}$'
    and lower(coalesce(e->>'segment_role','active_room')) not in('previous_room','transient_room','cancelled_room','no_show_room');
  ...
  where lower(coalesce(e->>'segment_role','active_room')) not in('previous_room','transient_room','cancelled_room','no_show_room')
    and not (coalesce(checkout->>coalesce(e->>'habitacion_id',''),'') ~ '^\d{4}-\d{2}-\d{2}$');
  ...
  values(r.property_id,r.id,'group_room_extension','Extensión de habitaciones',
    format('%s habitación(es) extendidas hasta %s.',cardinality(ids),p_new_end),
