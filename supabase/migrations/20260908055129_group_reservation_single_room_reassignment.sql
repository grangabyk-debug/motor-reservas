create or replace function public.hl_change_group_reservation_room_atomic(
  p_reserva_id bigint,
...
begin
  ...
  if coalesce(cardinality(v_before.habitaciones_ids),case when v_before.habitacion_id is null then 0 else 1 end)<=1 then
    raise exception using errcode='22023',message='Esta operación es para reservas con varias habitaciones.';
  end if;
  ...
  if exists(
    ...
    where d.property_id=v_before.property_id
      and d.reservation_id=v_before.id
  ...
  if v_source_detail is null then
    raise exception using errcode='P0002',message='No encontramos el detalle de esa habitación dentro del grupo.';
  end if;
  ...
  if coalesce(v_before.room_checkout_dates,'{}'::jsonb) ? p_from_room_id::text then
    raise exception using errcode='P0001',message='La habitación ya fue liberada mediante check-out individual y no se puede reasignar.';
  end if;

  select * into v_source
  from public.habitaciones
  where id=p_from_room_id and property_id=v_before.property_id;

  select * into v_target
  from public.habitaciones
  where id=p_to_room_id and property_id=v_before.property_id and activa is distinct from false;
  if not found then
    raise exception using errcode='P0002',message='Habitación de destino inexistente o inactiva.';
  end if;
  ...
  if v_start is null or v_end is null or v_end<=v_start then
    raise exception using errcode='22007',message='La habitación de origen no tiene una estadía válida.';
  end if;
  ...
  if exists(
    ...
    where r.property_id=v_before.property_id
      ...
      and public.hl_reservation_room_effective_end_date(r.habitaciones_detalle,r.room_checkout_dates,p_to_room_id,r.fecha_salida)>v_start
  ) then
    raise exception using errcode='23P01',message=format('La habitación %s está ocupada durante parte de %s → %s.',coalesce(v_target.nombre,p_to_room_id::text),v_start,v_end);
  end if;

  if exists(
    select 1 from public.bloqueos b
    where b.property_id=v_before.property_id
      and b.habitacion_id=p_to_room_id
      and b.fecha_desde<v_end
      and b.fecha_hasta>v_start
  ) then
    raise exception using errcode='23P01',message=format('La habitación %s tiene un bloqueo operativo durante esas fechas.',coalesce(v_target.nombre,p_to_room_id::text));
  end if;

  if exists(
    select 1 from public.hotel_folios f
    where f.reservation_id=v_before.id
      and f.room_id=p_to_room_id
      and f.folio_type='room'
  ) then
    raise exception using errcode='23505',message='La reserva ya tiene un folio vinculado a la habitación de destino.';
  end if;
  ...
  v_rate_delta:=round((v_new_rate-v_old_rate)*v_nights,2);

  select coalesce(jsonb_agg(
    case
      when coalesce(d->>'habitacion_id','') ~ '^\d+$' and (d->>'habitacion_id')::bigint=p_from_room_id then
  ...
  select array_agg(case when x=p_from_room_id then p_to_room_id else x end order by ord)
  into v_new_ids
  from unnest(coalesce(v_before.habitaciones_ids,array[v_before.habitacion_id]::bigint[])) with ordinality as t(x,ord);
  ...
  set room_id=p_to_room_id,
      label='Habitación '||coalesce(v_target.nombre,p_to_room_id::text),
      updated_at=now()
  where property_id=v_before.property_id
    and reservation_id=v_before.id
    and room_id=p_from_room_id
    and folio_type='room';

  update public.hotel_folio_items
  set room_id=p_to_room_id,
      source_key=case when source_type='lodging' and source_key='lodging:'||p_from_room_id::text then 'lodging:'||p_to_room_id::text else source_key end,
      updated_at=now()
  where property_id=v_before.property_id
    and reservation_id=v_before.id
    and room_id=p_from_room_id
  ...
  update public.hotel_group_rooming
  set room_id=p_to_room_id,updated_at=now()
  where property_id=v_before.property_id
    and reservation_id=v_before.id
    and room_id=p_from_room_id;
  ...
  set habitacion_id=case when habitacion_id=p_from_room_id then p_to_room_id else habitacion_id end,
  ...
  insert into public.hotel_reservation_events(
    ...
    v_before.property_id,
    v_before.id,
    'room',
    'Cambio de habitación en reserva grupal',
    format('Habitación %s → %s · %s → %s%s',coalesce(v_source.nombre,p_from_room_id::text),coalesce(v_target.nombre,p_to_room_id::text),v_start,v_end,case when p_reprice then format(' · tarifa %s → %s',v_old_rate,v_new_rate) else ' · tarifa original mantenida' end),
    jsonb_build_object(
      'group_room_move',true,
      'from_room_id',p_from_room_id,
      'to_room_id',p_to_room_id,
      'before_room_name',coalesce(v_source.nombre,p_from_room_id::text),
      'after_room_name',coalesce(v_target.nombre,p_to_room_id::text),
      'before_category',v_source.tipo,
      'after_category',v_target.tipo,
      'before_rate',v_old_rate,
      'after_rate',v_new_rate,
  ...
  insert into public.hotel_planning_operation_log(
    ...
    v_operation_group,v_before.property_id,'change_room',v_before.id,
    private.hl_planning_reservation_state(v_before),private.hl_planning_reservation_state(v_after),
    jsonb_build_object('group_room_move',true,'from_room_id',p_from_room_id,'to_room_id',p_to_room_id,'from_room_name',v_source.nombre,'to_room_name',v_target.nombre,'reprice',coalesce(p_reprice,false),'old_rate',v_old_rate,'new_rate',v_new_rate,'room_start',v_start,'room_end',v_end),
    v_actor
  );
...
revoke all on function public.hl_change_group_reservation_room_atomic(bigint,bigint,bigint,boolean) from public;
grant execute on function public.hl_change_group_reservation_room_atomic(bigint,bigint,bigint,boolean) to authenticated;
