create or replace function public.hl_cancel_reservation_policy_atomic(p_reserva_id bigint, p_penalty_status text default 'none'::text, p_note text default null::text)
...
begin
  ...
  -- La fecha de llegada no equivale a check-in. Mientras el huésped no haya
  -- ingresado realmente (estado = alojado), la reserva puede cancelarse.
  v_policy:=coalesce(v.cancellation_policy_snapshot,'{}'::jsonb);
  v_days:=greatest(0,v.fecha_entrada-current_date);
  ...
  v_type:=coalesce(v_rule->>'charge_type','none');
  ...
  if v_penalty<=0.01 then v_status:='none';
  elsif v_status='none' then raise exception using errcode='P0001',message='La política de cancelación prevé una penalidad de '||v_penalty::text||' '||coalesce(v.moneda,'ARS')||'. Indicá si fue cobrada o eximida.';
  end if;
