...
declare
  ...
  v_room_start date;
  v_room_end date;
  v_room_start_ts timestamp;
  v_room_end_ts timestamp;
begin
  if tg_op='UPDATE' then
    ...
    if v_old_count>1 and v_new_count<v_old_count and old.habitaciones_ids is distinct from new.habitaciones_ids and coalesce(current_setting('app.hl_group_room_operation',true),'')<>'on' then
  ...
  if v_end<=v_start then raise exception using errcode='22007',message='La hora de salida debe ser posterior a la hora de entrada.'; end if;
  ...
  if coalesce(array_length(v_room_ids,1),0)=0 then
    if coalesce(current_setting('app.hl_import_mode',true),'')='on' and coalesce(new.codigo_canal,'') like 'BDC:%' then return new; end if;
  ...
  foreach v_room_id in array v_room_ids loop
    ...
    v_room_end:=public.hl_reservation_room_effective_end_date(new.habitaciones_detalle,new.room_checkout_dates,v_room_id,new.fecha_salida);
    if v_room_end<=v_room_start then
      if coalesce(new.room_checkout_dates,'{}'::jsonb)?v_room_id::text then continue; end if;
      raise exception using errcode='22007',message='Cada habitación necesita una salida posterior a su entrada.';
    end if;
    v_room_start_ts:=v_room_start::timestamp+v_arrival;
    v_room_end_ts:=v_room_end::timestamp+v_departure;
    ...
    if exists(
      ...
      where r.property_id=new.property_id
        ...
        and tsrange(
          public.hl_reservation_room_start_date(r.habitaciones_detalle,v_room_id,r.fecha_entrada)::timestamp+private.hl_safe_time(r.hora_llegada_estimada,v_checkin),
          greatest(
            public.hl_reservation_room_effective_end_date(r.habitaciones_detalle,r.room_checkout_dates,v_room_id,r.fecha_salida)::timestamp+private.hl_safe_time(r.hora_salida_estimada,v_checkout),
            public.hl_reservation_room_start_date(r.habitaciones_detalle,v_room_id,r.fecha_entrada)::timestamp+private.hl_safe_time(r.hora_llegada_estimada,v_checkin)
          ),
          '[)'
        ) && tsrange(v_room_start_ts,v_room_end_ts,'[)')
    ) then
      raise exception using errcode='23P01',message=format('La habitación %s ya está ocupada durante parte de ese horario.',v_room_label);
    end if;
