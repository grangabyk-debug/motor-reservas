-- Guarda tarifa comercial explícita en eventos de cambio de habitación grupal durante estadía.
CREATE OR REPLACE FUNCTION public.hl_move_inhouse_group_room_segment_atomic_legacy_folios_v1(p_reserva_id bigint, p_from_room_id bigint, p_to_room_id bigint, p_effective_date date, p_reprice boolean DEFAULT false)
 RETURNS reservas
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  values(
    ...
    format(
      'Hab. %s hasta %s → Hab. %s desde %s hasta %s%s',
      coalesce(v_source.nombre,p_from_room_id::text),p_effective_date,coalesce(v_target.nombre,p_to_room_id::text),p_effective_date,v_end,
      case when p_reprice then format(
        ' · tarifa comercial %s → %s',
        (private.hl_price_breakdown_from_net(v_before.property_id,v_old_rate)->>'commercial'),
        (private.hl_price_breakdown_from_net(v_before.property_id,v_new_rate)->>'commercial')
      ) else ' · tarifa original mantenida' end
    ),
    jsonb_build_object(
      'mid_stay_split',true,
      'from_room_id',p_from_room_id,
      'to_room_id',p_to_room_id,
      'split_date',p_effective_date,
      'room_start',v_start,
      'room_end',v_end,
      'before_rate',v_old_rate,
      'after_rate',v_new_rate,
      'before_commercial_rate',(private.hl_price_breakdown_from_net(v_before.property_id,v_old_rate)->>'commercial')::numeric,
      'after_commercial_rate',(private.hl_price_breakdown_from_net(v_before.property_id,v_new_rate)->>'commercial')::numeric,
      'price_tax_mode',private.hl_price_breakdown_from_net(v_before.property_id,v_new_rate)->>'price_tax_mode',
      'vat_rate',(private.hl_price_breakdown_from_net(v_before.property_id,v_new_rate)->>'vat_rate')::numeric,
      'rate_currency',private.hl_price_breakdown_from_net(v_before.property_id,v_new_rate)->>'rate_currency',
