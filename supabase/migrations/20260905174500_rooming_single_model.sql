-- Unifica el armado de camas y la categoría comercial en habitaciones_detalle.
-- categoria_vendida permanece estable aunque cambie la habitación física;
-- categoria_asignada refleja siempre la categoría de la habitación física actual.
...
set habitaciones_detalle = coalesce((
  ...
  from unnest(
    case when coalesce(cardinality(r.habitaciones_ids),0)>0
      then r.habitaciones_ids
      else array[r.habitacion_id]
    end
  ) with ordinality u(room_id,ord)
  ...
  cross join lateral (
    select least(
      ...
      greatest(1,ceil(
        greatest(1,coalesce(r.cantidad_huespedes,1))::numeric /
        greatest(1,coalesce(cardinality(r.habitaciones_ids),1))
      )::int)
    ) guests
  ) g
),'[]'::jsonb);

create or replace function public.hl_create_reservation_atomic(
  p_reservation jsonb,
...
begin
  ...
  insert into public.reservas(
    ...
    jsonb_build_array(jsonb_build_object(
      ...
      'nombre',h.nombre,
      'categoria_asignada',coalesce(nullif(trim(h.tipo),''),'Habitación'),
      'categoria_vendida',v_type,
      'huespedes',v_guests,
      'tarifa_noche',v_nightly,
      'rooming',v_rooming
    )),
