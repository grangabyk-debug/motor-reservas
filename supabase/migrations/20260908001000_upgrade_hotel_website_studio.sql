alter table public.hotel_websites add column if not exists studio_config jsonb not null default '{}'::jsonb;
alter table public.hotel_websites add column if not exists published_studio_config jsonb not null default '{}'::jsonb;
...
begin
  ...
  return jsonb_build_object(
    ...
    'allowed_origins',e.allowed_origins,'min_advance_days',e.min_advance_days,'max_advance_days',e.max_advance_days,'min_nights',e.min_nights,'template',e.template,
    ...
    'terms_url',e.terms_url,'privacy_url',e.privacy_url,'payment_mode',e.payment_mode,'deposit_percent',e.deposit_percent,'room_type_rules',e.room_type_rules,
    'gallery',e.gallery,'seo_title',e.seo_title,'seo_description',e.seo_description,'allow_self_cancel',e.allow_self_cancel,'self_cancel_cutoff_hours',e.self_cancel_cutoff_hours,'withdrawal_button_enabled',true,
    'website_mode',coalesce(w.mode,'managed'),'website_enabled',coalesce(w.enabled,true),'website_custom_domain',w.custom_domain,'website_domain_status',coalesce(w.domain_status,'not_connected'),
    'website_seo_indexable',coalesce(w.seo_indexable,true),'website_seo_canonical_url',w.seo_canonical_url,'website_sections',coalesce(w.published_sections,w.sections,'[]'::jsonb),
    'website_studio_config',coalesce(w.published_studio_config,w.studio_config,'{}'::jsonb),'website_languages',coalesce(w.languages,array['es']::text[]),'website_default_language',coalesce(w.default_language,'es'),
    'floating_booking_enabled',coalesce(w.floating_booking_enabled,true),'floating_booking_label',coalesce(nullif(w.floating_booking_label,''),'Reservar'),'floating_booking_position',coalesce(w.floating_booking_position,'bottom_right'),
    'analytics_enabled',coalesce(w.analytics_enabled,true),'analytics_ga4_id',w.analytics_ga4_id,'analytics_gtm_id',w.analytics_gtm_id,'analytics_meta_pixel_id',w.analytics_meta_pixel_id,
    'website_room_types',v_room_types,'vouchers',v_vouchers
  );
end;
$function$;
