...
begin
  ...
  if new.habitacion_id is distinct from old.habitacion_id then
    select h.nombre into v_before_room from public.habitaciones h where h.id=old.habitacion_id and h.property_id=new.property_id;
    select h.nombre into v_after_room from public.habitaciones h where h.id=new.habitacion_id and h.property_id=new.property_id;
    insert into public.hotel_reservation_events(property_id,reservation_id,event_type,title,detail,payload,actor_user_id,actor_name)
    values(
      new.property_id,new.id,'room','Habitación modificada',
      'Habitación '||coalesce(v_before_room,old.habitacion_id::text,'Sin asignar')||' → Habitación '||coalesce(v_after_room,new.habitacion_id::text,'Sin asignar'),
      jsonb_build_object(
        'before_room_id',coalesce(v_before_room,old.habitacion_id::text),
        'after_room_id',coalesce(v_after_room,new.habitacion_id::text),
        'before_room_pk',old.habitacion_id,
        'after_room_pk',new.habitacion_id,
        'before_room_name',v_before_room,
        'after_room_name',v_after_room
      ),v_actor,v_name
    );
  end if;
...
with room_events as (
  select e.id,e.property_id,e.payload,
         case when coalesce(e.payload->>'before_room_pk',e.payload->>'before_room_id') ~ '^[0-9]+$' then coalesce(e.payload->>'before_room_pk',e.payload->>'before_room_id')::bigint end as before_pk,
         case when coalesce(e.payload->>'after_room_pk',e.payload->>'after_room_id') ~ '^[0-9]+$' then coalesce(e.payload->>'after_room_pk',e.payload->>'after_room_id')::bigint end as after_pk
  from public.hotel_reservation_events e
  where e.event_type='room'
), resolved as (
  select re.id,re.before_pk,re.after_pk,hb.nombre as before_name,ha.nombre as after_name
  from room_events re
  left join public.habitaciones hb on hb.id=re.before_pk and hb.property_id=re.property_id
  left join public.habitaciones ha on ha.id=re.after_pk and ha.property_id=re.property_id
)
update public.hotel_reservation_events e
set payload = coalesce(e.payload,'{}'::jsonb) || jsonb_build_object(
      'before_room_id',coalesce(r.before_name,r.before_pk::text,e.payload->>'before_room_id'),
      'after_room_id',coalesce(r.after_name,r.after_pk::text,e.payload->>'after_room_id'),
      'before_room_pk',r.before_pk,
      'after_room_pk',r.after_pk,
      'before_room_name',r.before_name,
      'after_room_name',r.after_name
    ),
    detail = 'Habitación '||coalesce(r.before_name,r.before_pk::text,'Sin asignar')||' → Habitación '||coalesce(r.after_name,r.after_pk::text,'Sin asignar')
from resolved r
where e.id=r.id;
